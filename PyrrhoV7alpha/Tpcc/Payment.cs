...
namespace tpcc
{
	...
	public class Payment : VTerm
	{
		...
		bool FetchCustFromLast(ref string mess)
		{
			...
			using (IDbCommand cmd = db.CreateCommand())
			{
				cmd.CommandText= "select c_id,c_first,c_middle,c_street_1,c_street_2,c_city,c_state,c_zip,c_phone,c_since,c_credit,c_credit_lim,c_discount,c_balance,c_ytd_payment,c_payment_cnt from customer where c_wid=" + cwid + " and c_d_id=" + cdid + " and  c_last='" + clast+"' order by c_first";
				using (IDataReader rdr = cmd.ExecuteReader())
				{
					rdr.Read();
					Set(17,(string)rdr[1]);
		...
		bool DoPayment(ref string mess)
		{
			...
			using (IDbCommand cmd = db.CreateCommand())
			{
				cmd.CommandText = "update district set d_ytd="+(dytd+camount)+" where d_w_id="+wid+" and d_id="+did;
				cmd.ExecuteNonQuery();
			}
			Set(36,(c_balance+camount).ToString("F2"));
			using (IDbCommand cmd = db.CreateCommand())
			{
				cmd.CommandText = "update customer set c_balance="+(camount+c_balance)+",c_ytd_payment="+(camount+c_ytd_payment)+",c_payment_cnt="+(c_payment_cnt+1)+" where c_w_id="+cwid+" and c_d_id="+cdid+" and c_id="+cid;
				cmd.ExecuteNonQuery();
			}
			using (IDbCommand cmd = db.CreateCommand())
			{
				cmd.CommandText = "update warehouse set w_ytd="+(ytd+camount)+" where w_id="+wid;
				cmd.ExecuteNonQuery();
			}
			if (c_credit=="BC")
			{
				...
				using (IDbCommand cmd = db.CreateCommand())
				{
					cmd.CommandText = "update customer set c_data='"+cdata+"' where c_w_id="+cwid+" and c_d_id="+cdid+" and c_id="+cid;
					cmd.ExecuteNonQuery();
				}
