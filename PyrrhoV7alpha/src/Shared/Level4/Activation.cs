...
namespace Pyrrho.Level4
{
    ...
    internal class TargetActivation : Activation
    {
            ...
            : base(cx,"")
        {
            ...
            _fm = fm;
            _tgt = fm.target;
            _td = cx._Dom(cx._Ob(_tgt))??throw new PEException("PE634");
        }
    ...
    internal class TableActivation : TargetActivation
    {
           ...
           Domain? iC=null) : base(cx,ts,tt)
        {
            ...
            for (var b = table.rindexes.First(); b != null; b = b.Next())
                if (db.objects[b.key()] is Table rt && cx._Dom(rt) is Domain rd)
                {
            ...
            switch (tt & (PTrigger.TrigType)7)
            {
                ...
                case PTrigger.TrigType.Update:
                    ...
                    for (var ass = ts.assig.First(); ass != null; ass = ass.Next())
                    {
                        ...
                        else if (cx._Dom(ts)?.representation.Contains(ua.vbl)==true)
                        {
        ...
        internal override void EachRow(int pos)
        {
            ...
            var trc = (cu!=null)?new TransitionRowSet.TransitionCursor(this, _trs, cu, pos, insertCols)
...
//#endif
        ...
        internal bool? Triggers(PTrigger.TrigType fg) // flags to add
        {
            ...
            for (var a = acts?.First(); r != true && a != null; a = a.Next())
                if (a.value() is TriggerActivation ta && ta._trig!=null)
                {
                    ta.cursors = cursors;
                    ta.values = values;
                    ta.db = db;
    ...
    internal class RESTActivation : TargetActivation
    {
        ...
        internal RESTActivation(Context cx, RowSet ts, PTrigger.TrigType tgt)
            : base(cx, ts, tgt)
        {
            if (cx.obs[ts.rsTargets.First()?.value() ?? -1L] is RowSet r0 &&
                ((r0 is RestRowSetUsing ru) ? cx.obs[ru.template] : r0) is RestRowSet rr &&
                cx.db.objects[ts.target] is RestView vw && role != null)
            { _rr = rr; _vw = vw; }
            else
                throw new PEException("PE649");
            _tgt = _rr.defpos;
        ...
        internal override void EachRow(int pos)
        {
                ...
                obs[ru.usingTableRowSet] is TableRowSet ut && db.objects[ut.target] is Table tb)
            {
                ...
                var vw = _Ob(_vw.viewPpos) ?? throw new DBException("42105");
                var vi = vw.infos[role.defpos];
                var ub = new StringBuilder(url);
                for (var b = _rr.matches.First(); b != null; b = b.Next())
                    if (_cx.obs[b.key()] is SqlValue sv)
                    {
                        var kn = sv.name;
                        ub.Append('/'); ub.Append(kn);
                        ub.Append('='); ub.Append(b.value());
                ...
                case PTrigger.TrigType.Update:
                    {
                        ...
                        for (var b = assig.First(); b != null; b = b.Next())
                            if (b.key() is UpdateAssignment ua && obs[ua.vbl] is SqlValue vb
                                    && obs[ua.val] is SqlValue va)
                            {
                                _sql.Append(cm); cm = ",";
    ...
    internal class HTTPActivation: TargetActivation
    {
        ...
        internal HTTPActivation(Context cx, RowSet ts,  PTrigger.TrigType tgt)
            : base(cx, ts, tgt)
        {
            _rr = (RestRowSet)(cx.obs[ts.rsTargets.First()?.value()??-1L]
                ??throw new DBException("42105"));
            _vw = (RestView)(cx.db.objects[ts.target]??throw new DBException("42105"));
            _tgt = _rr.defpos;
            switch (tgt & (PTrigger.TrigType)7)
            {
                case PTrigger.TrigType.Insert:
        ...
        internal override void EachRow(int pos)
        {
            ...
            if (cu == null)
                return;
            var vs = cu.values;
            if (role is not Role ro || _Ob(_vw.viewPpos) is not DBObject vw ||
                vw.infos[ro.defpos] is not ObInfo vi)
                throw new DBException("42105");
    ...
    internal class TriggerActivation : Activation
    {
            ...
            : base(cx, ot)
        {
                ...
                && cx.db.objects[trs.target] is Table tb && cx._Dom(tb) is Domain dm)
            {
                ...
                if (cx.obs[tg.oldTable] is TransitionTable tt && cx._Dom(tt) is Domain td)
                    new TransitionTableRowSet(tt.defpos, cx, trs, td, true);
                if (defer)
                    cx.deferred += this;
            }
        }
        ...
        internal bool Exec()
        {
            ...
            if (obs[_trig.action] is WhenPart wp)
            {
                ...
                if (tc != TBool.False)
                {
                    ...
                    if (next.values[_trs.target] is not TypedValue v)
