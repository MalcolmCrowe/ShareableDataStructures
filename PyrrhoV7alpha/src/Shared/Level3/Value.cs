...
namespace Pyrrho.Level3
{
    ...
    : base(nm.uid, cx.DoDepth((m ?? BTree<long, object>.Empty) + (ObInfo.Name, nm.ident)
            ...
            BTree<long, object> m, long ap)
        {
            ...
            if (name != null && ns.Contains(name) && cx.obs[ns[name]] is DBObject ob
        ...
        /// <param name="dm"></param>
        /// <returns></returns>
        internal virtual QlValue Having(Context c, Domain dm)
        {
    ...
    internal class SqlReview : QlValue
    {
        ...
        internal override ObTree _Apply(Context cx, DBObject ob, ObTree f)
        {
            if (ob.infos[cx.role.defpos] is ObInfo oi && name != null
                && oi.names[name] is long cp && cx.obs[cp] is TableColumn tc)
                return f + (defpos,new QlInstance(defpos, cx, name, from, tc));
    ...
    internal class QlInstance : QlValue
    {
        ...
        public QlInstance(long dp, Context cx, string nm, long fp, long cp, BTree<long,object>?m=null)
            :this(dp,cx,nm,fp,(DBObject?)(cx.obs[cp] ?? cx.db.objects[cp]),m)
        { }
        public QlInstance(long dp, Context cx, string nm, long fp, DBObject? cf, BTree<long, object>? m = null)
        ...
        internal override (DBObject?, Ident?) _Lookup(long lp, Context cx, Ident ic, Ident? n, DBObject? r)
        {
            if (n?.ident is string s && domain.infos[cx.role.defpos] is ObInfo oi
                && domain is UDType  && oi.names[s] is long p)
            {
    ...
    internal class SqlField : QlValue
    {
        ...
        internal override TypedValue _Eval(Context cx)
        {
            if ((cx.values[target]??cx.binding[target]) is TNode tt &&
                tt.dataType.infos[cx.role.defpos]?.names[name ?? ""] is long p
    ...
    QlValue? left, QlValue? right, BTree<long, object>? mm = null)
        ...
        {
            var ch = false;
            var nl = ((QlValue?)c.obs[left])?.Having(c, dm);
            ch = ch || (nl != null && nl.defpos != left);
            var nr = ((QlValue?)c.obs[right])?.Having(c, dm);
            ch = ch || (nr != null && nr.defpos != right);
            var nu = ((QlValue?)c.obs[sub])?.Having(c, dm);
    ...
    internal class SqlRowArray : QlValue
    {
        ...
        internal override bool isConstant(Context cx)
        {
            ...
            for (var b = rows.First(); b != null && r; b = b.Next())
                if (cx.obs[b.value() ?? -1L] is QlValue v)
                    r = v.isConstant(cx);
            return r;
        }
        internal override QlValue Having(Context c, Domain dm)
    ...
    {
        internal const long
            MultiSqlValues = -302; // BList<long?> QlValue
        /// <summary>
        /// the array
        /// </summary>
    ...
    {
        internal const long
            Elements = -261; // BList<long?> QlValue
        /// <summary>
        /// the array
        /// </summary>
...
: base(dp, BTree<long,object>.Empty + (Parms, acts) + (ObInfo.Name, pn) + (Var, tg))
    ...
    internal class SqlProcedureCall : SqlCall
    {
        ...
        public SqlProcedureCall(long dp, Context cx, string pn, BList<long?> acts, QlValue tg)
    ...
    internal class SqlMethodCall : SqlCall // instance methods
    {
        ...
        public SqlMethodCall(long dp, Context cx, Procedure pr,
            BList<long?> acts, QlValue? tg) : base(dp,cx, pr, acts, tg?.defpos??-1L)
        {
            cx.Add(this);
        }
        public SqlMethodCall(long dp, Context cx, string pn,BList<long?> acts, QlValue tg) 
            : base(dp, pn, acts, tg.defpos)
        {
            cx.Add(this);
        }
    ...
    internal class SqlConstructor : SqlCall
    {
        ...
        internal SqlConstructor(long dp, Context cx, Domain ut, BList<long?> args)
    ...
    internal class SqlFunction : QlValue
    {
        ...
        public SqlFunction(long dp, Context cx, Qlx f, QlValue? vl, QlValue? o1, QlValue? o2, Qlx m,
        ...
        internal override QlValue Having(Context c, Domain dm)
        {
            ...
            return ch ? (QlValue)c.Add(new SqlFunction(c.GetUid(), c, op, nv, n1, n2, mod)) : this;
        }
        internal override bool Match(Context c, QlValue v)
        {
        ...
        internal static Domain _Type(Context cx, Qlx op, QlValue? val, QlValue? op1)
        {
            switch (op)
            {
                ...
                case Qlx.FIRST: return Domain.Content;
                ...
                case Qlx.LAST: return Domain.Content;
                ...
                case Qlx.MAX: return val?.domain ?? Domain.Content;
                case Qlx.MIN: return val?.domain ?? Domain.Content;
                case Qlx.NEXT: return val?.domain ?? Domain.UnionDate;
        ...
        {   
            ...
            if (mod == Qlx.DISTINCT && vl is not null)
            {
                if (fc.mset == null)
                    fc.mset = new TMultiset(vl.domain);
            ...
            switch (op)
            {
                ...
                case Qlx.EVERY:
                    {
                        if (v is TBool vb)
                            fc.bval = ((fc.bval is bool xf) && (vb.value is bool xv))? (xf && xv):null;
                        break;
                    }
                case Qlx.FIRST:
                    {
    ...
    {
        internal SqlCoalesce(long dp, Context cx, QlValue op1, QlValue op2)
            : base(dp, cx, Qlx.COALESCE, null, op1, op2, Qlx.NO)
        {
            cx.Add(this);
        }
        ...
        internal override QlValue Having(Context c, Domain dm)
        {
            ...
            return (le == nl && rg == nr) ? this :
                (QlValue)c.Add(new SqlCoalesce(c.GetUid(), c, nl, nr));
        }
        internal override bool Match(Context c, QlValue v)
        {
    ...
    {
        internal SqlTypeUri(long dp, Context cx, QlValue op1)
            : base(dp, cx, Qlx.TYPE_URI, null, op1, null, Qlx.NO)
        {
            cx.Add(this);
        }
    ...
    internal class BetweenPredicate : QlValue
    {
        ...
        internal override QlValue Having(Context cx, Domain dm)
        {
...
cx.obs[high] is not QlValue hi)
    ...
    internal class GqlNode : QlValue
    {
        ...
        {
            if (dm is null && tgs[-(long)Qlx.TYPE] is TGParam tg && cx.names[tg.value] is long t)
                for (var b = cx.obs[t]?.infos[cx.role.defpos]?.names.First();b!=null;b=b.Next())
                    if (b.value() is long p && p<Transaction.Analysing && cx.bindings.Contains(p))
                            cx.Add(tg.value, this);
            cx.names += (nm.ident, defpos);
        }
        ...
        internal override (DBObject?, Ident?) _Lookup(long lp, Context cx, Ident ic, Ident? n, DBObject? r)
        {
            if (n is not null && domain.infos[cx.role.defpos] is ObInfo si && ic.sub is not null
                &&  cx.db.objects[si.names[n.ident]] is TableColumn tc1)
            {
        ...
        internal bool CheckProps(Context cx, MatchStatement ms, TNode n)
        {
            ...
            for (var b = docValue?.First(); b != null; b = b.Next())
                if (b.key() is string k)
                {
                    if (!ns.Contains(k))
                        return false;
                    if (ns[k] is long e && !n.tableRow.vals.Contains(e))
                        return false;
                    if (b.value().Eval(cx) is TypedValue xv && xv is not TArray)
                    {
                        ...
                        switch (k)
                        {
                            ...
                            case "SPECIFICTYPE":
                                ...
                                break;
                            default:
                                if (ns[k] is long d && xv.CompareTo(n.tableRow.vals[d]) != 0)
                                    return false;
                                break;
                        }
                    }
        ...
        internal CTree<long, TypedValue> EvalProps(Context cx, NodeType nt)
        {
            ...
            if (nt.infos[cx.role.defpos] is not ObInfo ni)
                return r;
            for (var b = docValue?.First(); b != null; b = b.Next())
                if (cx._Ob(ni.names[b.key()]) is DBObject ob && b.value().Eval(cx) is TypedValue v)
                    r += (ob.defpos, v);
            return r;
        }
    ...
    internal class GqlEdge : GqlNode
    {
            ...
            : base(nm, ch, cx, i, d, tgs, _Type(dm, cx, d, m, l, a), _Mem(cx, d, tgs, dm, m, nm, i, l, a, t))
        {
            if (dm is null && tgs[-(long)Qlx.TYPE] is TGParam tg
               && cx.names[tg.value] is long p && p < Transaction.Analysing && cx.bindings.Contains(p))
                cx.names += (tg.value, p);
            if (dm is EdgeType et)
            {
                    ...
                    && lt.EqualOrStrongSubtypeOf(ln.domain) == true)
                {
                    ...
                    cx.names += (ln.NameFor(cx), ln.defpos);
                    ...
                    && at.EqualOrStrongSubtypeOf(an.domain) == true)
                {
                    ...
                    cx.names += (an.NameFor(cx), an.defpos);
             ...
             CTree<long, TGParam> tgs, Domain? dm , BTree<long,object>?m, Ident nm, long i,long l,long a, Qlx t)
        {
            ...
            if (i > 0)
            {
                if ((cx.parse.HasFlag(ExecuteStatus.Compile)
                    ...
                    && cx.names[nm.ident] is long p && p>0)
                    i = p;
                m += (IdValue, i);
            }
            m += (DocValue, d);
        ...
        internal override Domain _NodeType(Context cx, NodeType dt, bool allowExtras = true)
        {
            ...
            { // unlabelled edges have types determined by their kind and property set
                ...
                else if (lT is not null && aT is not null)
                {
                    ...
                    {
                        nt = new EdgeType(cx.GetUid(), nd.label.name, new UDType(-1L, dn.mem),
            ...
            if (label is GqlLabel lb && lb.kind == Qlx.DOUBLEARROW && lb.AllowExtra(cx, mem) is not null)
            {
                ...
                if (rg is not null && rg.defpos > 0)
                {
                    if (lf is not null && lf.defpos > 0)
                    {
                        ...
                        {
                            var k = b.key();
                            var lc = cx.db.objects[ln[k]] as TableColumn;
                            var rc = cx.db.objects[rn[k]] as TableColumn;
                    ...
                    else // rg defined, create lf
                    {
                        ...
                        {
                            var k = b.key();
                            var rc = cx.db.objects[b.value()??-1L] as TableColumn ?? throw new DBException("42105");
                            lt += rc.defpos;
                            ls += (rc.defpos, rc.domain);
                            ln += (b.key(),rc.defpos);
                        }
                ...
                else
                {
                    ...
                    {
                        var k = b.key();
                        var lc = cx.db.objects[b.value()??-1L] as TableColumn ?? throw new DBException("42105");
    ...
    class GqlPath : GqlEdge
    {
        ...
        internal override int MinLength(Context cx)
        {
            ...
            for (var b = pattern.First(); b != null; b = b.Next())
                if (cx.obs[b.value()??-1L] is GqlNode p)
                    m += p.MinLength(cx);
        ...
        internal override GqlNode Add(Context cx, GqlNode? an, CTree<long, TGParam> tgs)
        {
            ...
            for (var b = pattern.Last(); b != null && last is null; b = b.Next())
                last = cx.obs[b.value() ?? -1L] as GqlEdge;
            if (an?.state[an.defpos] is TGParam lg)
                if (direction == Qlx.ARROWBASE)
                {
                    tgs += (-(long)Qlx.ARROW, lg);
    ...
    {
        internal const long
            MatchAlts = -486; // BList<long?> GqlMatchAlt
        internal BList<long?> matchAlts => (BList<long?>)(mem[MatchAlts] ?? BList<long?>.Empty);
        public GqlMatch(Context cx, BList<long?>ms) 
    ...
    {
        internal const long
            MatchExps = -487, // BList<long?> GqlNode
        ...
        public GqlMatchAlt(long dp,Context cx, Qlx m, Qlx sh, BList<long?> p, long pp)
                  ...
                  + (InclusionMode,sh))
        {
            ...
            {
                if (cx.obs[b.value() ?? -1L] is GqlNode sn)
                {
                    min += sn.MinLength(cx);
                    if (sn is GqlPath sp)
                    {
        ...
        internal override DBObject _Replace(Context cx, DBObject was, DBObject now)
        {
            ...
            for (var b = matchExps.First(); b != null; b = b.Next())
                if (cx.obs[b.value() ?? -1L] is GqlNode sa)
                {
