...
/*        internal virtual void AddNodeOrEdgeType(Context cx)
    ...
    Grant.Privilege pr = Grant.Privilege.Select, string? a = null,TableRowSet? ur = null)
            ...
            CTree<TypedValue,bool>? cs = null)
        {
            ...
            if (!cx.role.dbobjects.Contains(name))
            {
                ...
                for (var b = dc?.First(); b != null; b = b.Next())
                    ...
                    {
                        var pc = new PColumn3(nt, b.key(), -1, sv.domain, "", TMetadata.Empty, 
                            cx.db.nextPos, cx, true);
            ...
            Qlx q=Qlx.NO, NodeType? nt=null,CList<TypedValue>? md=null)
        {
            ...
            for (var b = ls.First(); b != null; b = b.Next())
            {
                ...
                var pc = new PColumn3(ut, n, -1, d, d.defaultString, d.metadata, cx.db.nextPos, cx);
    ...
    internal class GqlLabel : Domain
    {
            ...
            CTree<TypedValue,bool>? cs = null)
        {
            ...
            switch(k)
            {
                ...
                ?? EdgeTypeFor(new Ident(name??"",defpos), m, cx, cs), true);
                case Qlx.NO:
        ...
        static NodeType NodeTypeFor(string nm, BTree<long,object> m, Context cx)
        {
            ...
            if (nt is null || nt.defpos<0)
            {
                ...
                for (var b = dc.First(); b != null; b = b.Next())
                    if (!nt.HierarchyCols(cx).Contains(b.key()) && b.value() is QlValue q)
                    {
                        var pc = new PColumn3(nt, b.key(), -1, q.domain, 
                            q.domain.metastring, q.domain.metadata, cx.db.nextPos, cx, true);
                        nt = (NodeType)(cx.Add(pc)??throw new DBException("42105"));
                    }
        ...
        internal static EdgeType EdgeTypeFor(Ident nm, BTree<long, object> m, Context cx, CTree<TypedValue,bool>? cs=null)
        {
            ...
            for (var b = un.First(); b != null; b = b.Next())
                nu += ((b.key() is GqlLabel gl) ? (cx.db.objects[cx.role.dbobjects[gl.name ?? ""] ?? -1L]
                    as Domain) ?? throw new DBException("42107", gl.name ?? "??") : b.key(), true);
            var dc = (CTree<string, QlValue>)(m[GqlNode.DocValue] ?? CTree<string, QlValue>.Empty);
            if (cx.db.objects[cx.role.edgeTypes[nm.ident] ?? -1L] is not EdgeType et)
            {
            ...
            for (var b = dc.First(); b != null; b = b.Next())
                if (!e.HierarchyCols(cx).Contains(b.key()) && b.value() is QlValue q)
                {
    ...
    internal class EdgeType : NodeType
    {
        ...
        (EdgeType,TConnector) BuildNodeTypeConnector(Context cx,TConnector tc, NodeType nt)
        {
            ...
            var pc = new PColumn3(ut, cn, Length, Position,"", md, cx.db.nextPos, cx, false);
        ...
        internal override Table? Delete(Context cx, Delete del)
        {
            if (tableRows[del.delpos] is TableRow tr)
                for (var b = (metadata[Qlx.EDGETYPE] as TSet)?.First(); b != null; b = b.Next())
                    if (b.Value() is TConnector co && cx._Ob(co.ct) is NodeType nt
        ...
        {
            var cons = metadata[Qlx.EDGETYPE] as TSet;
            var dcons = d.metadata[Qlx.EDGETYPE] as TSet;
            ...
            for (var b = cons.First(); b != null; b = b.Next())
            {
                if (b.Value() is TConnector c)
                {
                    for (var db = dcons.First(); db != null; db = db.Next())
                        if (db.Value() is TConnector dc && cx._Ob(c.ct) is Domain td
        ...
        /// <summary>
        /// Two cases: adding new Table metadata (EDGETYPE) define a set of connector columns 
        ...
        /// This means we should check & modify the EDGETYPE metadata if needed (but avoid further recursion)
        /// </summary>
        /// <param name="cx"></param>
        /// <param name="s"></param>
        /// <param name="md"></param>
        ...
        internal override DBObject Add(Context cx, string s, TMetadata md)
        {
            ...
            if (md[Qlx.CONNECTING] is TConnector tc)
            {
                ...
                if (nc ==null) // this is a surprise but likely allowed
                {
                    var cm = tc.cm??TMetadata.Empty + (Qlx.OPTIONAL, TBool.False);
                    var pc = new PColumn3(r, tc.cn, r.Length, tc.cd, "", cm, tc.cp, cx, true);
                    ...
                    var om = r.metadata[Qlx.EDGETYPE] as TSet??new TSet(Connector);
                    r += (ObInfo._Metadata, r.metadata + (Qlx.EDGETYPE, om + tc));
                }
                var di = new Domain(-1L, cx, Qlx.ROW, new BList<DBObject>(nc), 1);
      ...
      //      cx.db += ro + (Role.EdgeTypes, ro.edgeTypes + (name, cx.Fix(defpos)));
            ...
            CTree<TypedValue,bool>? cs = null)
        {
            ...
            for (var b = dc?.First(); b != null; b = b.Next())
                ...
                {
                    var pc = new PColumn3(et, b.key(), -1, sv.domain, "",TMetadata.Empty, 
                        cx.db.nextPos, cx, true);
    ...
    {
        public readonly Qlx q; // FROM and TO are used in EdgeType, left as ARROWBASE etc in expressions
