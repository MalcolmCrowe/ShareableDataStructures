...
namespace Tpcc
{
	...
	public class OrderStatus : VTerm
	{
        ...
        bool DoDisplay(ref string mess)
        {
            ...
            while(s.Read())
            {
                Set(k++, (int)(decimal)s[1]);
                Set(k++, (int)(decimal)s[0]);
		...
		public OrderStatus(OracleConnection c, int w)
        {
            db = c;
            wid = w;
            //
