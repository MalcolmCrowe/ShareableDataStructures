...
namespace Tpcc
{
    ...
    public class NewOrder : VTerm
    {
        ...
        private OracleTransaction tr = null;
        public Button btn;
        ...
        bool FetchDistrict(ref string mess)
        {
            ...
            d_tax = (decimal)(float)s[0];
            o_id = (int)(decimal)s[1];
            Set(6, o_id);
		...
		bool DoOLCount(ref string mess)
		{
            ...
            cmd.CommandText = "insert into \"ORDER\"(O_ID,O_D_ID,O_W_ID,O_C_ID,O_ENTRY_D,O_OL_CNT,O_ALL_LOCAL)"+
                    ...
                    DateTime.Now.ToString("dd-MMM-yyyy") + "'," + ol_cnt + "," + (allhome ? "1" : "0") + ")";
            Form1.RecordRequest(cmd, fid, tid);
            cmd.ExecuteNonQuery();
		...
		public NewOrder(OracleConnection c,int w)
		{
            db = c;
            wid = w;
			//
