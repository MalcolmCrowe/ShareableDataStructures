...
namespace Tpcc
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
        ...
        public Form1()
		{
			conn = new MySqlConnection("Server=localhost;Database=Tpcc;Uid=root;Password=admin");
            conn.Open();
            //
        ...
        public static void RecordRequest(MySqlCommand cmd,int cid,int tid)
        {
            if (reqs == null)
                return;
            lock (reqs)
        ...
        public static void CloseRequests()
        {
            if (reqs != null)
            {
                var conn = new MySqlConnection("Server=localhost;Database=Tpcc;Uid=root;Password=admin");
                conn.Open();
	...
	//			wid = int.Parse(textBox1.Text);
		...
		private void button2_Click(object sender, System.EventArgs e) // Run button for New Order
		{
            ...
            if (checkBox1.Checked)
            {
                ...
                n.db = new MySqlConnection("Server=localhost;Database=Tpcc;Uid=root;Password=admin");
                n.db.Open();
            }
			n.btn = button2;
			n.txtBox = textBox3;
