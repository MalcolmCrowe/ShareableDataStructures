...
namespace Tpcc
{
    ...
    public class NewOrder : VTerm
    {
        ...
        bool FetchItemData1(int j, ref string mess)
        {
            OrderLine a = ols[j];
            int k = 10 + j * 8;
            var s = db.ExecuteQuery("select I_PRICE,I_NAME,I_DATA from ITEM where I_ID=" + a.oliid);
            if (s.IsEmpty)
            {
                db.Rollback();
        ...
        bool FetchItemData2(int j, ref string mess)
        {
            ...
            if (ds.Length > 2)
                ds = ds.Substring(1);
            var s = db.ExecuteQuery("select S_QUANTITY,S_DIST_" + ds + ",S_DATA from STOCK where S_I_ID=" + a.oliid + " and S_W_ID=" + a.ol_supply_w_id);
            if (s.IsEmpty)
            {
                db.Rollback();
