...
namespace Pyrrho.Level2
{
    ...
    internal class PView : Compiled
    {
        ...
        internal override DBObject? Install(Context cx)
        {
            ...
            if (vw.infos[cx.role.defpos] is ObInfo oi)
            {
    ...
    internal class PRestView : PView
    {
        ...
        internal override void OnLoad(Reader rdr)
        {
            ...
            var m = psr.ParseRowTypeSpec(new BTree<long,object>(Domain.Kind,Qlx.VIEW)).mem - Domain.Under;
    ...
    internal class PRestView2 : PRestView
    {
        ...
        void FixCols(Context cx)
        {
            ...
            for (var b = ts.rowType.First(); b != null; b = b.Next())
                if (b.value() is long p && ts.representation[p] is Domain c 
                    && c is not StandardDataType && vs[c.NameFor(cx)] is Domain oc)
                    cx.Replace(c, oc);
        }
        internal override void OnLoad(Reader rdr)
        {
