...
namespace Pyrrho.Level2
{
    ...
    internal abstract class Physical
    {
        ...
        public enum Type
        {
            ...
            Alter3, View1, Metadata, PeriodDef, Versioning, PCheck2, NotUsed7, //48-54 
        ...
        public virtual (Transaction?, Physical) Commit(Writer wr, Transaction? tr)
        {
            ...
            var ph = Relocate(wr);
            if (ph is PIndex pi && this is PIndex oi) // yuk
                pi.metadata = (TMetadata)oi.metadata.Fix(wr.cx);
        ...
        internal virtual long TableDefPos => 0L;
        // Used in Record and Delete during Transaction.Commit
        public CTree<long, bool> RelTables(Database db)
        {
            var tp = TableDefPos;
            if (tp == 0L) return CTree<long, bool>.Empty;
            var r = new CTree<long, bool>(tp, true);
            if (db.objects[tp] is Table tb)
            {
                for (var b = tb.super.First(); b != null; b = b.Next())
                {
                    var k = b.key().defpos;
                    if ((!r.Contains(k)) && db.objects[k] is Domain d)
                    {
                        for (var c = d.subtypes.First(); c != null; c = c.Next())
                            r += (c.key(), true);
                        r += (k, true);
                    }
                }
    ...
    internal class AlterEdgeType : Physical
    {
        ...
        internal override DBObject? Install(Context cx)
        {
            if (cx.db.objects[edgetype] is not Table et
                || cx.db.objects[reftype] is not Table nt)
                return null;
 ...
 /*           for (var b = nt.rindexes[et.defpos]?.First(); b != null && ip<0; b = b.Next())
    ...
    internal abstract class Defined : Physical
    {
        ...
        {
            return (p == pp) ? ppos : (wr.cx.uids[p] is long np && np!=0L) ? np : p;
        }
        public override long Dependent(Writer wr, Transaction tr)
        {
        ...
        public override string ToString()
        {
            ...
            if (dataType.Length == 0 && dataType.kind!=Qlx.TYPE)
    ...
    internal class PUnion : Physical
    {
        internal CTree<Domain,bool> alts = CTree<Domain,bool>.Empty;
        internal Table? domain = null;
        public PUnion(CTree<Domain,bool> a, long pp, Context cx)
            : base(Type.PUnion, pp, cx.db) 
        {
            alts = a;
        }
        /// <summary>
        /// Constructor: a object type from the file buffer
        /// </summary>
        /// <param name="bp">the buffer</param>
        /// <param name="pos">the buffer position</param>
        public PUnion(Reader rdr) : this(Type.PUnion, rdr) { }
        protected PUnion(Type t, Reader rdr) : base(t, rdr) { }
        protected PUnion(PUnion x, Writer wr) : base(x, wr) 
        {
            alts = wr.cx.FixTDb(x.alts);
        }
        public override long Dependent(Writer wr, Transaction tr)
        {
            for (var b = alts.First(); b != null; b = b.Next())
            {
                var p = (b.key().Fix(wr.cx) as Domain)?.Create(wr, tr)??throw new PEException("PE60621");
                if (p != b.key().defpos)
                { 
                    var d = (wr.cx.db.objects[p] as Domain) ?? throw new PEException("PE60622");
                    alts -= b.key();
                    alts += (d, true);
                }
                if (!Committed(wr, p)) return p;
            }
            return -1L;
        }
        protected override Physical Relocate(Writer wr)
        {
            return new PUnion(this, wr);
        }
        public override void Serialise(Writer wr)
        {
            wr.PutInt((int)alts.Count);
            for (var b = alts.First(); b != null; b = b.Next())
        ...
        public override void Deserialise(Reader rdr)
        {
            ...
            for (var i = 0; i < n; i++)
                a += (rdr.context.db.objects[rdr.GetLong()] as Domain??throw new PEException("PE60661"),true);
            alts = a;
            base.Deserialise(rdr);
        }
        internal override DBObject? Install(Context cx)
        {
            domain = new Table(ppos, alts);
            cx.db += domain;
            return (Domain)cx.Add(domain??throw new PEException("PE10261"));
        }
        public override string ToString()
        {
            var sb = new StringBuilder("PUnion");
            var cm = '[';
            for (var b=alts.First();b!=null;b=b.Next())
            {
                sb.Append(cm); cm = ',';
