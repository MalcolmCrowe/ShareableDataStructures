	...
	internal class Update : Record
	{
       ...
       //         sbT = ta.subtypes;
        ...
        public override void Deserialise(Reader rdr)
        {
            prev = rdr.GetLong();
            _defpos = rdr.GetLong();
            prevrec = (rdr.context.db.objects[tabledefpos] as Table)?.tableRows[_defpos];
                ...
                case Type.Delete2:
                    {
                        ...
                        if (cx.db.objects[de.tabledefpos] is Table tb)
                        {
                            if (db.objects[tb.defpos] is Table nt // possibly conflicting table version in db
                            && nt.sindexes.Contains(de.delpos))
                                throw new DBException("40029", de.delpos, that, ct);
                            for (var b = tb.refCols.First(); b != null; b = b.Next())
                                if (fields[b.key()]?.ToLong() == de.delpos)
                                    throw new DBException("40027", de.delpos, that, ct);
                        }
                        break;
                    }
                case Type.Update2:
        ...
        internal override Context Add(Context cx, Table tt, TableRow now)
        {
            if (tt.defpos < 0)
                return cx;
            base.Add(cx, tt, now);
            for (var b = RelTables(cx.db).First(); b != null; b = b.Next())   // update subtypes
        ...
        internal override Table AddRow(Table tt, TableRow now, Context cx)
        {
            ...
            for (var b = tt.rowType.First(); b != null; b = b.Next())
                if (cx._Ob(b.value()) is TableColumn co)
                    if (cx.db.objects[co.toType] is Table rt)
                    {
                        ...
                        // perform surgery on sindexes
                        var os = rt.sindexes;
                        var ol = pr[co.defpos]?.ToLong() ?? -1L;
                        var om = os[ol] ?? CTree<long, CTree<long, bool>>.Empty;
                        var on = om[co.defpos] ?? CTree<long, bool>.Empty;
                        if (prevrec != null)
                            on -= prevrec.defpos;
                        ...
                        if (nl > 0 && !sz)
                        {
                            var cm = os[nl] ?? CTree<long, CTree<long, bool>>.Empty;
                            var cn = cm[co.defpos] ?? CTree<long, bool>.Empty;
                            cn += (now.defpos, true);
                            cm += (co.defpos, cn);
                            os += (nl, cm);
                        }
            ...
            if (re != CTree<long, TypedValue>.Empty)
            {
                fields += re;
                if (cx.db is Transaction tr)
                {
                    tr += (Transaction.Physicals,tr.physicals+(ppos,this));
                    cx.db = tr;
                }
            }
            tt += now;
