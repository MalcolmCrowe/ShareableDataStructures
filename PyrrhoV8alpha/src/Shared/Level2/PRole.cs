...
namespace Pyrrho.Level2
{
	...
	internal class PRole: Defined
	{
        ...
        internal override DBObject? Install(Context cx)
        {
            ...
            if (first)
            {
                ...
                nr += (ObInfo.Privilege, Grant.AllPrivileges);
                cx.db = cx.db + (DBObject.Definer, nr.defpos) + nr;
                cx.db += (Database.Role, nr);
            }
            return nr;
        }
    }
    ...
    /// </summary>
     internal class PMetadata : Compiled
     {
         /// <summary>
        ...
        public PMetadata(string nm, long sq, DBObject ob, string s,TMetadata md, long nst, long pp, Context cx)
            : this(Type.Metadata, nm, sq, ob, s, md, nst, pp, cx) { }
        public PMetadata(Type t,string nm,long sq,DBObject ob, string s, TMetadata md,long nst, long pp, Context cx)
            :base(t,pp, cx, nm, ob.domain,nst)
        { 
            name = nm;
            seq = sq;
            defpos = ob.defpos;
        ...
        internal override DBObject? Install(Context cx)
        {
            ...
            if (ob.infos[cx.role.defpos]?.metadata.CompareTo(metadata)!=0)
                (cx,ob) = ob.Add(cx, metadata);
            ob += (DBObject._Framing, ob.framing + framing);
        ...
        {
            var ob = ((DBObject?)wr.cx.db.objects[wr.cx.Fix(defpos)]) 
                ?? throw new DBException("42000", "PMetadata"); 
            wr.cx.db += (defpos, ob + (DBObject._Framing, ob.framing+framing));
     ...
     internal class PMetadata2 : PMetadata
     {
       /// <summary>
        ...
        protected PMetadata2(Type tp,string nm, long sq, DBObject ob, string s, 
            TMetadata md, long nst, long pp, Context cx)
         : base(tp, nm, sq, ob, s, md, nst, pp, cx)
        {
        }
    ...
    internal class PMetadata3 : PMetadata2
    {
        ...
        public PMetadata3(string nm, long sq, DBObject ob, string s, 
            TMetadata md, long nst, long pp, Context cx)
            : base(Type.Metadata3, nm, sq, ob, s, md, nst, pp, cx)
        {  }
