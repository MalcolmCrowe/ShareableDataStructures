...
namespace Pyrrho.Level2
{
    ...
    internal class Record : Physical
    {
            ...
            Context cx) : base(t, pp, cx.db)
        {
            ...
            if (cx._Ob(tp) is Table tb)
            {
                ...
                siC += tb.sindexes; // snapshot is okay : don't even think about Fix!
        ...
        public override DBException? Conflicts(Database db, Context cx, Physical that, PTransaction ct)
        {
            switch (that.type)
            {
                case Type.Drop:
                    {
                        var d = (Drop)that;
                        for (var b = RelTables(cx.db).First(); b != null; b = b.Next())
                ...
                case Type.Delete:
                    {
                        ...
                        {
                            if (tb.sindexes.Contains(del.delpos))
                                return new DBException("40075", that);
                            for (var b = tb.colRefs.First(); b != null; b = b.Next())
                                for (var c=b.value().First();c!=null;c=c.Next())
                                if (fields[c.key()]?.ToLong() == del.delpos)
                                    throw new DBException("40027", del.delpos, that, ct);
                        }
                        for (var b=extraTables.First(); b != null; b=b.Next())
                            if (cx.db.objects[tabledefpos] is Table tc && tc.sindexes.Contains(del.delpos))
                                return new DBException("40075", that);
                        break;
                    }
            }
  ...
  /*      public static bool DifferentTables(Record a, Record b)
        ...
        internal virtual Context Add(Context cx, Table tt, TableRow now)
        {
            if (tt.defpos < 0)
                return cx;
            for (var b = suT.First(); b != null; b = b.Next())   // update supertypes: extra values are harmless
        ...
        internal virtual Table AddRow(Table tt, TableRow now, Context cx)
        {
            ...
            if (cx.db is Transaction && tt.colRefs.Count>0L)
            ...
            if (cx.db.objects[tt.defpos] is Table nt && nt.NameFor(cx)==""
                && nt.infos[cx.role.defpos] is ObInfo ni)
            {
                var md = ni.metadata ?? TMetadata.Empty;
                if (md != TMetadata.Empty && nt.NameFor(cx)==""
                    && !cx.db.unlabelledNodeTypes.Contains(md))
                    cx.db += (Database.UnlabelledNodeTypes, cx.db.unlabelledNodeTypes + (md, nt.defpos));
                if (nt!=tt)
                    cx.db += tt;
            }
            tt += now;
        ...
        /// <summary>
        /// When we get to here, all of the record values have been filled in and do not require changing.
        /// </summary>
        /// <param name="tt"></param>
        /// <param name="vs"></param>
        /// <param name="tc"></param>
        /// <param name="rt">The toType of the connector</param>
        /// <param name="rr">The referenced row</param>
        /// <param name="cx"></param>
        /// <returns></returns>
        /// <exception cref="DBException"></exception>
        CTree<long,TypedValue> Add(Table tt, CTree<long, TypedValue> vs, TableColumn tc, Table rt, TableRow rr, Context cx)
        {
            if (rr.tabledefpos != rt.defpos) // rr belongs to a subtype of rt
                for (var b=rt.subtypes.First();b!=null;b=b.Next())
                    if (cx._Ob(b.key()) is Table si && si.tableRows.Contains(rr.defpos))
                        Add(tt, vs, tc, si, rr, cx);
            var cn = rt.sindexes[rr.defpos] ?? CTree<long, CTree<long, bool>>.Empty;
            var cc = cn[tc.defpos] ?? CTree<long, bool>.Empty;
            cc += (defpos, true);
            cn += (tc.defpos, cc);
            rt += (Table.SysRefIndexes, rt.sindexes + (rr.defpos, cn));
            cx.Add(rt);
            cx.db += rt;
            return vs;
        }
        ...
        internal virtual void CheckFields(Context cx)
        {
            ...
            for (var c = tb.First(); c != null; c = c.Next())
            {
                ...
                if (fields[p] is TypedValue v && v != TNull.Value)
                    ...
                    {
                        for (var b = dv.alts.First(); b != null; b = b.Next())
                            if (cx.db.objects[b.key().defpos] is NodeType at 
                                && at.tableRows.Contains(v.ToLong() ?? -1L))
                                goto next;
                        throw new PEException("PE10706");
                    }
 ...
 /*   /// <summary>
    ...
    internal class Record4 : Record3
    {
        ...
        internal override DBObject? Install(Context cx)
        {
            ...
            Table? nj;
            if (cx.db.objects[cx.db.joinedTypes[jt]] is Table jd)
                nj = jd;
            else
            {
                nj= (Table)(cx.Add(new PType2("", Domain.TypeSpec, jt, Transaction.Executables,cx.db.nextPos, cx))
                    ??throw new DBException("42105"));
                cx.db += (Database.JoinedTypes, cx.db.joinedTypes + (jt,nj.defpos));
            }
            cx.db += nj + (Table.TableRows, nj.tableRows + (defpos, now));
            if (cx.db.mem.Contains(Database.Log))
                cx.db += (Database.Log, cx.db.log + (ppos, type));
