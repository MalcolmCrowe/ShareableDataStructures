...
namespace Pyrrho.Level1
{
    ...
    internal class TCPStream : Stream
    {
        ...
        internal class BBuf(BList<byte> b)
        {
            ...
            public static BBuf operator+(BBuf bb,(Context,TNode)x)
            {
                var (cx, tn) = x;
                var n = tn.dataType.Length;
                bb += n;
                var j = 0;
            ...
            public static BBuf operator+(BBuf b,(Context,Domain,TypedValue)x)
            {
                ...
                switch (tv.dataType.kind)
                {
                    ...
                    case Qlx.DESCRIBE:
                        ...
                        else if (tv is TNode xx)
                            return b+xx.Cast(cx, dt).ToString(cx);
                        break;
                    case Qlx.EDGETYPE:
                    case Qlx.NODETYPE:
                    case Qlx.TYPE:
        ...
        internal void PutColumns(Context cx,Domain dt)
        {
            ...
            for (var j = 0; j < m; j++)
                    ...
                    && cx.db.objects[sc.sPos] is TableColumn dn)
                {
                    ...
                    ((dn.generation != Generation.No) ? 0x200 : 0);
                    PutInt(flags);
                }
            Flush();
        }
            ...
            if (rec != null && rec != BList<TableRow>.Empty)
            {
                ...
                for (var b = rec.First(); b != null; b = b.Next())
                    if (cx.db.objects[b.value().tabledefpos] is Domain rd) {
                        if (rd.alts.Count==0 && rd.infos[ro.defpos] is ObInfo od &&
