...
namespace Pyrrho
{
    ...
    internal class PyrrhoServer
    {
            ...
            try
            {
                ...
                if (db == null)
                {
                    ...
                    if (!File.Exists(fp))
                    {
                        ...
                        wr.PutInt(777);
                        wr.PutInt(53); // v8 and greater: format is 53
                        wr.PutBuf();
                        ...
                        case Protocol.Authority:
                            {
                                ...
                                conn.props += ("Role", rn);
                                var rp = db.roles[rn];
                                if (db.objects[rp] is not Role ro || cx==null)
                                    throw new DBException("42105").Add(Qlx.ROLE);
                                db += ro;
                                db += (Database.Role, ro);
                                cx.db = db;
        ...
        internal void ReaderData()
        {
            ...
            var i = 0;
            if (rb.columns is CTree<int,long> co)
            {
    ...
    /*        var (rv, rc) = TCPStream.Check(cx, rb); */
        ...
        int DataLength(Context cx, TypedValue tv, string? rv = null, string? rc = null)
        {
            ...
            switch (tv.dataType.kind)
            {
                ...
                case Qlx.NODETYPE:
                    if (cx._Ob(tv.dataType.defpos) is Table nt && tv is not TNode)
                        return lc + 1 + StringLength(nt.Describe(cx));
                    return lc+1 + StringLength(((TNode)tv).ToString(cx));
                case Qlx.EDGETYPE:
                    if (cx._Ob(tv.dataType.defpos) is Table et && et.colRefs.Count>1L && tv is not TNode)
	...
	class PyrrhoStart
	{
        ...
        [
            ...
            "8.0alpha","(23 April 2026)", "https://pyrrhodb.com"
        ];
	}
}