...
namespace Pyrrho.Common
{
    ...
    public class BList<V> : BTree<int, V>
    {
        ...
        protected BList() : base() { dense = true; }
        BList(bool d,BTree<int, V> t) : base(t.root) { dense = d; }
        public BList(V v) : base(0, v) { dense = true; }
        /// <summary>
        ...
        protected BList(bool d,Bucket<int, V>? r) : base(r) { dense = d; }
        /// <summary>
        ...
        protected override ATree<int, V> Add(int k, V v)
        {
            ...
            return new BList<V>(false,r);
        }
        public static BList<V> operator-(BList<V> t, int k)
        {
            if (t.dense && k == t.Length - 1)
                return new BList<V>(true, ((BTree<int,V>)t) - k);
            var r = BTree<int,V>.Empty;
            var n = 0;
            for (var b = t.First(); b != null; b = b.Next())
                if (b.key() != k)
                    r += (n++,b.value());
            return new BList<V>(t.dense && k==0, r);
        }
