...
namespace Pyrrho.Common
{
    ...
    public abstract class TypedValue : IComparable
    {
        ...
        public static TypedValue operator+ (TypedValue left,TypedValue right)
        {
            ...
            if (right is TNull)
                return left;
            if (left is TRef pl && right is TRef pr)
                return new TSet(Domain.Ref) + pl + pr;
    ...
    internal class TInt : TypedValue
    {
        ...
        internal TInt(Domain dt, long v) : base(dt.Best(Domain.Int).Item2) { value = v; }
    ...
    internal class TInteger : TInt
    {
        ...
        internal TInteger(Domain dt, Integer i) : base(dt.Best(Domain.Int).Item2,0L) { ivalue = i; }
    ...
    //shareable
    internal class TIdent : TypedValue
    {
        internal readonly Ident val;
        internal static TIdent Empty = new(new Ident("",0L));
        ...
        public override string ToString()
        {
            ...
            if (!char.IsLetterOrDigit(val.ident[0]))
                special = true;
            for (var i = 1; i < val.ident.Length; i++)
                if (!char.IsLetterOrDigit(val.ident[i]) && val.ident[i] != '_')
                    special = true;
            return special? '"'+val.ident +'"': val.ident;
        }
    }
    // shareable
    ...
    internal class TNumeric : TypedValue
    {
        ...
        internal TNumeric(Domain dt, Numeric n) : base(dt.Best(Domain._Numeric).Item2) { value = n; }
    ...
    internal class TUnion : TypedValue 
    {
        ...
        internal TypedValue LimitToValue(Context cx,long lp)
        {
            ...
            for (var b = dataType.alts.First(); b != null; b = b.Next())
            {
                var dt = b.key();
        ...
        public static TArray operator +(TArray ar, (Context,long,TypedValue) x)
        {
            ...
            if (dt.kind != v.dataType.kind)
            {
                if (dt.kind == Qlx.CHAR && v.dataType.kind != Qlx.CHAR)
                    ...
                    && (cx._Ob(cx.role.dbobjects[tc.value]) as Domain)?.EqualOrStrongSubtypeOf(dt) != true)
                    goto bad;
            }
    ...
    internal class TTypeSpec : TypedValue
    {
        ...
        internal override TypedValue Fix(Context cx)
        {
            return new TTypeSpec(name,(Domain)_dataType.Fix(cx));
        }
 /*       internal UDType OnInsert(Context cx,QlValue lb)
        {
            var nn = ((name == "") ? (lb.Eval(cx) as TTypeSpec)?.name:name)??"";
            if (cx._Ob(cx.role.edgeTypes[nn]) is UDType t1)
                return t1;
            if (cx._Ob(cx.role.nodeTypes[nn]) is UDType t2)
                return t2; 
            if (cx._Ob(cx.role.dbobjects[nn]) is UDType t3)
                return t3;
            if (lb is SqlTypeExpr se
                && se.kind == Qlx.DOUBLEARROW && cx.obs[se.left] is QlValue lf
                && cx.obs[se.right] is QlValue rg)
            {
                cx.Add(OnInsert(cx, lf + (Domain.Under, new CTree<Domain, bool>(OnInsert(cx, rg), true))));
                return (UDType)(cx._Ob(cx.role.dbobjects[nn]) ?? throw new PEException("PE70752"));
            }
            if (lb is not SqlLiteral sl || sl.val is not TTypeSpec tt) throw new PEException("PE66033");
            if (tt.name is not string n || n == "") throw new PEException("PE66034");
            if (cx._Ob(cx.role.dbobjects[tt.name]) is UDType ta)
                return ta;
            var un = (CTree<Domain, bool>)(lb.mem[Domain.Under] ?? CTree<Domain, bool>.Empty);
            var ud = Domain.NodeType+(ObInfo.Name,n);
            ud = (NodeType)ud.New(cx.db.nextPos, ud.mem);
            Physical pt = (un==CTree<Domain,bool>.Empty)?
                new PType(tt.name,ud,cx.db.nextPos,cx):
                new PUnion(un,cx.db.nextPos, cx);
            cx.Add(pt);
            var ut = (cx._Ob(pt.ppos) as UDType) ?? throw new DBException("42105");
            return (UDType)(cx.Add(pt)??throw new DBException("42105"));
        } */
        public override string ToString()
        {
            return DBObject.Uid(_dataType.defpos);
        }
    }
    ...
    internal class TRow : TypedValue
    {
        ...
        internal CTree<int, long> columns => dataType.rowType;
        internal static TRow Empty = new (Domain.Row, CTree<long, TypedValue>.Empty);
        public TRow(Domain dt,CTree<long,TypedValue> vs,Context? cx=null)
            :base(dt)
        {
            values = vs;
        }
        public TRow(Domain dt, CTree<long, long> map, BTree<long, TypedValue> vs)
            : base(dt)
        {
        ...
        // This one modifies an ad-hoc domain as a path is constructed during Match
        public static TRow operator +(TRow p, (Context, long, TypedValue) x)
        {
            ...
            return new TRow((Domain)cx.Add(new Domain(dt.defpos, cx, Qlx.ROW, dt.representation + (k, v.dataType), dt.rowType + (dt.Length,k))),
                p.values + (k, v));
        }
        public static TRow operator +(TRow p, (Context, TNode) x)
        {
            var (cx, n) = x;
            var a = (p.Length > 0) ? (TArray)p[0L] : new TArray(Domain.TypeSpec);
    ...
    internal class TPath : TRow
    {
        ...
        internal TPath(long dp,Context cx) : base(new Domain(-1L,cx,Qlx.ROW,BList<DBObject>.Empty,0), 
            new CTree<long, TypedValue>(0, 
                new TList(new Domain(-1L,Qlx.LIST,Domain.TypeSpec),CList<TypedValue>.Empty))) 
        {
            matchAlt = dp;
        }
        ...
        public static TPath operator+(TPath p,(Context, long,TypedValue)x)
        {
            ...
            return new TPath(p.matchAlt,
                (Domain)cx.Add(new Domain(dt.defpos,cx,Qlx.ROW,dt.representation+(k,v.dataType),dt.rowType+(dt.Length, k))),
                p.values+(k,v));
        }
        public static TPath operator +(TPath p, (Context, TypedValue, long) x)
        {
            ...
            return new TPath(p.matchAlt,
                ...
                dt.representation + (k, new Domain(-1L,Qlx.ARRAY, v.dataType)), dt.rowType + (dt.Length, k))),
                p.values + (k, v));
        }
    ...
    internal class TSet : TypedValue
    {
        ...
        {
            var a = CTree<TypedValue, bool>.Empty;
        ...
        {
            if (a.tree.Contains(v))
                return a;
            return a.Add(v);
        }
        public static TSet operator +(TSet a, TSet b)
        {
            for (var c = b.tree.First(); c != null; c = c.Next())
                if (!a.tree.Contains(c.key()))
                    a += c.key();
            return a;
        }
        public static TSet operator -(TSet a, TypedValue v)
    ...
    internal class TMultiset : TypedValue
    {
        ...
        /// </summary>
        internal readonly CTree<TypedValue,long> tree; 
        /// <summary>
        /// Accessor
        /// </summary>
        ...
        internal TMultiset(Domain dt,CTree<TypedValue,long>t,long ct) 
            :base(_Type(dt,t))
        {
            tree = t; count = ct;
        }
        static Domain _Type(Domain dt, CTree<TypedValue,long> t)
        {
        ...
        internal class MultisetBookmark : IBookmark<TypedValue>
        {
            ...
            {
                _set = set; _pos = pos; _bmk = bmk; _rep = rep??bmk.value();
            }
    ...
    {
        readonly CTree<long, CTree<long, long>> list; // if long? >0 it is defpos of invertible adapter function
        internal Adapters() : this(CTree<long,CTree<long,long>>.Empty) { }
        Adapters(CTree<long,CTree<long,long>> e)
        {
            list = e;
        }
        Adapters _Add(long a, long b, long atob)
        {
            ...
            var ab = list[a];
            if (ab == null)
                r+=(a, new CTree<long, long>(b, atob));
