    ...
    + " \"style=\"border:1px solid #c3c3c3;\">\r\n");
            ...
            else
            {
                ...
                for (var b = rs.rowType.First(); b != null; b = b.Next())
                    if (fm is not null && b.value() is long p && cx._Ob(fm.sIMap[p]) is DBObject c &&
        ...
        public override void PutRow(Context cx, Cursor e)
        {
            ...
            if (chartType.Contains(Qlx.NODE) 
                ...
                && cx.db.objects[tc._table.defpos] is NodeType nt)
            {
                var (li,ts) = Table.NodeTable(cx, nt.Node(cx, tr));
                var bl = CTree<int,Table>.Empty;
                ...
                for (var b=li.First();b!=null;b=b.Next())
                    if (b.value() is Table.NodeInfo ni)
                    {
                        sbuild.Append(comma + "[");
