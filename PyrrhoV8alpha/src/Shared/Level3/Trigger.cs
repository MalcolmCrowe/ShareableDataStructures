...
namespace Pyrrho.Level3
{
    ...
    internal class Trigger : DBObject
	{
        ...
        public long action => (long)(mem[HandlerStatement.Action]??-1L);
		...
		public Trigger(PTrigger p,Role ro)
            : base(p.ppos, 
                  _Mem(p) + (HandlerStatement.Action,p.def) + (ObInfo.Name,p.name) +(Owner,p.owner)
        ...
        internal override DBObject _Replace(Context cx, DBObject so, DBObject sv)
        {
            ...
            if (a != action)
                r +=(cx, HandlerStatement.Action, a);
            var ch = false;
