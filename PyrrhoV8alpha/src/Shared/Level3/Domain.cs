...
namespace Pyrrho.Level3
{
    ...
    internal class Domain : DBObject, IComparable
    {
        internal const long
            ...
            Alts = -91, // CTree<Domain,bool> Domain
            Charset = -71, // Pyrrho.CharSet
            ColRefs = -145, // CTree<long,CTree<long,bool>> Referenced domain REFerencing column in this Domain
            Constraints = -72,  // CTree<long,bool> DBObjects 
            Culture = -73, // System.Globalization.CultureInfo
            Default = -74, // TypedValue
            ...
            Suffix = -400, // string (metadata)
            TrueRowSet = -437,// RowSet
            Under = -90; // CTree<Domain,bool> direct supertypes (and GQL joined types)
    ...
    Array, SetType, Multiset, List, Collection, Cursor, UnionNumeric, UnionDate,
    ...
    Connector, Vector, Identity; // Rvv is V7 validator type, Identity is identifier syntax
        internal static Table TableType;
        ...
        static Domain()
        {
            ...
            Ref = new StandardDataType(Qlx.REF);
        ...
        public virtual CTree<int,long> rowType => (CTree<int,long>)(mem[RowType] ?? CTree<int, long>.Empty);
        public int Length => (int)rowType.Count;
    ...
    : this(dp, BTree<long, object>.Empty + (Kind, t) + (Under, u))
        {
            ...
            Database._system += this; // only do this for system types
        }
        ...
        {  }
       /// <summary>
        /// Allow construction of ad-hoc derived types such as ARRAY, MULTISET, SET, LIST, COLLECT, REF
        /// </summary>
        /// <param name="t"></param>
        /// <param name="d"></param>
        ...
        static BTree<long, object> _Mem(Context cx, Qlx t, BList<DBObject> vs, int ds)
        {
            ...
            {
                var v = b.value();
                rs += (v.defpos, (v is Domain vd)?vd:v.domain);
                cs += ((int)cs.Count, v.defpos);
        ...
        static BTree<long, object> _Mem(Context cx, Qlx t, CTree<long, Domain> rs, CTree<int,long> cs, int ds)
        {
        ...
        internal override DBObject Instance(long lp, Context cx, RowSet? ur = null)
        {
            ...
            for (var b = constraints.First(); b != null; b = b.Next())
                if (cx._Ob(b.key()) is DBObject ob)
 ...
 Names ns, BTree<long, Names> ds, long ap)
        {
            ...
            for (var b = rowType.First(); b != null; b = b.Next())
                if (b.value() is long p && (cx.obs[p] ?? representation[p]) is QlValue tc)
                {
                    ...
                    rt += ((int)rt.Count, nc.defpos);
                    sr += ((int)sr.Count, tc.defpos);
        ...
        internal virtual (Table, CTree<int,long>, CTree<long, Domain>)
            ColsFrom(Context cx, CTree<int,long> rt, CTree<long, Domain> rs)
        {
            for (var b = super.First(); b != null; b = b.Next())
                if (b.key() is Table st)
            ...
            return (new Table(-1L, cx, rs, rt, (int)rt.Count), rt, rs);
        }

        internal Names AllCols(Context cx, long exc = -1L)
        {
            ...
            for (var b = a.super.First(); b != null; b = b.Next())
                if (b.key() is Table ut)
                    r += ut.AllCols(cx);
            return r;
        }
        public long? this[int i] => (i >= 0 && i < Length) ? rowType[i] : -1L;
        ...
        internal long Create(Writer wr, Transaction tr)
        {
            ...
            if (wr.cx.db.Find(d0) is Domain d && (d0.CompareTo(d) == 0 || d.defpos < Transaction.TransPos))
                return d.defpos;
            var n = NameFor(wr.cx);
            if (n!="" && wr.cx.role.dbobjects[n] is long p)
                return p;
            Physical pp;
            pp = (kind==Qlx.UNION)?new PUnion(alts,wr.Length,wr.cx):new PDomain(this, wr.Length, wr.cx);
            pp = pp._Commit(wr, tr);
            if (pp is PUnion pu && pu.domain is not null)
                wr.cx.db+= (Database.Types,wr.cx.db.types + (pu.domain, pp.ppos));
    //        if (pp is PDomain pd && pd.dataType.kind==Qlx.REF && pd.dataType.elType is not null)
    //            wr.cx.db += (Database.Types, wr.cx.db.types + (pd.dataType.elType, pp.ppos));
            return pp.ppos;
        }
        ...
        internal (BTree<string,TableColumn>,BTree<string,TableColumn>) 
            Needs(BTree<string,TableColumn> rs,BTree<string,TableColumn> ks,Context cx)
        {
            for (var b = rowType.First(); b != null; b = b.Next())
                if (b.value() is long p && cx._Ob(p) is TableColumn c
                    && c.name != null)
                {
                    ks += (c.name, c);
                    if (!c.optional)
        ...
        internal bool Match(string n)
        {
            for (var d = super.First(); d != null; d = d.Next())
                if (d.key() is Table su && (su.name == n || su.Match(n)))
                    return true;
            return false;
        }
        internal virtual bool Match(Context cx, CTree<Domain, bool> ts, Qlx tk = Qlx.Null)
        {
            ...
            return kind switch
            {
                Qlx.NO => ts.Contains(cx._Ob(cx.role.dbobjects[name]) as Domain ?? Null),
        ...
        internal Domain SourceRow(Context cx, long dp)
        {
            ...
            for (var b = rs.First(); b != null; b = b.Next())
                rt += (i++,b.key());
            return (rt == CTree<int,long>.Empty) ? Row : new Domain(-1L, cx, Qlx.ROW, rs, rt);
        }
        /// <summary>
                ...
                if (b.value() is long cp && representation[cp] is Domain dc)
                {
                    ...
                    if (cx._Ob(cp) is DBObject tc)
                        flags[i] += (((tc is Domain td) ? td : tc.domain).optional ? 0 : 0x100) +
                            (((tc as TableColumn)?.generation != Generation.No) ? 0x200 : 0);
                }
        }
        ...
        internal void _Show(StringBuilder sb)
        {
            ...
            if (defaultValue != TNull.Value && this is not Table)
            { sb.Append(" Default="); sb.Append(defaultValue); }
            if (mem.Contains(Element))
                if (elType is Domain nt && nt.defpos>0)
                { sb.Append(' '); sb.Append(Uid(nt.defpos)); }
                else
        ...
        internal void DisplayType(Context cx, StringBuilder sb)
        {
            ...
            for (var b = representation.First(); b != null; b = b.Next(), i++)
            {
                var p = b.key();
                var cd = b.value();
                if ((cd.kind != Qlx.ARRAY && cd.kind != Qlx.MULTISET && cd.kind != Qlx.SET) || cd == null)
                    continue;
                var tn = cx.NameFor(p);
                if (tn != null)
        ...
        {
            var tn = GetType().Name;
            if (this is StandardDataType || this is Table)
                tn = "";
            ...
            if (mem.Contains(RowType))
            {
                ...
                if (rowType != CTree<int, long>.Empty)
                    sb.Append(')');
            }
            _Show(sb);
            ...
            {
                var cm = "";
                if (mem.Contains(RowType) && (int)rowType.Count == representation.Count)
                {
                    for (var b = rowType.First(); b != null; b = b.Next())
                        if (b.value() is long p)
                        {
            ...
            if (alts != CTree<Domain, bool>.Empty)
            {
                var cm = " alts (";
      ...
      //          sb.Append(metadata);
        ...
        internal static Qlx Equivalent(Qlx kind)
        {
            return kind switch
            {
                ...
                Qlx.INT or Qlx.BIGINT or Qlx.SMALLINT => Qlx.INTEGER,
        ...
        internal virtual int Typecode()
        {
            return Equivalent(kind) switch
            {
                ...
                Qlx.NUMERIC => 2,
                Qlx.REAL => 8,
                Qlx.CHECK or Qlx.LEVEL or Qlx.NCHAR => 3, // TInt or TLevel
                Qlx.CHAR => 3,
                Qlx.TIMESTAMP => 4,
                Qlx.DATE => 13,
                Qlx.BLOB => 5,
                ...
                Qlx.BOOLEAN => 9,
                Qlx.INTERVAL => 10,
                Qlx.UNION or Qlx.DOCUMENT => 14,
                Qlx.RECORD => 1, // TPosition
                Qlx.TIME => 11,
                Qlx.PERIOD => 7,
                _ => 0,
            };
        ...
        public TypedValue Get(BTree<long, Physical.Type> log, Reader rdr, long pp)
        {
            ...
            switch (Equivalent(kind))
            {
                ...
                case Qlx.TABLE:
                    {
                        ...
                        return (vs.Count == 0L || rt == CTree<int,long>.Empty) ? TNull.Value :
                ...
                case Qlx.TYPE:
                    {
                        var dp = rdr.GetLong();
                        var ut = (Table)(rdr.context.db.objects[dp] ?? throw new DBException("42105").Add(Qlx.TYPE));
        ...
        internal override BTree<long, TableRow> For(Context cx, MatchStatement ms, GqlNode xn, BTree<long, TableRow>? ds)
        {
            ...
            for (var b = alts.First(); b != null; b = b.Next())
                if (cx._Ob(b.key().defpos) is Table nt)
                    ds = nt.For(cx, ms, xn, ds);
            return ds;
        }
        internal (int, Domain) Best(Domain dt)
        {
            int j = 1;
        ...
        public virtual bool EqualOrStrongSubtypeOf(Domain dt)
        {
            ...
            if (dk == Qlx.CONTENT || dk == Qlx.Null)
                return true;
            if (alts!=CTree<Domain,bool>.Empty && dt.alts == CTree<Domain,bool>.Empty)
            {
                for (var b = alts.First(); b != null; b = b.Next())
                    if (b.key() is Domain dm && dt.EqualOrStrongSubtypeOf(dm))
                        return true;
                return false;
            }
            if (alts!=CTree<Domain,bool>.Empty && dt.alts!=CTree<Domain,bool>.Empty)
            {
                for (var b = alts.First(); b != null; b = b.Next())
                    if (b.key() is Domain dm && !dt.alts.Contains(dm))
                        return false;
                return true;
            }
        ...
        /// <summary>
        /// Output the actual object type details if not the same as nominal object type
        /// </summary>
        /// <param name="wr"></param>
        public virtual void PutDataType(Domain nt, Writer wr)
        {
            ...
            && d.defpos < Transaction.TransPos && CompareTo(nt) != 0)
                    ...
                    for (var b = nt.alts.First(); b != null; b = b.Next())
                        if (EqualOrStrongSubtypeOf(b.key()) && wr.cx.db.Find(this) is Domain a)
                        {
                            PutDataType(a, wr);
                            return;
                        }
        ...
        public virtual void Put(TypedValue tv, Writer wr)
        {
            switch (Equivalent(kind))
            {
                ...
                case Qlx.EDGETYPE:
                case Qlx.NODETYPE:
                    wr.PutLong(wr.cx.Fix(((TNode)tv).defpos));
                    return;
                case Qlx.TYPE:
                    {
                        wr.PutLong(defpos);
        ...
        public virtual int CompareTo(object? obj)
        {
            ...
            if (c != 0)
                return c;
            var bb = that.super.First();
            var b = super.First();
            for (; c == 0 && bb != null && b != null; b = b.Next(), bb = bb.Next())
                c = Comp(b.value(), bb.value());
            if (c != 0)
                return c;
            if (b != null)
                return 1;
            if (bb != null)
                return -1;
            var aa = that.alts.First();
            var a = alts.First();
            for (; c == 0 && aa != null && a != null; a = a.Next(), aa = aa.Next())
                c = Comp(a.key(), aa.key());
            if (c != 0)
                return c;
            if (a != null)
            ...
            switch (kind)
            {
                ...
                case Qlx.UNION:
                    return alts.CompareTo(that.alts);
                case Qlx.TABLE:
                case Qlx.ROW:
                    return rowType.CompareTo(that.rowType);
                case Qlx.TYPE:
        ...
        public virtual int Compare(TypedValue a, TypedValue b)
        {
            ...
            switch (Equivalent(kind))
            {
                ...
                case Qlx.MULTISET:
                    {
                        ...
                        if (a is not TMultiset && elType.CanTakeValueOf(a.dataType))
                            a = new TMultiset(this, new CTree<TypedValue,long>(a, 1L), 1);
                        if (b is not TMultiset && elType.CanTakeValueOf(b.dataType))
                            b = new TMultiset(this, new CTree<TypedValue,long>(b, 1L), 1);
                ...
                case Qlx.UNION:
                    {
                        for (var bb = alts.First(); bb != null; bb = bb.Next())
        ...
        /// </summary>
        /// <param name="dt">The other object type</param>
        ...
        public virtual bool CanTakeValueOf(Domain dt)
        {
            if (sensitive != dt.sensitive)
                return false;
            if (dt is Table ut && ut.Match(name))
                return true;
            ...
            {
                for (var b = alts.First(); b != null; b = b.Next())
                    if (dt.alts.Contains(b.key()))
                        return true;
                return false;
            }
            if (alts != CTree<Domain, bool>.Empty && dt != Content)
            {
                for (var b = alts.First(); b != null; b = b.Next())
            ...
            return ki switch
            {
                ...
                Qlx.TYPE => rowType.Count == 0L || CompareTo(dt) == 0,
                Qlx.ROW => rowType.Count == 0L || CompareTo(dt) == 0,
                Qlx.TABLE => rowType.Count == 0L || CompareTo(dt) == 0, // generic TABLE etc is ok
        ...
        public virtual bool HasValue(Context cx, TypedValue v)
        {
            ...
            if (ki == Qlx.REF  && v is TRef tf)
      //          && tf.elType?.EqualOrStrongSubtypeOf(elType) == true)
                return true;
        ...
        public CTree<long, TypedValue> Parse(Context cx, string s)
        {
            ...
            for (var b = rowType.First(); b != null; b = b.Next())
                if (b.value() is long rp && cx.NameFor(rp) is string n)
                    ns += (n, rp);
            for (var j = 0; j < (int)rowType.Count;)
            {
                ...
                case Qlx.ROW:
                    {
                        ...
                        else
                        {
                            ...
                            //          var nms = CTree<string,long>.Empty;
                            var b = rowType.First();
                            for (; b != null; b = b.Next())
                            ...
                            while (lx.ch == comma)
                            {
                                ...
                                if (b.value() is long p)
                                {
                                    if (lx.ch != ':')
                                        ...
                                        lx.Advance();
                                    if (names?[n].Item2 is long np && np != p)
                                        p = np;
                                    if (names?[n.ToUpper()].Item2 is long mp && mp != p)
                                        p = mp;
                                    lx.White();
     ...
     /*           else if (v is TInt ri && ri.ToLong() is long rp)
                {
                    for (var b = super.First(); b != null; b = b.Next())
                        if (cx._Ob(b.key()?.defpos ?? -1L) is Table t
                    && t.tableRows.Contains(rp))
                            return new TRef(rp, t);
                    if (cx._Ob(elType?.defpos??-1L) is Table u
            ...
            if (alts!=CTree<Domain,bool>.Empty)
            {
                for (var b = alts.First(); b != null; b = b.Next())
                {
                    ...
                    if (d.kind == Qlx.REF)
                    {
                        if (v is TRef rf) // && d.elType?.defpos == rf.elType?.defpos)
                            return v;
                  /*      if (v is TInt ri && cx._Ob(d.elType?.defpos ?? -1L) is Table t && ri.ToLong() is long rp 
                            && t.tableRows.Contains(rp))
                            return new TRef(rp,t); */
                        continue;
                    }
            ...
            if (kind == Qlx.COLLECT && elType is not null && v.dataType.EqualOrStrongSubtypeOf(elType))
                return v;
            for (var dd = v.dataType; dd != null; dd = (dd.super.Count == 1L) ? dd.super.First()?.key() as Table : null)
            ...
            if (v.dataType is Table ut && CanTakeValueOf(ut) && v is TSubType ts)
            ...
            switch (kn)
            {
                ...
                case Qlx.INTEGER:
                    {
                        ...
                        if (vk == Qlx.NODETYPE || vk == Qlx.EDGETYPE)
                            return v;// CheckFields(((TNode)v).id);
                    }
                    break;
                case Qlx.NUMERIC:
                    {
                ...
                case Qlx.TYPE:
                    {
                        /* None of the following should be required
                        var tx = (Table)this;
                        var px = tx.FindPrimaryIndex(cx);
                        if (px?.keys.Length == 1 && cx._Ob(px.keys.First()?.value() ?? -1L) is TableColumn t0
                            && t0.domain.kind == Qlx.INTEGER && v is TChar ic
                            && long.Parse(ic.ToString()) is long il
                            && px.rows?.Get(new CList<TypedValue>(new TInt(il)), 0) is long xp)
                            return new TInt(xp);
                        if (v is TChar tc)
                        {
                            var s = tc.ToString();
                ...
                case Qlx.MULTISET:
                    if (elType is not null && v.dataType.elType is not null)
                        ...
                        {
                            var w = CTree<TypedValue,long>.Empty;
                            var ct = 0L;
        ...
        internal TypedValue Coerce(Context cx, object ob)
        {
            ...
            switch (Equivalent(kind))
            {
                    ...
                    if (ob is Document d)
                    {
                        ...
                        foreach (var f in d.fields)
                        {
                            if (f.Key == "$check")
                            {
                                ...
                                vs += (Defpos, new TRef((long)(d["$pos"] ?? 0L), (cx._Ob(defpos) as Domain)??Ref));
                            }
                ...
                case Qlx.MULTISET:
                    ...
                    break;
             //   case Qlx.IDENTITY:
             //       if (ob is TChar tc) return new TIdent(tc.value);
             //       break;
            }
        ...
        internal CTree<Domain, bool> FindType(Context cx, CTree<string, QlValue> dc)
        {
            ...
            for (var b = cx.role.dbobjects.First(); b != null; b = b.Next())
                if (cx._Ob(b.value()) is Table nt && CanTakeValueOf(nt))
                {
                    for (var c = dc.First(); c != null; c = c.Next())
                        if (!nt.names.Contains(c.key()))
                            goto NoMatch;
                    du += (nt, true);
                    r = nt;
                NoMatch:;
        ...
        /// <summary>
        /// Select a predefined object type
        /// </summary>
        ...
        internal virtual Domain Constrain(Context cx, long lp, Domain dt)
        {
            ...
            if (ki == Qlx.UNION && dk == Qlx.UNION)
            {
                ...
                return (Domain)cx.Add(new Domain(cx.GetUid(), nt));
            }
        ...
        {
            var r = (Domain)New(dp, cx.Fix(mem));
            var ts = cx.newTypes;
            if (ts[r] is long p)
            {
                if (p != r.defpos)
                    return (Domain?)(cx._Ob(p)) ?? throw new PEException("PE1553");
                else
     ...
     //           cx.Add(r);
        ...
        internal Domain Replaced(Context cx)
        {
            ...
            {
                var od = b.value();
                var nk = cx.done[b.key()]?.defpos ?? ((cx.uids[b.key()] is long pp && pp!=0L)? pp: b.key());
            ...
            for (var b = rowType.First(); b != null; b = b.Next())
                if (b.value() is long p)
                {
                    var np = cx.done[p]?.defpos ?? ((cx.uids[b.key()] is long pp && pp != 0L) ? pp : b.key());
                    if (p != np)
                        ch = true;
                    rt += ((int)rt.Count, np);
                }
        ...
        internal override Basis ShallowReplace(Context cx, long was, long now)
        {
            ...
            if (orderFunc != null && cx._Ob(orderFunc.defpos) is Procedure pr && pr != orderFunc)
                r += (OrderFunc, pr);
        ...
        internal CTree<Domain, bool> Supertypes(Context cx)
        {
            ...
            for (var b = super.First(); b != null; b = b.Next())
                if (cx._Ob(b.key().defpos) is Domain d && !r.Contains(d))
                    r += d.Supertypes(cx);
            return r;
        }
            ...
            CTree<TypedValue, bool>? cs = null)
        {
            var r = CTree<Domain, bool>.Empty;
            var tv = _Eval(cx);
            if (tv is TTypeSpec ts && cx._Ob(ts._dataType.defpos) is Table n)
                r += (n, true);
            if (tv is TChar tc && cx._Ob(cx.role.nodeTypes[tc.value]) is Table nt)
                r += (nt, true);
            return r;
        }
                ...
                case Qlx.TYPE:
                    {
                        ...
                        for (var b = r.dataType.representation.First(); b != null; b = b.Next(), i++)
                            if (r[i] is TypedValue tv && b.value() is DBObject mo
                                && mo.infos[cx.role.defpos] is ObInfo mi)
                            {
                                var kn = b.key();
                                var p = tv.dataType;
                                var m = mi.metadata;
        ...
        {
            if (EqualOrStrongSubtypeOf(dc.rd))
                return true;
            for (var b = dc.rd.alts.First(); b != null; b = b.Next())
                if (EqualOrStrongSubtypeOf(b.key()))
                    return true;
            return false;
        }
        internal CTree<long,CTree<long,bool>> HasRefCols(Database db)
        {
            var r = CTree<long, CTree<long,bool>>.Empty;
            if (kind == Qlx.UNION)
                for (var b = alts.First(); b != null; b = b.Next())
                {
                    if (b.key() is Table it && it.colRefs.Count > 1L)
                        r += it.colRefs;
                    else
                        r += b.key().HasRefCols(db);
                }
                        ...
                        && tc.domain.kind==Qlx.REF && tc.domain.elType is Domain rd)
{
    ...
    public class Level : IComparable
    {
        ...
        {
            if (wr.cx.db.levels == CTree<Level, long>.Empty)
                wr.cx.db += (D, 0L);
        ...
        {
            if (rd.context.db.levels == CTree<Level, long>.Empty)
                rd.context.db += (D, 0L);
            Level lev;
            var lp = rd.GetLong();
            if (lp != -1)
    ...
    internal class UDType : Table
    {
        ...
        { }
        internal UDType(long lp, long dp, string nm, Table dt, BTree<long, object> m, Context cx)
    : base(dp, _Mem(nm, lp, dt, m, cx))
        { }
        static BTree<long, object> _Mem(string nm, long lp, Table dt, BTree<long, object> m, Context cx)
        {
                    ...
                    if (cx._Ob(tb.key().defpos) is Table pd)
                    {
                        ...
                        for (var c = pd.rowType.First(); c != null; c = c.Next())
                            if (c.value() is long p && rp[p] is Domain rd && rd.kind != Qlx.Null
                                && cx._Ob(p) is TableColumn tc 
                                && tc.name is string cn && !ns.Contains(cn))
                            {
                                rt += (i++,p);
                                rs += (p, rd);
                                ns += (cn, (lp, p));
            ...
            oi += (ObInfo._Names, ns);
            r += (_Domain, new Domain(cx, rs, rt, ds, new BTree<long, ObInfo>(cx.role.defpos, oi)));
            return r;
        }
        ...
        internal override DBObject New(long dp, BTree<long, object> m)
        {
            return new UDType(dp, m);
        }
        internal virtual UDType? New(Ident pn, CTree<Domain, bool> un, long dp, Context cx)
        {
            var dt = (UDType)TypeSpec.Relocate(dp);
            if (dt is UDType ne && dt.defpos != defpos)
                ne.Fix(cx);
            return (UDType?)cx.Add(new PType2(pn.ident, dt, un, -1L, dp, cx));
        }
        ...
        internal override Table Specific(Context cx, TableRow tr)
        {
  ...
  /*      public override int CompareTo(object? obj)
        {
            ...
            if (c == 0 && obj is UDType that)
            {
                var s = metadata[Qlx.LABELS]??TNull.Value;
                var t = that.metadata[Qlx.LABELS]??TNull.Value;
        ...
        public override bool EqualOrStrongSubtypeOf(Domain dt)
        {
            ...
            if (dt.defpos < 0 && kind == dt.kind || (kind == Qlx.EDGETYPE && dt.kind == Qlx.NODETYPE))
                return true;
            if (dt.kind == Qlx.UNION && dt.mem[Under] is CTree<Domain,bool>un)
            {
        ...
        protected override void _Cascade(Context cx, Drop.DropAction a, BTree<long, TypedValue> u)
        {
            for (var b = methods.First(); b != null; b = b.Next())
                if (cx._Ob(b.key()) is Method md)
                    md.Cascade(cx, a, u);
            for (var b = subtypes.First(); b != null; b = b.Next())
                if (cx._Ob(b.key()) is Domain st)
                    st.Cascade(cx, a, u);
