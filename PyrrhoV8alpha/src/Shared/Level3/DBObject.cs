...
namespace Pyrrho.Level3
{
   ...
   //     internal Domain dataType => (Domain)mem[_DataType];
    ...
    (BTree<long, ObInfo>?)mem[Infos] ?? BTree<long, ObInfo>.Empty;
        ...
        public BTree<long, Names> defs => (BTree<long, Names>)(mem[ObInfo.Defs] ?? BTree<long, Names>.Empty);
        public CTree<string, long> constraintNames => (CTree<string, long>)(mem[ObInfo.ConstraintNames] ?? CTree<string, long>.Empty);
        public CTree<string, CTree<CList<Domain>, long>> methodInfos =>
            (CTree<string, CTree<CList<Domain>, long>>)(mem[ObInfo.MethodInfos] ?? CTree<string, CTree<CList<Domain>, long>>.Empty);
 ...
 //       public string metastring => (string)(mem[ObInfo.MetaString] ?? "");
        ...
        public long schemaKey => (long)(mem[ObInfo.SchemaKey] ?? 0L);
        public Domain domain => (Domain)(mem[_Domain] ?? Domain.Null);
        ...
        internal virtual long ColFor(Context cx, string c, Qlx kt= Qlx.NO)
        {
        ...
        internal DBObject Relocate(Context cx)
        {
            if (defpos < 0)
            {
                ...
                return (Domain)dm.New(cx.Fix(mem));
            }
            if (cx.done[defpos] is DBObject rr)
                return rr;
        ...
        internal virtual (Context,DBObject) Add(Context cx,TMetadata md)
        {
            ...
            var inf = infos;
            var ch = false;
            if (rmd!=TMetadata.Empty && inf[cx.role.defpos] is ObInfo oi)
            {
                oi += (ObInfo._Metadata,oi.metadata + rmd);
                inf += (cx.role.defpos,oi);
                ch = true;
            }
            if (dmd != TMetadata.Empty && inf[definer] is ObInfo di)
            {
        ...
        internal DBObject _AddMetadata(long p,Qlx q,TypedValue v)
        {
            var oi = infos[p]??new ObInfo(name);
            oi += (ObInfo._Metadata,oi.metadata + (q,v));
            return this + (Infos,infos+(p,oi));
        }
        ...
        {
            if (cx._Ob(defpos) is not DBObject ob || ob.dbg < dbg)
             ob = this;
            if (ob.alias is string s)
                return s;
            if (defpos < 0)
                return name;
             if (ob.infos == BTree<long, ObInfo>.Empty)
                return "";
    ...
    internal class Names : BTree<string,(long,long)>  // lp, uid
    {
        ...
        public static Names operator +(Names tree, Ident id)
        {
            return (id.ident == "" || !char.IsLetter(id.ident[0])) ? tree : 
                (Names)tree.Add(id.ident, (id.lp,id.uid));
        }
        ...
        internal Names Fix(Context cx)
        {
            var r = this;
            for (var b=First();b!=null;b=b.Next())
            {
                var (v, w) = b.value();
                r += (b.key(), (cx.Fix(v), cx.Fix(w)));
            }
            return r;
        }
    ...
    internal class Dummy : DBObject
    {
        public Dummy(long dp) : this(dp, BTree<long, object>.Empty)
        { }
        public Dummy(long dp, BTree<long, object> m) : base(dp, m)
        {
        }
        public Dummy(long pp, long dp, BTree<long, object>? m = null) : base(pp, dp, m)
        {
            throw new NotImplementedException();
        }
