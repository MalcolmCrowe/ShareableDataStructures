...
namespace Pyrrho.Level3
{
  ...
  //          JoinedTypes = -430,// CTree<CTree<Domain,bool>,long> 
            ...
            Users = -287; // CTree<string,long> users defined in the database
        ...
        internal CTree<string, long> roles =>
            (CTree<string, long>)(mem[Roles] ?? CTree<string, long>.Empty);
        ...
        public BTree<long, object> objects => mem; // access this only to populate cx.obs or commit a transaction
        public CTree<long, string> procedures =>
            (CTree<long, string>?)mem[Procedures] ?? CTree<long, string>.Empty;
        internal CTree<TMetadata, long> unlabelledNodeTypes =>
    (CTree<TMetadata, long>)(mem[UnlabelledNodeTypes] ?? CTree<TMetadata, long>.Empty);
    ...
    //    internal CTree<CTree<Domain, bool>,long> joinedTypes =>
   // (CTree<CTree<Domain, bool>,long>)(mem[JoinedTypes] ?? CTree<CTree<Domain, bool>, long>.Empty);
        ...
        Database(string n,User su,Role sr,Role gu) 
            : base((Levels,CTree<Level,long>.Empty),(LevelUids,CTree<long,Level>.Empty),
                  ...
                  (Types,CTree<Domain,long>.Empty),
                  (Roles,CTree<string, long>.Empty+(sr.name??"",sr.defpos)+(gu.name??"",gu.defpos)),                
        ...
        public virtual Transaction Transact(long t, Connection con,bool? auto = null)
        {
            ...
            if (objects[roles[user]] is not User u)// 2. if the user is unknown
            {
        ...
        internal CList<Domain> Signature(Context cx, CTree<int,long> ins)
        {
            var r = CList<Domain>.Empty;
        ...
        public Procedure? GetProcedure(string n,CList<Domain> a)
        {
            return (role.procedures[n] is CTree<CList<Domain>, long> pt &&
                pt.Contains(a)) ? objects[pt[a]] as Procedure
                : null;
        }
        public Domain? Find(Domain dm)
        {
            if ((dm.defpos != -1L && dm.defpos<Transaction.Analysing) || (dm.name?.Length > 0 && !char.IsLetter(dm.name[0])))
                return dm;
            return objects[types[dm]] as Domain;
        }
