...
namespace Pyrrho.Level3
{
    ...
    internal class RestView : View
    {
        ...
        internal override DBObject Instance(long lp, Context cx, RowSet? ur = null)
        {
            ...
            cx.names += ns;
            var nd = (rt==CTree<int,long>.Empty)?TableType:new Domain(cx.GetUid(), cx, Qlx.TABLE, rs, rt, (int)rt.Count);
            ...
            var rv = new RestView(cx.GetUid(), nd.mem + (UsingTable, usingTable)
                + (NamesMap, nm) + (ObInfo._Names, ns) + (ViewDef, viewDef) + (Infos,infos));
            (cx,var o) = cx.Add(rv,infos[cx.role.defpos]?.metadata??TMetadata.Empty);
            cx._Add(o);
            rv = (RestView)o;
