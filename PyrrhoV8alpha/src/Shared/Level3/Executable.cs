...
namespace Pyrrho.Level3
{
	...
	internal class Executable : DBObject
	{
		...
		internal static Context ObeyList(CList<long> stms, string label, Context cx)
        {
            Context act = new LabelledActivation(cx, label) { result = cx.result };
            for (var eb = stms.First(); eb is not null; eb = eb.Next())
                if (cx.obs[eb.value()] is Executable e) 
                    act = e._Obey(act);
            (act as LabelledActivation)?.signal?.Throw(cx);
            cx.db = act.db; // adopt changes into the transaction
    ...
    internal class QueryStatement : Executable
    {
        ...
        public QueryStatement(long dp, Domain r)
            : base(dp, new BTree<long, object>(Result, r.defpos)+(_Domain,r)) 
        { }
    ...
    internal class NestedStatement : Executable
    {
        ...
        internal NestedStatement(long dp, Context cx, CList<long> ss,
               BTree<long, object> m)
        ...
        {
            return cx.obs[stms.Last()?.value() ?? -1L] as Executable;
        }
        internal override bool Calls(long defpos, Context cx)
        {
	...
	internal class AssignmentStatement : Executable
    {
        ...
        /// </summary>
		public long val => (long)(mem[QlValue.Val]??-1L);
        /// <summary>
        /// Constructor: An assignment statement from the parser
        /// </summary>
		public AssignmentStatement(long dp,DBObject vb,QlValue va) 
            : base(dp,BTree<long, object>.Empty+(Vbl,vb.defpos)+(QlValue.Val,va.defpos)+(Gql,GQL.LetStatement)
        ...
        public override Context _Obey(Context cx)
        {
            ...
            if (cx.obs[val] is DBObject va && va.Eval(cx) is TypedValue tv)
            {
                ...
                else if (vb is SqlReview sr && sr.chain?[0] is Ident id && cx.names[id.ident].Item2 is long cp
    ...
    internal class TreatAssignment : Executable
    {
        ...
        /// </summary>
        public TreatAssignment(long dp, Table nd) :
	...
	internal class ReturnStatement : Executable
    {
        ...
        /// </summary>
        public ReturnStatement(long dp, Context cx, Domain xp, QlValue v) : base(dp,
            new BTree<long, object>(Ret, v.defpos) + (_Domain,xp)+ (Gql, GQL.PrimitiveResultStatement))
        {
            cx.Add(v + (RowSet._Scalar, xp.kind != Qlx.TABLE));
        }
    ...
    internal class CompositeQueryStatement : QueryStatement
    {
        ...
        public override Context _Obey(Context cx)
        {
            ...
            cx = er._Obey(cx) ?? throw new DBException("42105");
            ...
            for (var b = (cx.result as RowSet)?.First(cx); b != null; b = b.Next(cx))
                bl += (b._pos, b);
            var r = new ExplicitRowSet(0L, defpos, cx, rl, bl) + (ObInfo._Names, rl.names);
            cx.result = r;
            cx.Add(r);
            return cx;
        }
	...
	internal class LoopStatement : Executable
	{
        ...
        public override Context _Obey(Context cx)
        {
            ...
            while(na==lp)
            {
                ...
                na = (LabelledActivation)se.Obey(lp);
                if (na==lp)
                    lp.signal?.Throw(a);
            }
            if (na == lp)
    ...
    internal class ForStatement : Executable
    {
        ...
        internal long list => (long)(mem[QlValue.Val] ?? -1L);
        ...
        internal ForStatement(long dp, QlValue f, QlValue v, Qlx o, long c, Executable x, Context cx)
            : this(dp, BTree<long, object>.Empty + (Procedure.Body,x.defpos)
                  ...
                  + (AssignmentStatement.Vbl, f.defpos) + (QlValue.Val, v.defpos))
        { }
        ...
        public override Context _Obey(Context cx)
        {
            var vl = (cx.val is not null && cx.val!=TNull.Value)?cx.val:cx._Ob(list)?.Eval(cx);
            var vb = cx._Ob(vbl) as QlValue ?? throw new DBException("PE10602");
            if (vl is TMultiset tm)
                for (var b = vl.First(); b != null; b = b.Next())
                    cx.binding += (vbl, b.Value());
            else if (vl is TArray ta)
                for (var i=0;i<ta.Length;i++)
                    cx.binding += (vbl, ta[i]??TNull.Value);
            else if (vl is TList tl)
                for (var i = 0; i < tl.Length; i++)
                    cx.binding += (vbl, tl[i] ?? TNull.Value);
            return cx;
        }
        public override string ToString()
        {
        ...
        public override Context _Obey(Context cx)
        {
            if (cx.obs[cursor] is SqlCursor cu && cx.obs[cu.spec] is RowSet cs)
            {
                ...
                if (rb != null)
                {
                    ...
                    for (int i = 0; i < cs.rowType.Count; i++)
    ...
    : base(dp, m ?? BTree<long, object>.Empty
        ...
        public override Context _Obey(Context cx)
        {
            ...
            var proc = (Procedure)(cx._Ob(sc.procdefpos) ?? throw new DBException("42108"));
    ...
    internal class SignalStatement : Executable
    {
        ...
        internal CTree<Qlx, long> setlist =>
            (CTree<Qlx, long>)(mem[SetList] ?? CTree<Qlx, long>.Empty);
        /// <summary>
        ...
        public static SignalStatement operator +(SignalStatement e, (Context, long, object) x)
        {
            ...
            if (e.mem[p] == o)
                return e;
            if (p == SetList)
                m += (_Depth, cx._DepthTXV((CTree<Qlx,long>)o, d));
            else if (o is long q && cx.obs[q] is DBObject ob)
            {
                d = Math.Max(ob.depth + 1, d);
                if (d > e.depth)
        ...
        internal override DBObject _Replace(Context cx, DBObject so, DBObject sv)
        {
            ...
            var sl = CTree<Qlx,long>.Empty;
            for (var b = setlist.First(); b != null; b = b.Next())
                if (b.value() is long p)
                    sl += (b.key(), (cx.uids[p] is long q && q!=0L)?q:b.value());
            r+=(cx, SetList, sl);
            return r;
        }
        public override string ToString()
	...
	internal class Signal
    {
        ...
        internal CTree<Qlx,long> setlist = CTree<Qlx,long>.Empty;
        /// <summary>
        /// Constructor: a signal statement from the parser
        /// </summary>
        /// <param name="n">The signal name</param>
    ...
    internal class GetDiagnostics : Executable
    {
        ...
        internal override DBObject _Replace(Context cx, DBObject so, DBObject sv)
        {
            ...
            for (var b = list.First(); b != null; b = b.Next())
                ls += ((cx.uids[b.key()] is long q && q!=0L) ? q: b.key(), b.value());
            r+=(cx, List, ls);
            return r;
        }
        public override string ToString()
	...
	internal class SelectSingle : Executable
    {
		...
		public override Context _Obey(Context cx)
        {
            ...
            if (((RowSet?)cx.obs[sel])?.First(cx) is Cursor rb)
            {
                ...
                a.result = new TrivialRowSet(scope,a.GetUid(), a, rb);
            }
            else
                a.NoData();
            return cx;
    ...
    internal class SqlInsert : Executable
    {
     ...
     /*           if (tg is JoinRowSet js) // combine new records in a join insert
                {
                    var rp = -1L;
                    var flds = CTree<long, TypedValue>.Empty;
                    var xt = CTree<long,bool>.Empty;
                    for (var b = js.rsTargets.First(); b != null; b = b.Next())
                        if (cx._Ob(b.key()) is Table t)
                            xt += (t.defpos, true);
                    var ps = tr.physicals;
                    for (var c = tr.physicals.PositionAt(pos); c != null; c = c.Next())
                        if (c.value() is Record r && xt.Contains(r.tabledefpos))
                        {
                            if (rp < 0)
                                rp = r.ppos;
                            flds += r.fields;
                    ...
                    tr += (Transaction.Physicals, ps + (rp,new Record4(xt, flds, 0, Level.D, rp, cx)));
                } */
            }
            return cx;
        }
        public override string ToString()
        {
            ...
            if (insCols.rowType!=CTree<int, long>.Empty)
            {
                sb.Append(" Columns: [");
                var cm = "";
                ...
                if (b.value() is CList<GqlNode> ge)
                {
                    ...
                    for (; gb != null; gb = gb.Next())
                        if (gb.value() is GqlNode nd && nd is not GqlEdge && nd is not GqlReference)
                            nd.Create(cx, (Table)nd.domain, defpos, nd.docValue);
                    TNode? bn = null;
                    GqlEdge? ed = null;
                    GqlNode? bv = null;
                    GqlNode? er = null;
                        ...
                        if (gb.value() is GqlNode g)
                        {
                                    ...
                                    && ((g is GqlReference gg && cx.obs[gg.refersTo] is GqlNode rg) ? rg.Eval(cx) : g.Eval(cx)) is TNode nn)
                            {
                                if (ed.Eval(cx) is TEdge te) // we are adding something to an existing edge
                                {
                                    ...
                                    if (er.preCon is TConnector cr && bn != null && cr.rd is not null)
                                        vs += (cr.cp, new TRef(bn.defpos, cr.rd));
                                    if (er.postCon is TConnector co && co.rd is not null)
                                        vs += (co.cp, new TRef(nn.defpos, co.rd));
                                    var tr = cx.db as Transaction ?? throw new PEException("PE03061");
                                    var done = false;
                                    for (var bp = tr.physicals.First(); bp != null && !done; bp = bp.Next())
                                        if (bp.value() is Record rr && rr.defpos == te.defpos)
                                        {
                                            rr.fields += vs;
                                            done = true;
                                    ...
                                    if (!done)
                                    {
                                        var u = new Update(te.defpos, er.domain.defpos, vs, cx.db.nextPos, cx);
                                        cx.Add(u);
                                    }
                                }
                                else // we are building a new edge
                                {
                                    var el = (Domain)(cx._Ob(ed.domain.defpos)
                                            ?? throw new DBException("42000"));
                                    var nm = (el.HasRefCols(cx.db).Count > 0L) ? el?.NameFor(cx) : "";
                                    var pn = CTree<string, bool>.Empty;
                                    for (var pb = ed.docValue.First(); pb != null; pb = pb.Next())
                                        pn += (pb.key(), true);
                                    Table? et = null;
                                    if (cx.FindTable(nm) is Domain ew)
                                    {
                                        if (ew is Table ev)
                                            et = ev;
                                        else if (ew.kind == Qlx.UNION)
                                        {
                                            for (var c = ew.alts.First(); c != null; c = c.Next())
                                                if (cx._Ob(c.key().defpos) is Table ex)
                                                {
                                                    et = ex;
                                                    goto found;
                                                }
                                            ...
                                            et = ew as Table ?? throw new PEException("PE20283");
                                    found:;
                                    }
                                    if (cx.obs[ed.domain.defpos] is Table ut)
                                        et = ut;
                                    else for (var c = ed.domain.alts.First(); c != null; c = c.Next())
                                        if (c.key() is Table t
                                            && cx._Ob(t.defpos) is Table tt && tt.colRefs.Count > 1L)
                                        {
                                            et = tt;
                                            break;
                                        }
                                    et ??= ed._Type(cx, Domain.TableType, 0L, false);
                                    var ls = ed.docValue;
                                    if (er.preCon is TConnector rc && rc.cp > 0)
                                    {
                                        ...
                                        if (s == null || s == "")
                                            s = rc.cn;
                                        ls += (s,
                                            new SqlLiteral(cx.GetUid(), Table.Connect(cx, bn, rc, rc, ed, rn)));
                                    }
                                    else
                                    ...
                                    if (er.postCon is TConnector oc && oc.cp > 0)
                                    {
                                        ...
                                        if (s == null || s == "")
                                            s = oc.cn;
                                        ls += (s,
                                            new SqlLiteral(cx.GetUid(), Table.Connect(cx, nn, oc, oc, ed, rn)));
                                    }
                                    else
...
/*        /// <summary>
    ...
    class FilterStatement(long dp, BTree<long, object> m) : Executable(dp, m)
    {
        ...
        public override Context _Obey(Context cx)
        {
            ...
            var bt = cx.result
                ?? new ExplicitRowSet(ap,cx.GetUid(), cx, Domain.Row, new BList<(long, TRow)>((cx.GetUid(), TRow.Empty)));
            var nr = BList<(long, TRow)>.Empty;
            ...
            if (bt is RowSet br)
                ...
                for (var b = bu.First(cx); b != null; b = b.Next(cx) as BindingRowSet.Bindings)
                    if (b.Matches(cx))
                        nr += (b._pos, b);
            var nb = new ExplicitRowSet(ap,bt.defpos, cx, bt, nr);
            cx.Add(nb);
            cx.result = nb;
            return cx;
        }
    ...
    BTree<long, object> m)
        ...
        //    cx.binding += cx.values;
                ...
                if (cx.obs[b.key()] is SqlReview sr)
                {
                    ...
                    for (var c = sr.chain?.First(); c != null; c = c.Next())
                            ...
                            && cx._Ob(cp) is DBObject nv)
                        {
                            ...
                            nm = id.ident;
                            warn += '.' + nm;
                            ns = nv.domain.names ?? Names.Empty;
                        }
            ...
            {
                var (i, d) = b.value();
                if ((ac.obs[i] as QlValue)?.Eval(cx) is not TInt lm || (ac.obs[d] as QlValue)?.Eval(cx) is not TChar qc) throw new PEException("PE60801");
                var il = lm.ToInt() ?? throw new PEException("PE60802");
                var pd = new Parser(cx, qc.ToString());
                if (cx._Ob(b.key()) is Table nt)
                    pd.cx.AddDefs(0L,nt);
            ...
            if (cx.result is not RowSet incoming || incoming is EmptyRowSet)
                incoming = new TrueRowSet(cx);
            _step = 0;
                        ...
                        if (ac.obs[ab.value()] is GqlMatchAlt sa)
                        {
                            ...
                            if (gf is not null && xf is not null)
                                ExpNode(ac, new ExpStep(sa.mode, xf, ab.Next(),
                                    new GraphStep(sa.mode, gf.Next(), new EndStep(this,body),body),body), null, TNull.Value, null);
                        }
            }
            cx.obs = ac.obs;
            ...
            if (gDefs == CTree<long, TGParam>.Empty) 
                cx.result = ac.result = new TrueRowSet(cx); 
            ac.SlideDown();
            cx.funcs = ac.funcs;
            cx.obs = ac.obs;
            cx.db = ac.db;
        ...
        internal RowSet? DoExclusions(RowSet? rs,Context ac,Context cx)
        {
            for (var b = ac.paths.First(); b != null; b = b.Next())
                if (ac.obs[b.key()] is GqlPath gp && rs is RowSet ba)
                {
                    ...
                    var ers = new ExplicitRowSet(rs.scope,ac.GetUid(), ac, ba, bl);
                    ac.Add(ers);
                    cx.Add(ers);
                    rs = ers;
                }
        ...
        /// </summary>
        internal abstract class Step(MatchStatement m,long e)
        {
            ...
            public abstract void Next(Context cx, Step? cn, GqlNode? px, TNode? pd, Qlx match=Qlx.TRUE);
        ...
        /// </summary>
        internal class EndStep(MatchStatement m,long e) : Step(m,e)
        {
            /// <summary>
            /// On success we call AddRow
            /// </summary>
            public override void Next(Context cx, Step? cn, GqlNode? px, TNode? pd, Qlx match = Qlx.TRUE)
            {
        ...
        /// </summary>
        internal class GraphStep(Qlx mode, ABookmark<int, long>? graphs, Step n,long e) : Step(n._ms,e)
        {
                ...
                if ((!_ms.Done(cx)) && cx.obs[matchAlts?.value() ?? -1L] is GqlMatch sm)
                {
                    for (var ab = sm.matchAlts.First(); ab != null; ab = ab.Next())
                        if (cx.obs[ab.value()] is GqlMatchAlt sa)
                        {
                            ...
                            var an = new GraphStep(matchMode, matchAlts?.Next(), next,next._e);
                            _ms.ExpNode(cx, new ExpStep(matchMode, af, ab.Next(), an,next._e), null, TNull.Value, null);
                        }
                }
                else if (match!=Qlx.FALSE||_ms.optional)
                    next.Next(cx, cn, px, pd, match);
            }
        ...
        /// </summary>
        internal class ExpStep(Qlx matchMode, ABookmark<int, long>? m, ABookmark<int,long>?ab, 
            Step n,long e) : Step(n._ms,e)
        {
            ...
            /// </summary>
            public override void Next(Context cx, Step? cn, GqlNode? pg, TNode? pd, Qlx match = Qlx.TRUE)
            {
                if (!_ms.Done(cx) && matches!=null)
                    _ms.ExpNode(cx, new ExpStep(mode, matches, ab, cn ?? next, _e), pg,
        ...
        /// </summary>
        internal class PathFirstStep(Qlx matchMode, GqlPath s, int i, TypedValue cr, GqlNode? pg,
            Step n,long e) : Step(n._ms, e)
        {
            public Qlx mode = matchMode;
            ...
            public override void Next(Context cx, Step? cn, GqlNode? px, TNode? pd, Qlx match = Qlx.TRUE)
            {
                _ms.ExpNode(cx, (ExpStep)next, px, pc, pd);
            }
            public override Step? Cont => next;
            public override string ToString()
            {
        ...
        internal class PathLastStep(Qlx matchMode, GqlPath s, TypedValue cr, TNode st, ABookmark<int, long>? m, int i, 
            Step n,long e) 
            : Step(n._ms, e)
        {
            public Qlx mode = matchMode;
            ...
            public override void Next(Context cx, Step? cn, GqlNode? px, TNode? pd, Qlx match = Qlx.TRUE)
            {
                ...
                if (match!=Qlx.FALSE && (i < sp.quantifier.Item2 || sp.quantifier.Item2 < 0) && pd is not null)
                    _ms.PathFirstNode(cx, new PathFirstStep(mode, sp, i, pc, px, 
                        new PathLastStep(mode, sp, pc, start, matches, i, next,_e),_e), pd);
            }
            public override Step? Cont => next;
            public override string ToString()
            {
            ...
            : Step(n._ms, n._e)
        {
            ...
            public override void Next(Context cx, Step? cn, GqlNode? pg, TNode? pd, Qlx match = Qlx.TRUE)
            {
                if (match==Qlx.TRUE||_ms.optional)
                     _ms.ExpNode(cx, (ExpStep)next, pg, xn.postCon, pd);
            }
            public override Step? Cont => next;
            public override string ToString()
            {
        ...
        void ExpNode(Context cx, ExpStep be, GqlNode? gp, TypedValue cr, TNode? pd)
        {
            if (cx.obs[be.matches?.value() ?? -1L] is not GqlNode xn)
            {
                be.next.Next(cx, null, gp, pd);
                return;
            }
            cx.conn.Awake();
            var qd = pd;
            ...
            if (xn is GqlPath sp && sp.pattern.First() is ABookmark<int, long> ma && pd is not null)
            {
                ...
                PathFirstNode(cx, new PathFirstStep(be.mode, sp, 0, cr, gp, 
                    new PathLastStep(be.mode, sp, cr, pd, be.matches?.Next(), 0, 
                    new ExpStep(be.mode, be.matches?.Next(), be.alts, be.next,be._e),be._e),be._e), pd);
                return;
            }
            ...
            else if (xn is GqlReference gr)
            {
                ...
                else if (cx.obs[gr.refersTo] is QlValue qv && qv.Eval(cx) is TypedValue uv)
                {
                    if (uv is TNode ur)
                        ...
                        && cx._Ob(qv.domain.target) is Table it
            ...
            else if (pd is not null && pd.dataType is Table pn)
            {
                ...
                ds = Traverse(cx, this, xn, truncator, cr, pd, pn, ctr, ds);
                for (var b = pn.super.First(); b != null; b = b.Next())
                    if (cx._Ob(b.key().defpos) is Table ps)
        ...
        bool Connects(TConnector ec,Qlx q)
        {
            return ec.q switch
            {
                Qlx.ARROW or Qlx.RARROW or Qlx.TO => q == Qlx.TO,
                Qlx.RARROWBASE or Qlx.ARROWBASE or Qlx.FROM => q==Qlx.FROM,
                Qlx.TILDE or Qlx.ARROWBASETILDE or Qlx.RBRACKTILDE or Qlx.WITH => q == Qlx.WITH,
                _ => false
            };
        }
        static BTree<long,TableRow> Traverse(Context cx,MatchStatement ms,GqlNode xn, BTree<long,(int,Domain)> tr,
            TypedValue cr, TNode pd, Table pn,CTree<Domain,int> ctr,BTree<long,TableRow>ds)
        {
        ...
        void DbNode(Context cx, NodeStep bn, TNode? pd)
        {
            int step;
            var ob = cx.binding;
            var ov = cx.values;
            ...
            while (ns is not null)
            {
                ...
                if (ns?.value() is not TableRow tr)
                    goto backtrack;
                if (cx._Ob(tr.tabledefpos) is not Table dt)
                    goto backtrack;
                var oc = cx.binding;
                ov = cx.values;
                dn = dt.Node(cx, tr);
                ...
                if (dn is not null)
                {
                    ...
                    if (!DoBindings(cx, bn, bn.xn, dn))
                        goto another;
                    if (bn.xn is GqlReference && dn is TEdge de && de.dataType is Table et
                        && et.infos[et.definer] is ObInfo ei
                        && bn.xn.preCon is TConnector xc)
                    {
                        var conn = false;
                        for (var b = et.colRefs.First(); b != null; b = b.Next())
                            for (var c = b.value().First(); c != null; c = c.Next())
                                if (Connects(xc, xc.q)
                                && dn.tableRow.vals[c.key()]?.ToLong() == pd?.defpos)
                                conn = true;
                        if (!conn)
                            goto another;
                    }
            ...
            next:
                if (match)
                    anymatch = true;
                bn.next.Next(cx, null, bn.xn, dn, match ? Qlx.TRUE : Qlx.FALSE);
            backtrack:
                if (ot is not null)
                    cx.binding += (-1L, ot[0]);
            ...
            if (optional && !anymatch)
                bn.next.Next(cx, null, bn.xn, dn, Qlx.FALSE);
            if (ot is not null)
                cx.binding += (-1L, ot[0]);
            cx.binding = ob;
        }
        ...
        void PathFirstNode(Context cx, PathFirstStep bp, TNode dn)
        {
            ...
            if (cx.obs[fn.value()] is GqlNode xn && bp.sp is GqlPath sp
                ...
                ExpNode(cx, new ExpStep(bp.mode, fn, null, bp.next,bp._e), xi, bp.pc, dn); // use ordinary ExpNode for the internal pattern
                                                                                  // dn must be an edge
        ...
        bool DoBindings(Context cx, Step st, GqlNode xn, TNode dn)
        {
            ...
            for (var b = xn.domain.names.First(); b != null; b = b.Next())
            ...
            if (xn.state != CTree<long, TGParam>.Empty && dn.dataType is Table nt)
            {
                var bi = cx.binding;
                if (nt.names is Names ns)
        ...
        CTree<TypedValue, bool> Connects(Context cx,Table et, TConnector cr)
        {
            var r = CTree<TypedValue, bool>.Empty;
            var ei = et.infos[et.definer] ?? throw new PEException("PE69102");
            for (var b = (ei.metadata[Qlx.REFERENCES] as TSet)?.First(); b != null; b = b.Next())
        ...
        static Table SubType(Context cx, Table nt,TNode dn)
        {
            for (var b = nt.subtypes.First(); b != null; b = b.Next())
                if (cx.db.objects[b.key()] is Table st && st.tableRows.Contains(dn.defpos))
