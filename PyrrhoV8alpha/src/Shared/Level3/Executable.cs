 ...
 //       internal bool detach => mem.Contains(Index.IndexConstraint);
    ...
    internal class GraphInsertStatement : Executable
    {
        ...
        public override Context _Obey(Context cx)
        {
            ...
            while (td.Count > 0)
            {
                var on = cx.newNodes; // starting state
                TNode? nb = null; // before node if defined
                GqlNode? gb = null; // before node
                for (var bb = td.First(); bb != null; bb = bb.Next())
                {
                    var gc = bb.value() as GqlNode ?? throw new PEException("PE78010");
                    TNode? nc = null;
                    if (cx.values[gc.defpos] is TNode dc)
                    {
                        nc = dc;
                        td -= gc.defpos;
                    }
                    else if (gc is GqlReference gr && cx.values[gr.refersTo] is TNode nr)
                    {
                        nc = nr;
                        td -= gr.defpos;
                    }
                    else
                    {
                        ...
                        if (gc.postCon is TConnector co && bb?.Next()?.value() is GqlNode gn
                            && co.rd is not null && cx.values[gn.defpos] is TNode na)
                            vs += (co.cp, new TRef(na.defpos, co.rd));
                        else ok = false;
                        for (var c = gc.domain.First(); c != null; c = c.Next())
                            if (cx.obs[c.value()] is QlValue q && q.domain.kind == Qlx.REF
                                && !vs.Contains(c.value()) && q.Eval(cx) is TNode qn
                                && q.domain.elType is Table dm)
                                vs += (c.value(), new TRef(qn.defpos, dm));
                            else ok = false;
                        /*                                                for (var bp = tr.physicals.First(); bp != null && !done; bp = bp.Next())
                                                                    ...
                                                                    {*/
                        var et = gc.domain as Table;
                        if (cx.FindTable(gc.name ?? "") is Domain ew)
                        {
                            if (ew is Table ev)
                                et = ev;
                            else if (ew.kind == Qlx.UNION)
                            {
                                ...
                                throw new DBException("22G0W", gc.domain.name);
                            }
                            else
                                et = ew as Table ?? throw new PEException("PE20283");
                        found:;
                        }
                        if (cx.obs[gc.domain.defpos] is Table ut)
                            et = ut;
                        else for (var cc = gc.domain.alts.First(); cc != null; cc = cc.Next())
                            if (cc.key() is Table t
                                && cx._Ob(t.defpos) is Table tt && tt.colRefs.Count > 1L)
                            {
                                et = tt;
                                break;
                            }
                        et ??= gc._Type(cx, Domain.TableType, 0L, false);
                        var ls = gc.docValue;
                        var ng = gc.Create(cx, (Table)gc.domain, 0L, ls);
                        cx.Add(ng);
                        nc = cx.values[ng.defpos] as TNode;
                        if (nc != null)
                            td -= gc.defpos;
                    }
...
/*        /// <summary>
    ...
    internal class MatchStatement : QueryStatement
    {
        ...
        internal CList<TNode> trail => (CList<TNode>)(mem[Trail] ?? CList<TNode>.Empty);
        /// <summary>
    ...
    BTree<long, object> m)
        ...
        /// </summary>
        internal abstract class Step(long e)
        {
            public long _e = e;
            /// <summary>
            ...
            Step n,long e) : Step(e)
        {
            ...
            public override string ToString()
            {
                var sb = new StringBuilder("First"+((alt.mode == Qlx.NONE) ? "Path" : alt.mode.ToString()));
                sb.Append(im);
            ...
            : Step(e)
        {
            ...
            public override bool Next(Context cx, MatchStatement _ms, Step? cn, GqlNode? px, TNode? pd, Qlx match = Qlx.TRUE)
            {
                ...
                if (sp.inclusionMode != Qlx.ANY && pd?.defpos is long pp)
                {
                    ...
                    cx.binding += (sp.defpos, new TRow(sp.domain, cx.values)); // is this good enough?
                }
                if (_ms.Done(cx))
            ...
            public override string ToString()
            {
                var sb = new StringBuilder("Last"+((alt.mode == Qlx.NONE) ? "Path" : alt.mode.ToString()));
                sb.Append(im);
                sb.Append(Show(sp?.pattern.First()));
                sb.Append(','); sb.Append(next.ToString());
                return sb.ToString();
        ...
        bool DbNode(Context cx, NodeStep bn, TNode? pd, QlValue? pa=null)
        {
            ...
            while ((!r) && ns is not null)
            {
                ...
                if (dn is null)
                    goto another;
                if (bn.alt.mode == Qlx.TRAIL && dn is TEdge && cx.binding[pa?.defpos??-1L] is TRow pv 
                    && pv.HasNode(dn))
                    goto another;
                if (bn.alt.mode == Qlx.ACYCLIC && dn is not null && cx.binding[pa?.defpos??-1L] is TRow pw
                    && pw.HasNode(dn) == true)
                    goto another;
                if (dn is not null)
                {
                    if (dn.defpos!=pd?.defpos) // don't duplicate the last node in the path after PathLastStep
                        ms += (Trail, ot + dn);
                    if (pa is not null && cx.binding[pa.defpos] is TRow pu)
                    {
                        pu += (cx, dn);
                        cx.binding += (pa.defpos, pu);
                    }
                    if (!ms.DoBindings(cx, bn, bn.xn, dn))
                        goto another;
