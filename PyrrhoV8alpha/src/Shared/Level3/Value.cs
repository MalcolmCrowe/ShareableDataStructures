...
namespace Pyrrho.Level3
{
    ...
    internal class QlValue : DBObject,IComparable
    {
        ...
        internal long val => (long)(mem[Val] ?? -1L);
      //  internal long selectDepth => (long)(mem[SelectDepth] ?? -1L);
        ...
        internal bool scalar => (bool)(mem[RowSet._Scalar]??false);
    ...
    : base(nm.uid, cx.DoDepth((m ?? BTree<long, object>.Empty) + (ObInfo.Name, nm.ident)
        ...
        {
            if (cx._Ob(Eval(cx).dataType.defpos) is Table nt) 
        ...
        internal virtual RowSet RowSetFor(long ap,long dp,Context cx,CTree<int,long> us,
        ...
        internal virtual Domain FindType(Context cx,Domain dt,Qlx q = Qlx.NO)
        {
        ...
        public override string ToString()
        {
            ...
            if (id == null && name!=null && name!="") { sb.Append(' '); sb.Append(name); }
    ...
    internal class SqlReview : QlValue
    {
        ...
        {
            if (name != null
                && ob.names[name].Item2 is long cp && cx.obs[cp] is TableColumn tc)
                return f + (defpos,new QlInstance(ob.names[name].Item1,defpos, cx, name, from, tc));
    ...
    internal class QlInstance : QlValue
    {
        ...
        {
            if (n?.ident is string s
                && domain is Table  && domain.names[s].Item2 is long p)
            {
    ...
    /// <summary>
    /// A TYPE constraint for use in CAST and in Match
    ///     
    /// </summary>
    internal class SqlTypeExpr : QlValue
    {
        internal static SqlTypeExpr Empty = new SqlTypeExpr(new Ident("", --_uid), Domain.TypeSpec);
        internal Qlx kind => (Qlx)(mem[Domain.Kind] ?? Qlx.NO);
        /// <summary>
        /// constructor: a new Type expression
        /// </summary>
        /// <param name="ty">the type</param>
        internal SqlTypeExpr(Ident id, Domain ty) : base(id.uid,
            new BTree<long, object>(_Domain, ty) + (ObInfo.Name, id.ident))
        { }
        protected SqlTypeExpr(long dp, BTree<long, object> m) : base(dp, m) { }
        internal SqlTypeExpr(long dp, Qlx op, QlValue? lt, QlValue? at, BTree<long, object> m)
            : base(dp, _Mem(op, lt, at, m))
        { }
        static BTree<long, object> _Mem(Qlx op, QlValue? lt, QlValue? at, BTree<long, object> m)
        {
            m += (SqlValueExpr.Op, op);
            if (lt is not null)
                m += (Left, lt.defpos);
            if (at is not null)
                m += (Right, at.defpos);
        ...
        internal override TypedValue _Eval(Context cx)
        {
            if (kind == Qlx.DOUBLEARROW && cx.obs[left] is QlValue q)
                return q.Eval(cx);
        ...
        internal Table? ForExtra(Context cx, BTree<long, object>? m = null, CTree<TypedValue, bool>? cs = null)
        {
            ...
            var oi = Eval(cx).dataType.OnInsert(cx, defpos, m, cs);
            return (oi.Count == 1 || dc is null || dc.Count == 0 || op == Qlx.COLON) ? oi.First()?.key() as Table : null;
        }
 /*       internal CTree<Domain, bool> OnInsert(Context cx, long ap, BTree<long, object>? m = null,
            CTree<TypedValue, bool>? cs = null)
        {
            var r = CTree<Domain, bool>.Empty;
            var lb = (cx.obs[left] as QlValue) ?? throw new PEException("PE66031");
            var lf = (lb.Eval(cx) as TTypeSpec)?.OnInsert(cx, lb) ?? Domain.Null;
            var rb = (cx.obs[right] as QlValue) ?? throw new PEException("PE66032");
           var rg = (rb.Eval(cx) as TTypeSpec)?.OnInsert(cx, rb) ?? Domain.Null;
            ...
            switch (k)
            {
                ...
                case Qlx.DOUBLEARROW:
                    ...
                    return lf.OnInsert(cx, ap, lm + (Domain.Under, new CTree<Domain, bool>(rg, true)));
                case Qlx.NODETYPE:
                case Qlx.EDGETYPE:
                    if (name is string n && cx._Ob(cx.role.dbobjects[n]) is Table t)
                            r += (t.Build(cx, null, ap, "", dc, k)??t, true);
                     return r;
                case Qlx.NO:
                    if (cx._Ob(cx.role.dbobjects[name ?? ""]) is Table d)
                        r += (d, true);
                    return r;
                default:
                    return r;
        ...
        {
            var sb = new StringBuilder(base.ToString());
            sb.Append(' '); sb.Append(kind.ToString());
            return sb.ToString();
        }
    }
    /// <summary>
    ...
    internal class SqlTreatExpr : QlValue
    {
        ...
        internal new long val => (long)(mem[TreatExpr]??-1L);
        /// <summary>
    ...
    internal class SqlCaseSimple : QlValue
    {
        ...
        {
            var r = cx.DoDepth(new BTree<long,object>(_Domain,dm) + (QlValue.Val, vl.defpos) 
    ...
    QlValue? left, QlValue? right, BTree<long, object>? mm = null)
        ...
        internal override Domain FindType(Context cx, Domain dt, Qlx q=Qlx.NO)
        {
            var le = cx.obs[left];
            var rg = cx.obs[right];
    ...
    internal class SqlLiteral : QlValue
    {
        internal const long
            _Val = -317;// TypedValue
        internal new TypedValue val=>(TypedValue)(mem[_Val]??TNull.Value);
        ...
        {
            var vt = val.dataType;
            if (dt.alts != CTree<Domain, bool>.Empty)
            {
                for (var b = dt.alts.First(); b != null; b = b.Next())
                    if (b.key().CanTakeValueOf(vt))
                        return FindType(cx, b.key(), q);
                goto bad;
            }
    ...
    internal class SqlDateTimeLiteral : SqlLiteral
    {
        ...
        /// </summary>
        /// <param name="cx">the context</param>
        /// <param name="op">the object type</param>
    ...
    {
        internal CTree<int,long> rowType     // rarely used: see ParseSqlInsert with LBRACE
            => (CTree<int,long>)(mem[Domain.RowType] ?? CTree<int,long>.Empty);
        ...
        static BTree<long, object> _Mem(Context cx, BList<DBObject> vs, BTree<long, object>? m)
        {
            ...
            m ??= BTree<long, object>.Empty;
            m = m + (_Domain, dm) + (Domain.Aggs, dm.aggs);
            var d = 0;
            for (var b = vs.First(); b != null; b = b.Next())
            {
                var ob = b.value();
                m += (ob.defpos, ob);
                d = Math.Max(d, ob.depth);
            }
            return m + (_Depth,d+1);
        }
    ...
    Domain xp, CList<long> vs)
        ...
        public static SqlRow operator +(SqlRow s, (Context,QlValue) x)
        {
            ...
            return (SqlRow)s.New(s.mem + (_Domain,
                dm.New(dm.mem + (Domain.RowType, dm.rowType + ((int)dm.rowType.Count,sv.defpos))
    ...
    internal class SqlOldRow : SqlRow
    {
        ...
        static BTree<long, object> _Mem(Ident ic, Context cx, RowSet fm)
        {
            ...
            var r = fm.mem + (_Ident, ic) + (ObInfo.Name, ic.ident) 
                + (_From, fm.defpos) + (_Depth, fm.depth + 1);
            var ids = Names.Empty;
            var on = cx.names;
    ...
    internal class SqlNewRow : SqlRow
    {
        ...
        {
            var tg = cx._Dom(fm.target);
            var r = fm.mem + (_Depth,fm.depth+1) + (_Ident,ic)
    ...
    internal class SqlRowArray : QlValue
    {
        ...
        internal override RowSet RowSetFor(long ap, long dp, Context cx, CTree<int, long> us,
            CTree<long, Domain> re)
        {
            var dm = domain;
    ...
    internal class SqlValueMultiset : QlValue
    {
        ...
        internal override TypedValue _Eval(Context cx)
        {
            var vs = CTree<TypedValue,long>.Empty;
            var n = 0;
            for (var b = multi?.First(); b != null; b = b.Next())
    ...
    internal class QlMatchValue : QlValue
    {
        ...
        {
            return new QlMatchValue(dp, cx.Fix(mem));
        }
        public override string ToString()
        {
...
: base(dp, BTree<long,object>.Empty + (Parms, acts) + (ObInfo.Name, pn) + (Var, tg))
        ...
        {
            var ro = cx.role ?? throw new DBException("42105").Add(Qlx.ROLE);
            if (cx._Ob(procdefpos) is not Procedure proc
    ...
    internal class SqlMethodCall : SqlCall // instance methods
    {
        ...
        internal override SqlCall? Resolve(Context cx)
        {
            if (cx.obs[var] is QlValue tg && tg.domain is UDType ut && procdefpos<0
                        ...
                        && cx._Ob(ut.methodInfos[name ?? ""]?[ms] ?? -1L) is Method pr)
            ...
            BTree<long,object> m, long ap)
        {
            ...
            if (ls[0] is SqlMethodCall mc && cx.obs[mc.var] is QlValue ov && mc.name is not null)
            {
                ...
                if (cx.role is not null && ov.domain.infos[cx.role.defpos] is ObInfo oi && name is string nm
                    && cx._Ob(oi.methodInfos[nm]?[cx.db.Signature(cx, parms)] ?? -1L) is Procedure pr)
                { 
        ...
        internal override TypedValue _Eval(Context cx)
        {
            ...
            cx.values = oc + (procdefpos, r) + (defpos, r);
            return r;
        }
        internal override CTree<long, bool> Needs(Context cx, long rs)
        {
    ...
    internal class SqlFunction : QlValue
    {
        ...
        // public long val => (long)(mem[Val] ?? -1L); declared in  QlValue
        /// <summary>
        ...
        internal static Domain _Type(Context cx, Qlx op, QlValue? val, QlValue? op1)
        {
            switch (op)
            {
                ...
                case Qlx.END: return val?.domain.elType ?? Domain.Content;
                ...
                case Qlx.START: return val?.domain.elType ?? Domain.Content;
        ...
        internal override TypedValue _Eval(Context cx)
        {
            ...
            {
                var gc = og.groupCols;
                var key = (gc == null || gc.kind == Qlx.Null) ? TRow.Empty : new TRow(gc, cx.values);
                ...
                case Qlx.END:
                    {
                        TypedValue? a = cx.obs[val]?.Eval(cx);
                        if (a is TArray n)
                            return n.array.Last()?.value()??TNull.Value;
                        return TNull.Value;
                    }
                case Qlx.EXP:
                    {
          ...
          /*      case Qlx.ID:
                ...
                case Qlx.LABELS:
                    {
                        ...
                        var tb = (Table)n.dataType;
                        var ti = tb.infos[tb.definer];
                        for (var b = ti?.metadata[Qlx.LABELS]?.First(); b!=null;b=b.Next())
                            s += b.Value();
                        return s;
                    }
              ...
              /*          if (op1 != -1L && op2 != -1L)
                        ...
                        for (var b = rs.rsTargets.First(); b != null; b = b.Next())
                            if (b.value() is long p && cx.cursors[p] is Cursor c)
                                if (c is TableRowSet.TableCursor tc && tc._rec is TableRow tr) 
                                    return new TRef(tr.defpos,cx._Ob(tr.tabledefpos) as Domain??Domain.Content);
                ...
                case Qlx.RECORD: throw new PEException("71713");
                /*        {
                            var o1 = cx.obs[op1]?.Eval(cx) ?? TNull.Value;
                            if (cx._Ob(cx.role.dbobjects[o1.ToString().ToUpper()]) is not Table tb)
                                return TNull.Value;
                            ...
                            { 
                                ...
                                {
                                    if (tb is Table et && et.metadata[Qlx.REFERENCES] is TSet ts)
                ...
                case Qlx.START:
                    {
                        TypedValue? a = cx.obs[val]?.Eval(cx);
                        if (a is TArray n)
                            return n.array.First()?.value() ?? TNull.Value;
                        return TNull.Value;
                    }
                case Qlx.STDDEV_POP:
                    {
                ...
                case Qlx.TYPE:
                    {
                        ...
                        if (a is not TNode n)
                            return TNull.Value;
                        return new TTypeSpec(n.dataType.name,n.dataType);
                    }
                case Qlx.UNNEST:
                    {
...
cx.obs[high] is not QlValue hi)
    ...
    internal class NullPredicate : QlValue
    {
        ...
        /// the value to test
        /// </summary>
        public new long val => (long)(mem[NVal]??-1L);
        /// <summary>
    ...
    internal class GqlNode : QlValue
    {
        internal const long
            ...
            Before = -473,      // GqlNode
            Delimit = -325,     // bool
            DocValue = -477,    // CTree<long,QlValue>  Table
            IdValue = -480,     // long     QlValue of Int
            Implied = -461,     // CTree<string,bool>
        ...
        public GqlNode(Ident nm, BList<Ident> ch, Context cx, long i, CTree<string, QlValue> d,
           ...
           : this(cx, nm, ch, i, d, Fix(nm,tgs), _Type(dm,cx,d,m), m)
        {  }
        internal GqlNode(Context cx, Ident nm, BList<Ident> ch, long i, CTree<string, QlValue> d,
            ...
            : base(nm, nm, ch, cx, dm, _Mem(nm, i, d, tgs, dm, cx, m))
        {
            if (dm is null && tgs[-(long)Qlx.TYPE] is TGParam tg && cx.names[tg.value].Item2 is long t)
                for (var b = cx.obs[t]?.names.First(); b != null; b = b.Next())
        ...
        { }
        static CTree<long,TGParam> Fix(Ident nm,CTree<long,TGParam>tgs)
        {
            for (var b = tgs.First(); b != null; b = b.Next())
                if (b.value() is TGParam t && t.value == nm.ident)
                    tgs += (nm.uid, new TGParam(nm.uid, nm.ident,t.dataType, t.type, t.from));
            return tgs;
        }
        static BTree<long, object> _Mem(Ident nm, long i, CTree<string, QlValue> d, 
            CTree<long, TGParam> tgs, Domain dm, Context cx, BTree<long, object>? m)
        {
            m ??= BTree<long, object>.Empty;
            if (i > 0)
            {
   ...
   //                     && !(cx.obs[p] is FormalParameter)) // there may be others to avoid
        ...
        protected static Domain _Type(Domain? dm, Context cx, CTree<string, QlValue> d, BTree<long, object>? m)
        {
            if (dm is Table nt && dm.defpos >= 0)
            {
                ...
                // this complicates the coding here a lot 
                for (var b = nt.super.First(); b != null; b = b.Next())
                    if (cx.names[b.key().name].Item1 > cx.names[nt.name].Item1) // need to do this using lexpos
                        st = b.key();
                if (d.Count != 0L)
                    nt = st.ForExtra(cx, m + (DocValue, d))??nt;
                return nt;
            }
            if (m is null)
            ...
            if (dm?.kind == Qlx.AMPERSAND)
                return dm;
            Domain r = dm?.ForExtra(cx, m + (DocValue, d)) ?? Domain.TypeSpec;
            if (r.defpos<0 && m[Before] is GqlNode be && be.postCon is TConnector pc)
        ...
        internal override (DBObject?, Ident?) _Lookup(long lp, Context cx, Ident ic, Ident? n, DBObject? r)
        {
            if (n is not null && domain.infos[cx.role.defpos] is ObInfo si && ic.sub is not null
                &&  cx._Ob(si.names[n.ident].Item2) is TableColumn tc1)
            {
            ...
            bool allowExtras = true)
        {
            if (dt.kind == Qlx.AMPERSAND)
            {
                ...
                {
                    r1 = r1.Create(cx, (Table)b.key(), ap, ls, false);
                    ds += (b.key(), true);
                }
                return (GqlNode)cx.Add(r1 + (_Domain, new Table(cx.GetUid(), ds)));
            }
            var nd = this;
            var nm = dt.NameFor(cx)??throw new PEException("PE66041");
            if (allowExtras)
            {
                ...
                if (dt.infos[cx.role.defpos] is ObInfo oi)
                    cx.names += oi.names;
                if (dt.defpos >= 0 && dt.defpos < Transaction.Analysing)
                    dt = dt.Check(cx, nd, 0L, allowExtras);
                else if (cx._Ob(cx.role.defpos) is Role rr
                          && cx._Ob(rr.dbobjects[nm]) is Table ot)
                    dt = ot;
                else
                    dt = dt.Build(cx, this, ap, nm, ls);
            }
            var nt = dt;
            nd += (_Domain, nt);
            ...
            if (nt.defpos > 0 && !cx.parse.HasFlag(ExecuteStatus.GraphType))
            {
                ...
                for (var bb = ts.rowType.First(); bb != null && tb != null; bb = bb.Next(), tb = tb.Next())
                    if (bb.value() is long bq && cx.NameFor(bq) is string n9
                        && ts.sRowType[bb.key()] is long ic  && cx._Ob(ic) is TableColumn co
                        && ValueFor(ls,n9,co) is QlValue sv
        ...
        //                    ?? ls[(co.tc as TConnector)?.cn??""]) is QlValue sv 
                ...
                for (var b = ll.First(); b != null; b = b.Next())
                {
                    ...
                    if (sv.defpos < 0)
                        continue;
                    dr += ((int)dr.Count,sv.defpos);
                    ds += (sv.defpos, co.domain);
                    cx.Add(sv);
                }
                var fm = (TableRowSet)ts.New(cx.GetUid(), ts.mem + (Domain.RowType, dr) 
                    + (Domain.Representation, ds) + (Domain.Display, (int)dr.Count));
                var dm = (dr==CTree<int,long>.Empty)?Domain.Row:new Domain(-1L, cx, Qlx.ROW, ds, dr);
                cx.Add(fm);
                var rn = new SqlRow(cx.GetUid(), cx, dm);
                cx.Add(rn);
                QlValue n = rn;
      ...
      //              nd += (IdValue, tn.tableRow.defpos);
        ...
        //                && (ls[n9] ?? ls[n9.ToUpper()] ?? ls[n9.ToLower()]
        //                    ?? ls[(co.tc as TConnector)?.cn??""]) is QlValue sv 
        private QlValue? ValueFor(CTree<string, QlValue> ls, string n9, TableColumn co)
        {
            var r = ls[n9] ?? ls[n9.ToUpper()] ?? ls[n9.ToLower()];
            if (r != null) return r;
      /*      for (var b = model; b != null; b = b.Next())
                if (ls[b.value().cn] is QlValue v)
                    return v; */
            return null;
        }
       ...
       //         return false;
            ...
            {
                var opg = cx.parsingGQL;
                cx.parsingGQL = Context.ParsingGQL.Match; // yuk prevent creation of nodetype binding variable
                var ts = domain.OnInsert(cx, defpos);
                cx.parsingGQL = opg;
                for (var b = ts.First(); b != null; b = b.Next())
                    if (b.key().defpos == Domain.TypeSpec.defpos
                        || (cx.obs[ms.bindings] as RowSet)?.representation.Contains(b.key().defpos) != false)
                        ts -= b.key();
                if (n.dataType is Table nt
              //      && (nt.metadata.Contains(Qlx.NODETYPE) || nt.metadata.Contains(Qlx.EDGETYPE))
                    && !nt.Match(cx, ts))
                    return false;
            }
            for (var b = docValue?.First(); b != null; b = b.Next())
  ...
  /*          if (search != CTree<long, bool>.Empty)
                for (var b = n.dataType..names.First(); b != null; b = b.Next())
   ...
   //         cx.names = ob;
        ...
        internal CTree<long, TypedValue> EvalProps(Context cx, Table nt)
        {
            var r = CTree<long, TypedValue>.Empty;
            if (nt.infos[cx.role.defpos] is not ObInfo ni)
                return r;
            // this is used only in Schema queries
            for (var b = docValue?.First(); b != null; b = b.Next())
                if (cx._Ob(ni.names[b.key()].Item2) is DBObject ob && b.value().Eval(cx) is TypedValue v)
                    r += (ob.defpos, v);
            return r;
        }
        internal virtual Table? InsertSchema(Context cx)
        {
            var ns = names;
            var tb = domain as Table;
            for (var b=docValue.First();b!=null;b=b.Next())
        ...
        public override string ToString()
        {
            ...
            if (before != null) { sb.Append(" before "); sb.Append(Uid(before.defpos)); }
            if (after != null) { sb.Append(" after "); sb.Append(Uid(after.defpos)); }
            cm = " ";
    ...
    internal class GqlReference : GqlNode
    {
        ...
        internal GqlReference(Context cx,long ap, long dp, GqlNode n, BTree<long,object> m)
            : this(dp, n.mem -PreCon - PostCon -Before - After 
                  + (RefersTo, n.defpos)+ _Mem(cx,m,n))
        {
            cx.names += (n.name ?? throw new PEException("PE40431"), (ap,n.defpos));
        }
        internal GqlReference(long dp, QlValue v, Table tg)
            : this(dp, BTree<long,object>.Empty + (_Domain,tg) + (RefersTo, v.defpos)) 
        { }
        static BTree<long,object> _Mem(Context cx,BTree<long,object> m,GqlNode n)
        {
            if (cx.obs[n.defpos] is GqlNode g)
                m += (_Domain, g.domain);
            return m;
        }
    ...
    internal class GqlEdge : GqlNode
    {
            ...
            : base(nm, ch, cx, i, d, tgs, _Conn(nm, dm, cx, m), m)
        {
            ...
            {
                Domain bd = bn.domain;
                if (dm?.defpos > 0 != true && tgs[-(long)Qlx.TYPE] is TGParam tg)
                    if (cx.names[tg.value].Item2 is long p && p < Transaction.Analysing && cx.bindings.Contains(p))
                        cx.names += (tg.value, (nm.lp, p));
                    else
                        dm = (cx._Ob(cx.role.edgeTypes[tg.value])
                            ?? cx._Ob(cx.role.nodeTypes[tg.value])
                            ?? cx._Ob(cx.role.dbobjects[tg.value])) as UDType;
                var u = CTree<Domain, bool>.Empty;
                //we are in a MatchExp and should constrain b
                if (m[PreCon] is TConnector ec && ec.rd.defpos > 0)
                {
                    u += (ec.rd, true);
                    bd = ec.rd;
                }
                    ...
                    for (var b = dm?.colRefs.First(); b != null; b = b.Next()) // the domains that dm references
                        ...
                        for (var c = b.value().First(); c != null; c = c.Next())
                            if (cx._Ob(c.key()) is TableColumn cr)
                                ...
                                {
                                    var (q,cn) = e.value();
                                    if ((ec.q == Qlx.ARROWBASE || ec.q == Qlx.FROM) && q != Qlx.FROM) continue;
                                    if ((ec.q == Qlx.RARROW || ec.q == Qlx.TO) && q != Qlx.TO) continue;
                                    if (ec.q == Qlx.TILDE && q != Qlx.WITH) continue;
                                    bd = cx._Ob(cr.toType) as Domain??bd;
                                    u += (bd, true);
                                } */
        ...
        protected static Table _Conn(Ident nm, Domain dm, Context cx, BTree<long, object>? m)
        {
            ...
            var os = md[Qlx.REFERENCES] as TSet;
            (et,var ns) = et.Connect(cx, bf, af, pr, true, nm.uid);
            (et, ns) = et.Connect(cx, bf, af, po, true, nm.uid);
            if (ns != os && ns.Cardinality() >= 2)
                cx.Add(new PMetadata(et.name, 0, et, md.ToString(), md, cx.db.nextStmt, cx.db.nextPos, cx));
            return et;
        }
       ...
       /// <summary>
            ...
            CTree<TypedValue,bool>? cr = null)
        {
            ...
            var ls = docValue;
            var ti = et.infos[cx.role.defpos] ?? throw new PEException("PE40461");
            var ts = ti.metadata[Qlx.EDGETYPE] as TSet ?? new TSet(Domain.Connector);
            for (var b = cr.First(); b != null; b = b.Next())
                if (b.key() is TConnector cc)
                {
                    for (var c = ts.First(); c != null; c = c.Next())
                        if (c.Value() is TConnector oc)
                        {
                            if (cc.q == oc.q && (cc.cn == oc.cn || cc.cn == ""))
                            {
                                if (cc.rd.EqualOrStrongSubtypeOf(oc.rd))
                                    cr -= cc; // request is already satisfied
                                else
                                { // we need to create or extend the target union type
                                    var un = new CTree<Domain, bool>(cc.rd, true);
                                    if (oc.rd.kind != Qlx.UNION)
                                        un += (oc.rd, true);
                                    else
                                        un += oc.rd.super;
                                    var nc = new TConnector(oc.q, cc.cn,
                                        new Domain(cx.GetUid(), Qlx.UNION, un), oc.cp);
                                    ts = ts - oc + nc;
                                    goto skip;
                                }
                            }
                            if (cc.q != oc.q)
                                continue;
                            if (cc.rd.defpos >= 0 && oc.rd.defpos != cc.rd.defpos && !oc.rd.super.Contains(cc.rd))
                                continue;
                        }
            ...
            if (cr!=null)
                et = (Table)cx.Add(et, ti.metadata +(Qlx.REFERENCES, ts)).Item2;
            cx.Add(et);
            return et;
        }
