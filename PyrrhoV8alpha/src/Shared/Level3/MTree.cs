...
namespace Pyrrho.Level3
{
    ...
    /// The implementation is by single-level SqlTrees (keycomponent TypedValue)->(T TypedValue)
    /// where T is TMtree for next level, TPartial for CTree(pos,true), or TInt for pos.
    ...
    internal sealed class MTreeBookmark 
    {
        ...
        internal MTreeBookmark ResetToTiesStart(int depth)
        {
            ...
            var mn = new MTreeBookmark(_mt, _outer, _info, false,
                    m, ov?.First(), _inner?._pos??0);
            return (mn.Value() != null)? mn: this;
        }
        /// <summary>
