    ...
    internal class Transaction : Database
    {
        ...
        public override Database RdrClose(ref Context cx)
        {
            ...
            else
            {
                ...
                for (var b = cx.db.objects.PositionAt(TransPos); b != null && b.key() < Analysing; b = b.Next())
                    cx.db -= b.key();
                var aff = cx.affected;
        ...
        internal override Database Commit(Context cx)
        {
            ...
            for (var b = cx.checkEdges.First(); b != null; b = b.Next())
                    ...
                    && et.tableRows[b.key()] is TableRow nr)
                {
                    if (et.mindexes != CTree<long, long>.Empty)
                        mt += (et, true);
                    for (var c = (et.infos[et.definer]?.metadata[Qlx.REFERENCES] as TSet)?.First();
                        c != null; c = c.Next())
            ...
            for (var b = mt.First(); b != null; b = b.Next())
            {
                ...
                for (var c = ta.mindexes.First(); c != null; c = c.Next())
                    ...
                    {
                        var minv = co.infos[co.definer]?.metadata[Qlx.MINVALUE].ToInt() ?? 0;
                        var maxv = co.infos[co.definer]?.metadata[Qlx.MAXVALUE].ToInt() ?? int.MaxValue;
            ...
            {
                ph = pb.value();
                var reT = ph.RelTables(db);
                if (ph.type == Physical.Type.PTransaction)
                    pt = (PTransaction)ph;
                for (var b = reT.First(); b != null; b = b.Next())
                    if (cx.rdS[b.key()] is CTree<long, bool> ct)
                    {
                        if (ct.Contains(-1L))
                        {
                            cx.rconflicts++;
                            throw new DBException("40008", reT);
                        }
            ...
            string query, string? mime, string sdata)
        {
            ...
            if (path.Length >= ln)
            {
                ...
                if (cx.role != null && long.TryParse(path[j], out long t) && cx.db.objects[t] is Table tb
                    && name != null)
                {
                    if (sk != 0L && tb.schemaKey > sk)
                        throw new DBException("2E307", tb.name);
                    fm = tb.RowSets(new Ident(tb.name, cx.GetUid()), cx, tb, cx.GetPrevUid(), 0L);
                    j++;
                }
                switch (method)
                {
                ...
                case "table":
                    {
                        ...
                        if (cx.db == null || cx.db.role == null
                            || objects[cx.db.role.dbobjects[tbn.ident]] is not Table tb)
        ...
        internal override Context Put(Context cx, TableRowSet rs, string s)
        {
            ...
            for (var c = rs.rowType.First(); c != null; c = c.Next())
                if (c.value() is long cc)
                {
                    ...
                    else if (cx._Ob(cc) is DBObject oc && oc.name != null)
                        n = oc.name;
        ...
        static void AccessColumns(Context cx, bool grant, Grant.Privilege pr, Table tb, PrivNames list, BList<DBObject> grantees)
        {
            ...
            for (var b = tb.representation.First(); b != null; b = b.Next())
                if (b.value() is DBObject oc && oc.name != null
                    && !ne && list.cols.Contains(oc.name))
                {
                    list.cols -= oc.name;
		...
		internal void AccessRole(Context cx, bool grant, CList<string> rols, BList<DBObject> grantees, bool opt)
        {
            ...
            for (var b = rols.First(); b is not null; b = b.Next())
                if (b.value() is string s && roles.Contains(s) && objects[roles[s]] is Role ro)
