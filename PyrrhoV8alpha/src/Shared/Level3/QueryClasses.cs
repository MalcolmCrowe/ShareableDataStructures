...
namespace Pyrrho.Level3
{
    ...
    internal class WindowSpecification : Domain
    {
        internal const long
            Exclude = -222,// Qlx
            _High = -223, //WindowBound
            _Low = -224,// WindowBound
        ...
        /// </summary>
        internal WindowBound? low => (WindowBound?)mem[_Low];
        /// <summary>
        /// high WindowBound if specified
        /// </summary>
        internal WindowBound? high => (WindowBound?)mem[_High];
        /// <summary>
  ...
  /*          var cs = CTree<long, bool>.Empty;
    ...
    internal class UpdateAssignment : Basis,IComparable
    {
        ...
        public TypedValue tv => (TypedValue)(mem[SqlLiteral._Val]??TNull.Value);
        public UpdateAssignment(long vb, long vl) : base(BTree<long, object>.Empty
            + (AssignmentStatement.Vbl, vb) + (QlValue.Val, vl))
        { }
        public UpdateAssignment(long vb, TypedValue vl) : base(BTree<long, object>.Empty
            + (AssignmentStatement.Vbl, vb) + (SqlLiteral._Val, vl))
        { }
        ...
        {
            if (mem.Contains(SqlLiteral._Val))
                return tv;
            return cx.obs[val]?.Eval(cx)??TNull.Value;
        }
        public override string ToString()
        {
            var sb = new StringBuilder(base.ToString());
            sb.Append(" Vbl: ");sb.Append(DBObject.Uid(vbl));
            sb.Append(" Val: "); sb.Append((val<0)?tv.ToString():DBObject.Uid(val));
            return sb.ToString();
        }
