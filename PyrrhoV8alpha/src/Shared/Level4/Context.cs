...
namespace Pyrrho.Level4
{
    ...
    internal class Context
    {
        ...
        internal ObTree obs = ObTree.Empty; // database objects with metadata for the current role
        ...
        internal CTree<long, CTree<long, CTree<string, TMetadata>>> metaPending
            = CTree<long, CTree<long, CTree<string, TMetadata>>>.Empty; internal CTree<CTree<string, bool>, long> unlabelledNodeTypesInfo = CTree<CTree<string, bool>, long>.Empty;
        internal CTree<CTree<string, bool>, long> unlabelledEdgeTypesInfo = CTree<CTree<string, bool>, long>.Empty;
        ...
        internal Names ApplyDone(Names ns)
        {
            ...
            {
                var p = b.value();
                if (done[p.Item2] is DBObject nb)
                    r += (b.key(), (p.Item1,nb.defpos));
                else
                    r += (b.key(), p);
            }
            return r;
...
*/
  ...
  /*      internal Iix GetIid()
        ...
        internal int _DepthTil(CTree<int, long> t, int d)
        {
            for (var b = t?.First(); b != null; b = b.Next())
                d = _DepthV(b.value(), d);
            return d;
        ...
        internal int _DepthTXV<V>(CTree<V, long> t, int d) where V : IComparable
        {
            for (var b = t.First(); b != null; b = b.Next())
                d = _DepthV(b.value(), d);
            return d;
        ...
        internal int _DepthTVV(CTree<long, long>? t, int d)
        {
            for (var b = t?.First(); b != null; b = b.Next())
        ...
        internal int _DepthTlPiD(BTree<long,(int,Domain)> tg, int d)
        {
            ...
            {
                var (i, dm) = b.value();
                if (db.objects[b.key()] is Table et)
        ...
        internal BTree<long, QlValue> Map(CTree<int,long> s)
        {
        ...
        internal (Context,DBObject) Add(DBObject ob,TMetadata md)
        {
            return ob.Add(this,md);
        }
        internal void AddObs(Table tb)
        {
            ...
            Add(tb.framing); // outside the if() as tb itself might already be there
        }
        /// <summary>
     ...
     //       Table? tb = null;
            for (var b = cx.obs?.PositionAt(0); b != null; b = b.Next())
                    ...
                    && ob.infos[role.defpos] is ObInfo oi && oi.name is string nm)
                {
                    ...
                    if (dm == null)
                        continue;
                    if (dm.kind == Qlx.TABLE || db.objects[dm.defpos] is Table)
                        ti = ic;
                    else if (ti != null)
                    {
                        ic = new Ident(ti, ic);
      ...
      //          cx.Add(tb + (Table.TableCols, rs));
        ...
        internal static bool Match(CTree<int,long> a, CTree<int,long> b)
        {
        ...
        internal Domain _Dom(long dp, params (long, object)[] xs)
        {
            ...
            if (dm.defpos < Transaction.Executables)
            {
                ...
                if (nd is Table ne && nd.defpos != dm.defpos)
                    ne.Fix(this);
                dm = nd;
            }
        ...
        internal int _Depth(long p, object o, int d)
        {
            ...
            //        Domain.NodeTypes => _DepthTDb((CTree<Domain, bool>)o, d),
                    JoinRowSet.OnCond => _DepthTVV((CTree<long, long>)o, d),
             ...
             //       Table.RefIndexes => _DepthTlTDD((CTree<long,CTree<Domain,Domain>>)o,d),
                    ...
                    Domain.RowType => _DepthTil((CTree<int,long>)o, d),
                    SignalStatement.SetList => _DepthTXV((CTree<Qlx,long>)o, d),
                    GroupSpecification.Sets => _DepthLl((CList<long>)o, d),
                    RowSet.SIMap => _DepthTXV((CTree<long, long>)o, d),
                    ...
                    QlValue.Val => Math.Max(((QlValue)o).depth, d),
        ...
        internal void Replace(DBObject was, DBObject now)
        {
            ...
            if (now is Table ne && was.defpos != now.defpos)
                ne.Fix(this);
            _Add(now);
        ...
        internal BTree<long,TableRow> ReplacedTlR(BTree<long,TableRow>rs)
        {
            for (var b = rs.First(); b != null; b = b.Next())
            {
                ...
                var nk = done[k]?.defpos ?? k;
                var tr = b.value();
                var np = Replaced(tr.tabledefpos);
        ...
        internal CTree<int, long> ReplacedTil(CTree<int, long> wh)
        {
            for (var b = wh.First(); b != null; b = b.Next())
                if (b.value() is long p)
                {
        ...
        internal BTree<long, BList<TypedValue>> ReplacedTlBV(BTree<long, BList<TypedValue>> t)
        {
            for (var b = t.First(); b != null; b = b.Next())
        ...
        internal static CTree<int, long> ShallowReplace(CTree<int, long> fl, long was, long now)
        {
        ...
        /// <summary>
        /// Important for this design (v7.1): 
        ...
        internal DBObject? _Ob(long dp)
        {
            if (dp == -1L)
                return null;
            if ((done[dp] ?? obs[dp]) is DBObject ob)
                return ob;
            if ((db.objects[dp]??db.objects[Fix(dp)]) is not DBObject o)
                return null;
            o += (DBObject._Depth, _Depth(0, o, 0));
            if (o.infos[role.defpos] is ObInfo oi)
                o = o + (ObInfo._Metadata, oi.metadata)
                    + (ObInfo.Name,oi.name??"") + (ObInfo._Names,oi.names)
                    + (ObInfo.ConstraintNames,oi.constraintNames)
                    + (ObInfo.Defs,oi.defs) + (ObInfo.Inverts,oi.inverts)
                    + (ObInfo.MethodInfos,oi.methodInfos)
                    + (ObInfo.SchemaKey,oi.schemaKey)
                    + (ObInfo.Privilege,(o.definer==role.defpos)?Grant.AllPrivileges:oi.priv);
            return Add(o);
        }
        ...
        internal BTree<long, object> Replaced(BTree<long, object> m)
        {
            ...
            for (var b = m.First(); b != null; b = b.Next())
            {
                ...
                //        case GqlNode._Label: v = ((DBObject)v).Replaced(this); break;
                        ...
                        case ObInfo._Metadata: v = ((TMetadata)v).Replaced(this); break;
                        ...
                        case SignalStatement.SetList: v = Replaced((CTree<Qlx, long>)v); break;
                        ...
                        case InstanceRowSet.SRowType: v = ReplacedTil((CTree<int,long>)v); break;
                        ...
                        case QlValue.Val: v = Replaced((long)v); break;
        ...
        /* This may be clever? but I don't think we have an opportunity to use it
        // invariant depth(ob)==depth(Fix(ob))
        internal void Fix()
        {
            var ndepths = BTree<int, ObTree>.Empty;
            for (var b = depths.First(); b != null; b = b.Next())
            {
                var nd = b.value();
                for (var c = nd.First(); c != null; c = c.Next())
                {
                    var p = c.key();
                    var np = Fix(p);
                    if (c.value() is DBObject ob) {
                        if (done[p] is DBObject od && od != ob)
                            nd += (np, od);
                        else
                        {
                            var nb = ob.New(np,Fix(ob.mem));
                            done += (p, nb);
                            nd += (np, nb);
                            obs += (np, nb);
                        }
        ...
        internal long Fix(long dp)
        {
            ...
            if (dp != r && !done.Contains(r) && obs[dp] is DBObject o)
            {
                done += (r, new Dummy(r));
                o = (DBObject)o.Fix(this);
                done += (r, o);
            }
            return r;
        }
        internal BTree<long,object> Fix(BTree<long,object> m, long w = -1L)
        {
            var r = BTree<long, object>.Empty;
            for (var b = m.First(); b != null; b = b.Next())
            {
                ...
                if (v is not null)
                    switch (k)
                    {
                        case HandlerStatement.Action: v = Fix((long)v); break;
                        case Level3.Index.Adapter: v = Fix((long)v); break;
                        case GqlNode.After: v = ((GqlNode)v).Fix(this); break;
                        case RowSet.AggDomain: v = ((Domain)v).Fix(this); break;
                        case Domain.Aggs: v = FixTlb((CTree<long, bool>)v); break;
                        case Domain.Alts: v = FixTDb((CTree<Domain, bool>)v); break;
                        case SqlValueArray._Array: v = FixLl((CList<long>)v); break;
                        case SqlSelectArray.ArrayValuedQE: v = Fix((long)v); break;
                        case RowSet.Assig: v = FixTUb((CTree<UpdateAssignment, bool>)v); break;
                        case GqlNode.Before: v = ((GqlNode)v).Fix(this); break;
                        case Procedure.Body: v = Fix((long)v); break;
                        case BindingRowSet.Builder: v = Fix((long)v); break;
                        case CallStatement.Call: v = Fix((long)v); break;
                        case SqlCaseSimple.CaseElse: v = Fix((long)v); break;
                        case SqlCaseSimple.Cases: v = FixBll((BList<(long, long)>)v); break;
                        ...
                        case Check.Condition: v = Fix((long)v); break;
  //                      case TableColumn.Connectors: v = FixTDV((CTree<Domain,TConnector>)v); break;
                        case GqlEdge.Connects: v = FixTCl((CTree<TConnector, long>)v); break;
                        case ObInfo.ConstraintNames: v = FixTsl((CTree<string, long>)v); break;
                        case Domain.Constraints: v= FixTlb((CTree<long,bool>)v); break;
                        case ForStatement.CountCol: v = Fix((long)v); break;
                        case RowSet._CountStar: v = Fix((long)v); break;
                        case FetchStatement.Cursor: v = Fix((long)v); break;
                        case TransitionTableRowSet.Data: v = FixTlR((BTree<long, TableRow>)v); break;
                        case RowSet._Data: v = Fix((long)v); break;
                        case Role.DBObjects: v = FixTsl((CTree<string,long>)v); break;
                        case Domain.Default: v = ((TypedValue)v).Fix(this); break;
                        case Domain.DefaultRowValues: v = FixTlV((CTree<long, TypedValue>)v); break;
                        case TransitionRowSet.Defaults: v = FixTlV((CTree<long, TypedValue>)v); break;
                        case DBObject.Definer: v = Fix((long)v); break;
                        case DBObject.Defpos: v = Fix((long)v); break;
                        case ObInfo.Defs: v = FixTlN((BTree<long, Names>)v); break;
                        case Transaction.Diagnostics: v = FixTqV((BTree<Qlx, TypedValue>)v); break;
                        case SqlTreatExpr._Diff: v = Fix((BTree<long, object>)v, w); break;
                        case WindowBound.Distance: v = ((TypedValue)v).Fix(this); break;
                        ...
                        case ConditionalStatement.Elsif: v = FixLl((CList<long>)v); break;
                        case PeriodDef.EndCol: v = Fix((long)v); break;
                        case LikePredicate.Escape: v = Fix((long)v); break;
                        case ExplicitRowSet.ExplRows: v = FixBlT((BList<(long, TRow)>)v); break;
                        case RowSet.Filter: v = FixTlV((CTree<long, TypedValue>)v); break;
                        ...
                        case RowSet.GroupIds: v = FixTlD((CTree<long, Domain>)v); break;
                        case Grouping.Groups: v = FixLg((CList<Grouping>)v); break;
                        case RowSet.Having: v = FixTlb((CTree<long, bool>)v); break;
                        case DBObject.HavingDom: v = ((Domain)v).Fix(this); break;
                        case QuantifiedPredicate.High: v = Fix((long)v); break;
                        case WindowSpecification._High: v = ((WindowBound)v).Fix(this); break;
                        ...
                        case Procedure.Inverse: v = Fix((long)v); break;
                        case ObInfo.Inverts: v = Fix((long)v); break;
                        case RowSet.ISMap: v = FixTll((CTree<long, long>)v); break;
              ...
              //          case JoinRowSet.JoinUsing: v = FixTll((CTree<long, long>)v); break;
                        ...
                        case Database.LevelUids: v = FixTlL((CTree<long,Level>)v); break;
                        case MemberPredicate.Lhs: v = Fix((long)v); break;
                        case MultipleAssignment.LhsType: v = ((Domain)v).Fix(this); break;
                        case GetDiagnostics.List: v = Fix((CTree<long, Qlx>)v); break;
                        case MultipleAssignment.List: v = FixLl((CList<long>)v); break;
                        ...
                        case ObInfo._Names: v = ((Names)v).Fix(this); break;
                        ...
                        case Trigger.OldTable: v = Fix((long)v); break;
                        case JoinRowSet.OnCond: v = FixTll((CTree<long, long>)v); break;
                        ...
                        case FetchStatement.Outs: v = FixLl((CList<long>)v); break;
                        ...
                        case Database.Prefixes: v = FixTsl((CTree<string, long>)v); break;
                        case SqlCall.ProcDefPos: v = Fix((long)v); break;
                        ...
                        case RestRowSet.RestValue: v = ((TList)v).Fix(this); break;
                        case RestRowSet._RestView: v = Fix((long)v); break;
                        case QueryStatement.Result: v = Fix((long)v); break;
                        case ReturnStatement.Ret: v = Fix((long)v); break;
                        case MultipleAssignment.Rhs: v = Fix((long)v); break;
                        case QlValue.Right: v = Fix((long)v); break;
                        case CompositeRowSet._Right: v = Fix((long)v); break;
                        case TransitionTableRowSet.RMap: v = FixTll((CTree<long, long>)v); break;
                        case Database.Roles: v = FixTsl((CTree<string, long>)v); break;
                        case RowSet.RowOrder: v = ((Domain)v).Fix(this); break;
                        case SqlRowArray._Rows: v = FixLl((CList<long>)v); break;
                        case RowSet._Rows: v = FixLR((CList<TRow>)v); break;
                        case Domain.RowType: v = FixTil((CTree<int, long>)v); break;
                        ...
                        case SignalStatement.SetList: v = Fix((CTree<Qlx, long>)v); break;
                        case GroupSpecification.Sets: v = FixLl((CList<long>)v); break;
                        case RowSet.SIMap: v = FixTll((CTree<long, long>)v); break;
                        case TableRowSet.SingleNode: v = Fix((long)v); break;
                        case TrivialRowSet.Singleton: v = ((TRow)v).Fix(this); break;
                        case RowSet._Source: v = Fix((long)v); break;
                        case SqlCursor.Spec: v = Fix((long)v); break;
                        case QlInstance.SPos: v = Fix((long)v); break;
                        case SqlRowSet.SqlRows: v = FixLl((CList<long>)v); break;
                        case InstanceRowSet.SRowType: v = FixTil((CTree<int, long>)v); break;
                        case PeriodDef.StartCol: v = Fix((long)v); break;
                        case NestedStatement.Stms: v = FixLl((CList<long>)v); break;
                        case QlValue.Sub: v = Fix((long)v); break;
                        case ForwardReference.Subs: v = FixTlb((CTree<long, bool>)v); break;
                        case Domain.Subtypes: v = FixTlb((CTree<long, bool>)v); break;
                        case Database.Suffixes: v = FixTsl((CTree<string, long>)v); break;
                        case SqlValueList.Svs: v = Fix((long)v); break;
                        case Table.SystemPS: v = Fix((long)v); break;
                        case TableColumn._Table: v = Fix((long)v); break;
                        case Table.TableChecks: v = FixTlb((CTree<long, bool>)v); break;
                        case Level3.Index.TableDefPos: v = Fix((long)v); break;
                    ...
                    //    case Table.TableRows: v = FixTlR((BTree<long, TableRow>)v); break; !!!!!
                        ...
                        case Trigger.TrigPpos: v = Fix((long)v); break;
                        case MatchStatement.Truncating: v = FixTlll((BTree<long, (long, long)>)v); break;
                        case Database.Types: v = FixTDl((CTree<Domain, long>)v); break;
                        case Domain.Under: v = FixTDb((CTree<Domain, bool>)v); break;
                        case Database.UnlabelledNodeTypes: v = FixTTlbl((CTree<CTree<long, bool>, long>)v); break;
                        case Trigger.UpdateCols: v = FixLl((CList<long>)v); break;
                        case RestRowSetUsing.UrlCol: v = Fix((long)v); break;
                        case Executable.UseGraph: v = Fix((long)v); break;
                        case Database.Users: v = FixTsl((CTree<string, long>)v); break;
                        case RestRowSetUsing.UsingCols: v = FixLl((CList<long>)v); break;
                        case RowSet.UsingOperands: v = FixTll((CTree<long, long>)v); break;
                        case RestView.UsingTable: v = Fix((long)v); break;
                        case RestRowSetUsing.UsingTableRowSet: v = Fix((long)v); break;
                        case QlValue.Val: v = Fix((long)v); break;
                        case SqlLiteral._Val: v = ((TypedValue)v).Fix(this); break;
                        case QuantifiedPredicate.Vals: v = FixLl((CList<long>)v); break;
                        case SqlInsert.Value: v = Fix((long)v); break;
                        case SelectRowSet.ValueSelect: v = Fix((long)v); break;
                        case Executable.ValueType: v = ((Domain)v).Fix(this); break;
                        case SqlCall.Var: v = Fix((long)v); break;
                        case AssignmentStatement.Vbl: v = Fix((long)v); break;
                        case View.ViewResult: v = Fix((long)v); break;
                        case WhileStatement.What: v = Fix((long)v); break;
                        case SimpleCaseStatement.Whens: v = FixLl((CList<long>)v); break;
                        case FetchStatement.Where: v = Fix((long)v); break;
                        case RowSet._Where: v = FixTlb((CTree<long, bool>)v); break;
        ...
        private object FixTTlbl(CTree<CTree<long, bool>, long> ps)
        {
        ...
        private object FixTlll(BTree<long, (long, long)> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private CList<TRow> FixLR(CList<TRow> ps)
        {
            var r = CList<TRow>.Empty;
            var ch = false;
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
                var v = b.value();
                var nv = (TRow)v.Fix(this);
                if (v != nv)
                    ch = true;
                r += nv;
            }
        ...
        private CTree<long,Level> FixTlL(CTree<long, Level> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private CTree<long,Qlx> FixTlq(CTree<long, Qlx> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private CTree<Level,long> FixTLl(CTree<Level, long> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private CList<Grouping> FixLg(CList<Grouping> ps)
        {
            var r = CList<Grouping>.Empty;
            var ch = false;
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
                var v = b.value();
                var nv = (Grouping)v.Fix(this);
                if (v != nv)
                    ch = true;
                r += nv;
            }
        ...
        private object FixTlG(CTree<long, GqlNode> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private object FixTsQ(CTree<string, QlValue> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private object FixBQ(BList<QlValue> vs)
        {
            var r = BList<QlValue>.Empty;
            for (var b=vs.First();b!=null;b=b.Next())
            {
                var v = b.value();
                var nv = (QlValue)v.Fix(this);
        ...
        private object FixTqV(BTree<Qlx, TypedValue> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private object FixTCl(CTree<TConnector, long> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private object FixTlTDi(CTree<long, CTree<Domain, int>> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private CTree<Domain, int> FixTDi(CTree<Domain, int> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private object FixTDl(CTree<Domain, long> ps)
        {
            for (var b = ps.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private object FixTsl(CTree<string, long> ns)
        {
            for (var b = ns.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        private BTree<long,PeriodSpec> FixTlP(BTree<long, PeriodSpec> ps)
        {
            for (var b=ps.First();b!=null;b=b.Next())
            {
                var k = b.key();
        ...
        internal void AddDefs(long ap,Domain dm)
        {
            names += (dm.NameFor(this), (ap,dm.defpos));
        ...
        {
            for (var b = ns.First(); b != null; b = b.Next())
                if (_Ob(b.value().Item2) is DBObject ob && !names.Contains(b.key()))
                    Add(b.key(), b.value().Item1,ob);
        }
            ...
            if (role != null && pr.infos[role.defpos] is ObInfo oi && oi.name != null)
            {
                ...
                for (var b = pr.ins.First(); b != null; b = b.Next())
                    if (b.value() is long p && obs[p] is FormalParameter pp) {
                        ...
                        names += (pp.NameFor(this), (pn.lp,pp.defpos));
                        values += (p, TNull.Value); // for KnownBy
                        Add(pp);
                    }
                names += (oi.name, (0L,pr.defpos));
                defs += (pr.defpos, names-oi.name);
            }
        }
        internal Table? FindTable(string s)
        {
            var p = role.nodeTypes[s];
            if (p == 0)
                p = role.edgeTypes[s];
            if (p == 0)
                p = role.dbobjects[s];
        ...
        {
            return role.dbobjects.Contains(n) ? _Ob(role.dbobjects[n]) : null;
        }
        ...
        internal Domain GroupCols(CList<long> gs, Domain dm)
        {
            ...
            var i = 0;
            for (var b = gc.First(); b != null; b = b.Next(),i++)
                rt += (i,b.key());
            return (rt==CTree<int,long>.Empty)?Domain.Row:new Domain(-1L, this, Qlx.ROW, gc, rt);
        }
        internal Domain GroupCols(CTree<int,long> gs, Domain dm)
        {
            var gc = CTree<long, Domain>.Empty;
            if (dm is RowSet rs)
                for (var b = gs.First(); b != null; b = b.Next())
        ...
        internal long UnlabelledNodeSuper(CTree<string, bool> ps)
        {
            var rp = -1L;
            int n = 0;
            for (var b = unlabelledNodeTypesInfo.First(); b != null; b = b.Next())
            {
                var nn = 0;
                for (var c = b.key().First(); c != null; c = c.Next())
        ...
        internal CTree<int,long> FixTil(CTree<int,long> ord)
        {
        ...
        internal CTree<UpdateAssignment,bool> FixTub(CTree<UpdateAssignment,bool> us)
        {
            ...
            {
                var k = b.key();
                var u = (UpdateAssignment)k.Fix(this);
                if (u != k)
                us = us - k + (u,true);
            }
            return us;
        ...
        internal CTree<UpdateAssignment,bool> FixTUb(CTree<UpdateAssignment,bool> u)
        {
            for (var b = u.First(); b != null; b = b.Next())
            {
                var k = b.key();
                var p = (UpdateAssignment)k.Fix(this);
                if (p != k)
                    u += (p, true);
            }
            return u;
        ...
        internal BTree<long,Names> FixTlN(BTree<long,Names> ds)
        {
            for (var b=ds.First(); b != null;b=b.Next())
            {
                var k = b.key();
        ...
        internal CTree<Domain, bool> FixTDb(CTree<Domain, bool> su)
        {
            ...
            {
                var k = b.key();
                var nk = (Domain)(db.objects[Fix(k.defpos)]??throw new PEException("PE80881"));
                if (nk.CompareTo(k)!=0)
                    su = su -k + (nk, true);
            }
            return su;
        }
        internal CTree<Domain,TConnector> FixTDV(CTree<Domain,TConnector> cs)
        {
            for (var b = cs.First(); b != null; b = b.Next())
            {
                ...
                var nk = (Domain)(db.objects[Fix(k.defpos)] ?? throw new PEException("PE80881"));
                var nv = v.Fix(this);
                if (nk.CompareTo(k) != 0 || v.CompareTo(nv)!=0)
                    cs = cs - k + (nk, nv);
            }
            return cs;
        }
        ...
        {
            for (var b = rs.First(); b != null; b = b.Next())
                if (db.objects[Fix(b.value().defpos)] is Domain od)
                {
                    var k = b.key();
                    var p = Fix(k);
                    if (p < -1L)
        ...
        internal CTree<long, CTree<Domain, bool>> FixTlTDb(CTree<long, CTree<Domain,bool>> rs)
        {
            for (var b = rs.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        internal BList<(long, long)> FixBll(BList<(long, long)> rs)
        {
            var r = BList<(long, long)>.Empty;
            var ch = false;
            for (var b = rs.First(); b != null; b = b.Next())
        ...
        internal BList<(long, TRow)> FixBlT(BList<(long, TRow)> rs)
        {
            ...
            for (var b = rs.First(); b != null; b = b.Next())
            {
                ...
                var (v1, v2) = b.value();
                var n1 = Fix(v1);
                var n2 = new TRow((Domain)v2.dataType.Fix(this),FixTlV(v2.values));
                if (n1 != v1 || n2 != v2)
                    ch = true;
                r += (n1, n2);
            }
        ...
        internal CTree<long, CTree<long, bool>> FixTTllb(CTree<long, CTree<long, bool>> ds)
        {
            for (var b = ds.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        internal BTree<long,TableRow> FixTlR(BTree<long,TableRow> t) 
        {
            for (var b=t.First();b!=null;b=b.Next())
            {
                ...
                if (k != nk)
                    t -= k;
                var r = b.value()??throw new PEException("70771");
                var nr = r.Fix(this);
                if (nk>0 && !r.Equals(nr))
                    t += (nk, nr);
            }
        ...
        internal CTree<long, CList<TypedValue>> FixTlLV(CTree<long, CList<TypedValue>> t)
        {
            for (var b = t.First(); b != null; b = b.Next())
            {
                var k = b.key();
        ...
        internal CTree<long, string> FixTls(CTree<long, string> t)
        {
            for (var b=t.First();b!=null;b=b.Next())
            {
                var k = b.key();
    ...
    internal class Framing : Basis // for compiled code
    {
        ...
        static BTree<long, object> _Mem(Context cx,long nst)
        {
            ...
            var os = ObTree.Empty;
            for (var b = cx.obs.PositionAt(nst); b != null; b = b.Next())
            {
                var k = b.key();
                if (cx.parse!=ExecuteStatus.Prepare &&
                    k >= Transaction.HeapStart) // we only want new executables
                    continue;
        ...
        protected override BTree<long, object> _Fix(Context cx, BTree<long, object> m)
        {
            ...
            r += (Obs, os);
            r += (Executable.ValueType, valueType.Fix(cx));
            return r;
        }
        public override string ToString()
        {
    ...
    internal class PeriodSpec
    {
        ...
        internal PeriodSpec Fix(Context context)
        {
            return new PeriodSpec(periodname, kind, (QlValue?)time1?.Fix(context), (QlValue?)time2?.Fix(context));
        }
    }
    /// <summary>
