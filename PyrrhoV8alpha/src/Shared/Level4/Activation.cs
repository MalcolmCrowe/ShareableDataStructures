...
namespace Pyrrho.Level4
{
    ...
    internal abstract class Activation : Context
    {
        ...
        protected Activation(Context cx, DBObject pr)
            : base(cx, (Role)(cx._Ob(pr.definer) ?? throw new PEException("PE639")),
            ...
            : base(cx, p)
        {
            ...
            if (p is Method mt)
            {
                ...
                for (var b = mt.udType.rowType.First(); b != null; b = b.Next())
                    if (b.value() is long c)
                        bindings += (c, cx._Ob(c) as Domain ?? Domain.Null);
            }
        }
    ...
    internal class TableActivation : TargetActivation
    {
        ...
        internal long refcol = -1L; // for setnull cascade
        ...
        internal readonly PIndex.ConstraintType flags;
        ...
        internal TableActivation(Context cx, TableRowSet ts, RowSet data, PTrigger.TrigType tt,
           Domain? iC = null,PIndex.ConstraintType fl=PIndex.ConstraintType.NoType) : base(cx, ts, tt)
        {
            _cx = cx;
            ...
            for (var b = table.refCols.First(); b != null; b = b.Next())
                    ...
                    && !casc.Contains(rt.defpos))
                {
                    ...
                    if (ca)
                    {
                        var rf = new TableRowSet(GetUid(), this, rt.defpos, 0L);
                        for (var xb = rf.rowType.First(); xb != null; xb = xb.Next())
                            if (xb.value() is long xp && obs[xp] is QlInstance xc
                                ...
                                Add(tc);
                        var da = (obs[rf.data] ?? obs[rf.defpos]) as RowSet;
                        if (da is null)
                            continue;
                        casc += (rt.defpos, new TableActivation(this, rf, da, rtt) 
                            { updates=ru, refcol = b.key()});
                    }
                }
                    ...
                    for (var ass = ts.assig.First(); ass != null; ass = ass.Next())
                    {
                        ...
                        else if (ts.iSMap[ua.vbl] is long vp)
                        {
                            ...
                            while (vb is QlInstance sc && db.objects[sc.sPos] is DBObject oc) // Views have indirection here
                                vb = oc;
                            if (vb is TableColumn tc && tc.generation != Generation.No)
...
//#endif
        ...
        internal override Context EachRow(Context _cx,int pos)
        {
            ...
            var ne = -1L;
            if (next != null && next.exec is SqlInsert si && si.forNode is long fn && fn > 0)
            {
                ne = fn;
                var pp = next.newNodes[ne] ?? CTree<long, bool>.Empty;
                next.newNodes += (ne, pp + (np, true));
            }
                ...
                case PTrigger.TrigType.Insert:
                    {
                        ...
                        count++;
                        if (db.objects[table.defpos] is Table nt)
                            values += (r.defpos, (nt.colRefs.Count>1L)?new TEdge(this,rc):new TNode(this,rc));
                ...
                case PTrigger.TrigType.Update:
                    {
                        ...
                        if (table.enforcement.HasFlag(Grant.Privilege.Update)
                            && (db.user == null ||
                            (db.user.defpos != db.owner && ((rc != null) ?
...
//#if MANDATORYACCESSCONTROL
                        if (db.user==null || 
                            db.user.defpos != db.owner && table.enforcement.HasFlag(Grant.Privilege.Delete) ?
...
//#endif
        ...
        CTree<long,TypedValue> ReferenceColumns(Cursor tgc,CTree<long,TypedValue> vs)
        {
                    ...
                    && tc.domain.kind==Qlx.REF && db.objects[tc.domain.elType?.defpos??-1L] is Table rt)
                {
                        ...
                        && rt.FindPrimaryIndex(this) is Level3.Index px)
                    {
                        ...
                        for (var c = tc.keyMap.First(); c != null; c = c.Next())
                        {
                            ...
                            k += vs[c.value()] ?? TNull.Value;
                        }
                        var lp = px.rows?.Get(k, 0);
                        if (fd && (!rd) && lp != null)
                            vs += (tc.defpos, new TInt(lp.Value));
                        else if (rd && !fd)
                        {
                ...
                case PTrigger.TrigType.Update:
                    ...
                    if (cx.obs[ass.key().vbl] is QlValue c){
                        ...
                        while (oc is QlInstance sc && cx.obs[sc.sPos] is DBObject oo) // Views have indirection here
                            oc = oo;
                        if (oc is TableColumn tc && tc.generation != Generation.No)
    ...
    internal class TriggerActivation : Activation
    {
        ...
        internal readonly CTree<long, long> trigTarget, targetTrig; // trigger->target, target->trigger
        /// <summary>
            ...
            : base(cx, ot)
        {
            ...
            obs = cx.obs;
            if (cx._Ob(ot.defpos) is Trigger t0 && t0.Instance(trs.defpos, this) is Trigger tg
                && cx._Ob(trs.target) is Table tb)
            {
                Add(tb);
                _trig = tg;
            ...
            else throw new PEException("PE66711");
        }
        static (CTree<long,long>,CTree<long,long>) _Map(Domain dm,Trigger tg)
        {
