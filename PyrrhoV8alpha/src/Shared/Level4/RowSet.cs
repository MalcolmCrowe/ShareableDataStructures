...
namespace Pyrrho.Level4
{
    ...
    internal abstract class RowSet : Domain
    {
        ...
        internal const long
            ...
            SIMap = -214, //CTree<long,long> TableColumn,QlValue
            _Scalar = -206, // bool
        ...
        internal override long ColFor(Context cx, string c, Qlx kt)
        {
            if (cx.names[c].Item2 is long q) return q;
        ...
        protected BTree<long, object> _Depths(Context cx, BTree<long, object> m, int d, long p, object o)
        {
            switch (p)
            {
                ...
                case InstanceRowSet.SRowType: m += (_Depth, cx._DepthTil((CTree<int,long>)o, d)); break;
        ...
        internal override (DBObject?, Ident?) _Lookup(long lp, Context cx, Ident ic, Ident? n, DBObject? r)
        {
            ...
            for (var b = dm.rowType.First(); b != null; b = b.Next())
                if (b.value() is long p && dm.representation[p] is DBObject co && n is not null)
                {
                    ...
                    if (co is Table ut && ut.infos[cx.role.defpos] is ObInfo ui && n is not null
                        && cx._Ob(ui.names[n.ident].Item2) is DBObject so)
                    {
                        ...
                        case Assig:
                            {
                                ...
                                {
                                    var ua = b.key();
                                    if ((!Knows(cx, ua.vbl, false))
                                        || !((QlValue?)cx.obs[ua.val])?.KnownBy(cx, this, true) == true)
                                        sg -= b.key();
                                    else if (cx.obs[ua.vbl] is QlInstance sc)
                                    {
                                        im += (sc.defpos, sc.sPos);
                                        sm += (sc.sPos, sc.defpos);
                                        m += (SIMap, sm);
                                        m += (ISMap, im);
                                    }
                                }
            ...
            for (var b = Sources(cx).First(); b != null; b = b.Next())
                if (cx.obs[b.key()] is RowSet s)
                {
                        ...
                        && cx.obs[rsu.usingTableRowSet] is TableRowSet uts)
                    {
                        ...
                        var gl = CList<long>.Empty;
                        for (var c = groupCols.First(); c != null; c = c.Next())
                            gl += c.value();
                        rsu = rsu + (RowType, rt) + (Representation, rp) + (Display, sa.display)
                            + (cx, GroupCols, cx.GroupCols(gl, rsu)) + (Aggs, rrs.aggs);
                        cx.Add(rsu);
                        continue;
                    }
                    var rc = CTree<int,long>.Empty;
        ...
        internal override BTree<long, TargetActivation> Update(Context cx, RowSet fm, 
            PIndex.ConstraintType fl = PIndex.ConstraintType.NoType)
        {
            if (rsTargets == CTree<long, long>.Empty)
                throw new DBException("42174");
            var r = base.Update(cx, fm, fl);
            for (var b = rsTargets.First(); b != null; b = b.Next())
                if (b.value() is long p && cx.obs[p] is Table tb)
                    r += tb.Update(cx, fm, fl);
            return r;
        }
            ...
            PIndex.ConstraintType fl = PIndex.ConstraintType.NoType)
        {
            if (rsTargets == CTree<long, long>.Empty)
                throw new DBException("42174");
            var r = base.Delete(cx, fm, fl);
            for (var b = rsTargets.First(); b != null; b = b.Next())
                if (b.value() is long p && cx.obs[p] is Table tb)
                    r += tb.Delete(cx, fm, fl);
            return r;
        }
        ...
        {
            if (cx.obs[rp] is QlInstance sc && Knows(cx, sc.sPos, ambient))
                return true;
            if (rp == defpos)
                return true;
        ...
        internal CList<long> _Info(Context cx, Grouping g, CList<long> gs)
        {
            gs += g.defpos;
        ...
        internal override void Show(StringBuilder sb)
        {
            ...
            if (groupCols != null && groupCols.rowType != CTree<int,long>.Empty)
            {
                sb.Append(" GroupCols(");
                cm = "";
            ...
            var ro = (CTree<long, long>)(mem[UsingOperands] ??CTree<long,long>.Empty);
    ...
    : base(dm, rw.values)
        ...
        {
            if (cx._Ob(_rowsetpos) is Table nt && Rec()?[0] is TableRow tr)
    ...
    internal class BindingRowSet : RowSet
    {
        internal const long
            ...
            ColTypes = -251; // CTree<long,CTree<Domain,int>?> QlValue 
        internal long builder => (long)(mem[Builder] ?? -1L);
        internal MTree? mt => (MTree?)mem[Level3.Index.Tree];
        internal CTree<long, CTree<Domain, int>> cts =>
            (CTree<long, CTree<Domain,int>>)(mem[ColTypes] ?? CTree<long, CTree<Domain, int>>.Empty);
        ...
        /// existing binding overlapping with the new one.
        /// </summary>
        /// <param name="sr"></param>
        /// <param name="x"></param>
        /// <returns></returns>
        public static BindingRowSet operator +(BindingRowSet sr, (Context, TRow) x)
        {
            ...
            {
                if (b.value() is long p && r[p] is TypedValue tv)
                {
                    k += tv;
                    if (sr.representation[p]?.defpos < 0) // create union types for values
                    {
                        var dp = tv.dataType;
                        if (sr.cts[p] is CTree<Domain, int> ti)
                        {
                            if (!ti.Contains(dp))
                                sr += (ColTypes, sr.cts + (p, ti + (dp, (int)ti.Count)));
                        }
                        else
                            sr += (ColTypes, sr.cts + (p, new CTree<Domain, int>(dp, 0)));
                    }
                }
            }
        ...
        internal static BindingRowSet Get(Context cx, TRow rw)
        {
            if (cx.result is BindingRowSet bs)
            {
                ...
                for (var b = rw.dataType.representation.First(); b != null; b = b.Next())
                {
                    ...
                    {
                        rt += ((int)rt.Count,b.key());
                        rs += (b.key(), b.value());
                    }
                }
                if (rt.Count == bs.Length)
                    return bs;
                var nd = (rt.Count == 0L) ? TableType : new Domain(cx.GetUid(), cx, Qlx.TABLE, rs, rt);
                ...
                return (BindingRowSet)cx.Add(new BindingRowSet(cx, nd.defpos, nd)
                    + (_Rows, rws));
            }
      ...
      //          e._Obey(cx);
        ...
        public class Bindings : Cursor
        {
            ...
            {
                return (rs.mt?.First() is MTreeBookmark mb && rs.rows[(int)(mb.Value() ?? -1L)] is TRow tr) ?
                    new Bindings(cx, rs, mb, tr) : null;
            }
    ...
    internal class TrivialRowSet : RowSet
    {
        ...
        protected override Cursor? _First(Context cx)
        {
            if (rowType.Count == 1L && cx.obs[rowType.First()?.value() ?? -1L] is SqlCall sc
    ...
    internal class SelectRowSet : RowSet
    {
        ...
        internal override DBObject _Replace(Context cx, DBObject so, DBObject sv)
        {
            ...
            var no = so.alias ?? ((so as QlValue)?.name) ?? so.name;
            var nv = sv.alias ?? ((sv as QlValue)?.name) ?? sv.name;
        ...
        internal override RowSet ApplySR(BTree<long, object> mm, Context cx)
        {
            ...
            cx.done = ObTree.Empty;
            var m = mem;
            if (mm[GroupCols] is Domain gc && gc.Length > 0 && mm[Groupings] is CList<long> lg
                && mm[Group] is long gs)
            {
                m += (GroupCols, gc);
                m += (Groupings, lg);
        ...
        internal BTree<long, BTree<long, object>> SplitAggs(BTree<long, object> mm, Context cx)
        {
            ...
            for (var b = ((Domain?)mm[GroupCols])?.representation?.First(); b != null; b = b.Next())
                if (cx.obs[b.key()] is QlValue g) {// might be an expression
                    ...
                    {
                        var u = c.key();
                        if (cx._Ob(u) is QlValue ue && ue.isConstant(cx))
                            continue;
                        if (ub.Contains(u) || !ua.Contains(u)) goto no;
                        if (sa < 0)
                            sa = ua[u];
            ...
            for (var b = ss.First(); b != null; b = b.Next())
            {
                ...
                if (sp == null)
                    continue;
                var nc = s.groupCols?.rowType ?? CTree<int,long>.Empty;
                ...
                if (nc != CTree<int, long>.Empty)
                {
                    var nd = (nc==CTree<int,long>.Empty) ? Row : new Domain(cx.GetUid(), cx, Qlx.ROW, ns, nc);
        ...
        internal override RowSet Build(Context cx)
        {
            ...
            if (ags != CTree<long, bool>.Empty && cx.obs[source] is RowSet sce)
            {
                ...
                if (display == 1 && cx.obs[rowType[0]] is SqlFunction sf
    ...
    {
        internal const long
            SRowType = -215; // CTree<int,long>   TableColumn
        ...
        static BTree<long, object> _Mem1(Context cx, long ap, BTree<long, object> m)
        {
            ...
            var rs = (CTree<long, Domain>)(m[Representation] ?? CTree<long, Domain>.Empty);
            var ism =CTree<long,long>.Empty; // QlValue, TableColumn
            var sim =CTree<long,long>.Empty; // TableColumn,QlValue
            var sr = (CTree<int,long>?)m[SRowType] ?? CTree<int,long>.Empty;
            var ds = (int)(m[Display] ?? 0);
            ...
            if (nr == CTree<long, Domain>.Empty)
                nr = rs;
            return m + (ISMap, ism) + (SIMap, sim) + (ObInfo._Names, ns) + (Representation, nr)
                + (RowType, nt+nd) + (Display, ds);
        }
        ...
        internal override DBObject _Replace(Context cx, DBObject so, DBObject sv)
        {
            ...
            var srt = cx.ReplacedTil(r.sRowType);
            if (srt != r.sRowType)
                r += (cx, SRowType, srt);
            return r;
        }
    ...
    {
        internal const long
            SingleNode = -201, // long TableRow
            _Index = -410; // long Index
        public long index => (long)(mem[_Index] ?? -1L);
        public long singleNode => (long)(mem[SingleNode]??-1L);
        /// <summary>
        ...
        { }
        static BTree<long,object> _Mem(long dp, Context cx, long t, BTree<long,object>? m,long ap)
        {
            m ??= BTree<long, object>.Empty;
        ...
        internal override CList<long> SourceProps => CList<long>.Empty;
        internal override RowSet Apply(BTree<long,object> mm, Context cx, BTree<long,object>? m = null)
        {
            if (mm == BTree<long, object>.Empty)
                return this;
            ...
            if (mm[_Where] is CTree<long, bool> w)
            {
                for (var b = w.First(); b != null; b = b.Next())
                {
                    ...
                    {
                        if (le is SqlFunction sf && sf.op == Qlx.REF && ri.isConstant(cx)
                            && !imm.Contains(ri.defpos))
                            mm += (SingleNode, ri.Eval(cx).ToLong()??0L);
                        else if (ri is SqlFunction ss && ss.op == Qlx.REF && le.isConstant(cx)
            ...
            if (mm[SIMap] is CTree<long,long>)
            {
                ...
                for (var b = tb.indexes.First(); b != null; b = b.Next())
                {
                    var bs = BList<DBObject>.Empty;
                    for (var c = b.key().First(); c != null; c = c.Next())
                        if (c.value() is long p && cx._Ob(sIMap[p]) is QlValue tp)
                            bs += tp;
                    var nk = (bs.Length == 0) ? Row : (Domain)cx.Add(new Domain(-1L, cx, Qlx.ROW, bs, bs.Length));
                    if (pk == Row)
                    {
                        for (var c = b.value().First(); c != null; c = c.Next())
                            if (cx._Ob(c.key()) is Level3.Index x)
            ...
            if (mm[_Matches] is CTree<long,TypedValue> ma && rowOrder.Length==0)
            {
                ...
                index ??= trs.BestForOrdSpec(cx, (Domain)(m[OrdSpec]??Row));
                           ...
                           && x.tabledefpos == target)
                    {
                        ...
                        for (var b = x.keys.rowType.Last(); b != null; b = b.Previous())
                        {
                            ...
                            for (var fd = filter.First(); fd != null; fd = fd.Next())
                                if (cx.obs[fd.key()] is QlInstance co && co.sPos==d)
                                {
                                    sc += 9 - j;
                                    nm++;
                                    pr += fd.value();
        ...
        internal override bool Knows(Context cx, long rp, bool ambient=false)
        {
            if (cx.obs[rp] is SqlFunction sf && 
                (sf.op==Qlx.REF || sf.op==Qlx.SECURITY || sf.op == Qlx.SPECIFICTYPE))
                    return true;
            ...
            if (cx.obs[from] is SelectRowSet ss && cx.obs[ss.valueSelect] is QlValue svs && cx.obs[svs.from] is RowSet es)
                for (var b = es.Sources(cx).First(); b != null; b = b.Next())
                    if ((cx.obs[b.key()] as Domain)?.representation.Contains(rp)==true)
                        return true;
            if (cx.obs[rp]?.from is long fp && cx.obs[fp] is QlInstance sp && sp.from == defpos)
                return true;
            return base.Knows(cx, rp, ambient);
        }
        internal override DBObject _Replace(Context cx, DBObject so, DBObject sv)
        {
            ...
            if (sv is QlInstance sc && so is TableColumn tc && sc.sPos==tc.defpos)
            {
                var rt = CTree<int,long>.Empty;
                var tr =CTree<long,long>.Empty;
                for (var b = rowType.First(); b != null; b = b.Next()) // NB old rowType
                    if (b.value() is long p)
                    {
                        if (p == tc.defpos)
                        {
                            rt += ((int)rt.Count, sc.defpos);
                            tr += (tc.defpos, sc.defpos);
                        }
                        else
                        {
                            rt += ((int)rt.Count, p);
                ...
                var pk = Row;
                var tb = cx._Ob(r.target) as Table ?? throw new PEException("PE30801");
                for (var b = tb.indexes.First(); b != null; b = b.Next())
                {
                    var bs = BList<DBObject>.Empty;
                    for (var c = b.key().First(); c != null; c = c.Next())
                ...
                if (b.value() is long p)
                {
                    ...
                    if (v == TNull.Value)
                        for (var c = matching[p]?.First(); v == TNull.Value && c != null; c = c.Next())
                            for (var d = cx.cursors.First(); v == TNull.Value && d != null; d = d.Next())
                                if (d.value()[c.key()] is TypedValue tv && tv != TNull.Value)
                                    v = tv;
                    if (v == TNull.Value)
                        return TableCursor.New(cx, this, null);
                    key += v;
                }
            return TableCursor.New(cx, this, key);
        }
        protected override Cursor? _First(Context cx)
        {
            throw new NotImplementedException();
        ...
        {
            return new BTree<long, TargetActivation>(target,
                new TableActivation(cx, this, fm, PTrigger.TrigType.Update, null, fl));
        }
        /// <summary>
        ...
        internal class TableCursor : Cursor
        {
            ...
            protected static TRow _Row(Context cx,TableRowSet trs, TableRow rw)
            {
            ...
            {
                var table = cx._Ob(trs.target) as Table;
...
//#endif
        ...
        internal class SingleNodeCursor : TableCursor
        {
            ...
            internal static SingleNodeCursor? New(Context cx,TableRowSet trs)
            {
                if (cx._Ob(trs.target) is not Table table
                    || table.tableRows[trs.singleNode] is not TableRow rec)
                    return null;
                return new(cx, trs, table, 0, rec, null);
            }
            protected override Cursor? _Next(Context cx)
            {
                return null;
    ...
    internal class EmptyRowSet : RowSet
    {
        ...
        internal EmptyRowSet(long dp, Context cx,Domain dm,CTree<int,long>?us=null,
        ...
        static BTree<long,object> _Mem(Context cx,Domain dm,CTree<int, long>? us,
    ...
    internal class SqlRowSet : RowSet
    {
        ...
        internal class SqlCursor : Cursor
        {
            ...
            static TRow _Row(Context cx, SqlRowSet rs, int p)
            {
                ...
                if (n < ((rs.display==0)?rs.Length:rs.display))
        ...
        static BTree<long,object> _Mem(Context cx,Domain dt,BList<(long,TRow)>r)
        {
            ...
            if (r.Length == 0)
            {
                ...
                var x = new Level3.Index(cx.GetUid(), 
                    new BTree<long, object>(Level3.Index.Keys, dt) + (Level3.Index.Tree,t));
                cx.Add(x);
    ...
    internal class ProcRowSet : RowSet
    {
        ...
        internal ProcRowSet(SqlCall ca, long ap, Context cx) 
            :base(ap,cx.GetUid(),cx,
                 _Mem(cx,ca,cx._Ob(ca.procdefpos) as Procedure
        ...
        static BTree<long, object> _Mem(Context cx, SqlCall? ca, Procedure pr)
        {
            ...
            var r = BTree<long, object>.Empty
                + (RowType, rt) + (Representation, rs) + (Display, (int)rt.Count)
                + (InstanceRowSet.SRowType, sr)
                + (Target, pr.defpos) + (ObInfo._Names, ns) + (ObInfo.Name, tn.ident)
                + (ObInfo.Defs,ds)
    ...
    internal class TransitionRowSet : RowSet
    {
            ...
            TableRowSet ts, RowSet data)
        {
            ...
            if (t == null || t.Denied(cx, Grant.Privilege.Insert))
                throw new DBException("42105").Add(Qlx.INSERT, new TChar(t?.name ?? "??"));
        ...
        internal class TargetCursor : Cursor
        {
                      ...
                      new TRow(td, vals))
            {
                ...
                for (var b = ((Table)_td).tableChecks.First(); b != null; b = b.Next())
                    if (b.key() is long cp && ta._Ob(cp) is Check ck)
                    {
                ...
                bool check)
            {
                                ...
                                else if (cx.conn.refIdsToPos)
                                {
                                    if (cx.table.infos[cx.role.defpos] is ObInfo ti
                                       && tc.NameFor(cx) is string cn)
                                        for (var c = (ti.metadata[Qlx.REFERENCES] as TSet)?.First();
                                               c != null; c = c.Next())
                                        {
                ...
                return new TargetCursor(cx, trc, (Cursor?)fb, t, vs);
            }
            static (CList<long>,CTree<long,Domain>) 
                ColsFrom(Context cx,Table tb,CList<long> rt,CTree<long,Domain> rs)
            {
                cx.Add(tb.framing);
                ...
                for (var b = tb.First(); b != null; b = b.Next())
                    if (b.value() is long p && cx._Ob(p) is TableColumn tc)
                    {
                        rt += tc.defpos;
                ...
                CTree<long, TypedValue> vs)
            {
                if (cx is TableActivation ta)
                {
                    ...
                    // This works cleverly for multi-column indexes. 
                    if (tb.Top().FindPrimaryIndex(ta) is Level3.Index ix)
                    {
                        ...
                        for (var b = ix.keys.First(); b != null; b = b.Next())
                                ...
                                && vs[tc.defpos] is TypedValue v)
                            {
                                if (v == TNull.Value)
                                {
                                    if (ix.rows is null || ix.rows.Count == 0)
                                        v = tc.domain.kind switch
                                        {
                                            Qlx.CHAR => new TChar("1"),
                                            Qlx.INTEGER => new TInt(1),
                                            _ => tc.domain.defaultValue
                                        };
                                    else
...
;                return new TriggerCursor(ta, _tgc+(ta.next,tp,v));
    ...
    internal class TransitionTableRowSet : RowSet
    {
        ...
        internal CTree<long, long> map =>
            (CTree<long, long>)(mem[_Map]??CTree<long,long>.Empty);
        internal CTree<long,long> rmap =>
            (CTree<long, long>)(mem[RMap]??CTree<long, long>.Empty);
            ...
            if ((!old) && cx.newTables.Contains(trs.defpos)
                ...
                for (var b = trs.First(cx); b != null; b = b.Next(cx))
                    for (var c = b.Rec()?.First(); c is not null; c = c.Next())
                        if (cx._Ob(c.value().tabledefpos) is Table tb)
                        {
                            dat += (tb.defpos, c.value());
                            for (var d = tb.super.First(); d != null; d = d.Next())
        ...
        public static TransitionTableRowSet operator +(TransitionTableRowSet rs, (Context, long, object) x)
        {
            ...
            switch(p)
            {
                ...
                case _Map:
                case RMap: m += (_Depth, cx._DepthTVV((CTree<long, long>)o, d)); break;
    ...
    internal class RowSetSection : RowSet
    {
        ...
        static BTree<long,object> _Mem(Context cx,RowSet r)
        {
            ...
            if (r.names != Names.Empty)
                m += (ObInfo._Names, r.names);
            m += (_Source, r.defpos);
        ...
        static BTree<long, object> _Mem(Context cx, SqlRowArray d)
        {
            ...
            if (d != null)
                for (int i = 0; i < d.rows.Count; i++)
                    if (cx.obs[d.rows[i]] is QlValue v)
                        vs += v;
    ...
    internal class RestRowSet : InstanceRowSet
    {
        internal const long
            ...
            RemoteCols = -373, // CList<long> QlValue
            RestValue = -457,   // TList
            SqlAgent = -256; // string
        ...
        { }
        static BTree<long, object> _Mem(Context cx, long lp, RestView vw, Domain q)
        {
            ...
            var r = fd.mem + (ObInfo.Name, vw.NameFor(cx))
                   + (RestView.NamesMap, nm) + (ObInfo._Names,mn)
                   + (RestView.UsingTable, vw.usingTable)
                   + (Matching, mg) + (RemoteCols, vd) + (ObInfo._Names,mn)
                   + (RSTargets, new CTree<long,long>(vw.defpos, lp));
            ...
            string? url = null)
        {
            ...
            else
            {
                ...
                if (url == null || url==" Null")
                    return (null, null);
                ss = url.Split('/');
                if (ss.Length <= 5) // url might have been clobbered: try again
                {
                    url = vi?.metadata[Qlx.DESC]?.ToString() ?? cx.url;
                    if (url == null || url==" Null")
                        return (null, null);
        ...
        internal override RowSet Apply(BTree<long, object> mm, Context cx, BTree<long, object>? m = null)
        {
            ...
            {
                var nc = CTree<int,long>.Empty; // start again with the aggregating rowType, follow any ordering given
                ...
                for (var b = rowType.First(); b != null; b = b.Next())
                    if (b.value() is long p && cx.obs[p] is QlValue v)
                    {
                        ...
                        for (var c = es.First(); c != null; c = c.Next())
                            if (v._MatchExpr(cx, c.value(), this) && v.name is not null)
                                cx.Replace(c.value(), 
                                    new QlInstance(new Ident(v.name,cx.GetUid()), cx, defpos, v));
        ...
        CList<long> KnownCols(Context cx, CList<long> ls)
        {
            ...
            for (var b = (ls.First()); b != null; b = b.Next())
                if (b.value() is long p && cx.obs[p] is Grouping g)
                    for (var c = g.members.First(); c != null; c = c.Next())
                        if (cx._Ob(c.key()) is QlValue v
                                && v.KnownBy(cx, this))
                            r += p;
            return r;
        }
        ...
        RestRowSet RoundTrip(Context cx)
        {
            ...
            else
            {
                ...
                for (var b = rowType.First(); b != null; b = b.Next())
                    ...
                    {
                        if (hasAggs && (s is QlInstance || s is SqlReview) 
                            && !s.Grouped(cx, this))
                            continue;
                ...
                if (groupCols.rowType!=CTree<int,long>.Empty)
                {
        ...
        //           Console.WriteLine(sr);
                TypedValue? a = null;
                var or = cx.result;
                cx.result = cx.obs[target] as RowSet; // sneaky
                if (sr != null)
    ...
    internal class RestRowSetUsing : InstanceRowSet
    {
        internal const long
            ...
            UsingCols = -259,// CList<long> QlValue
        ...
        public RestRowSetUsing(long lp,Context cx,RestView vw, RestRowSet rs,
            TableRowSet uf)  :base(lp,_Mem(cx,rs,uf) 
        ...
        static BTree<long,object> _Mem(Context cx,RestRowSet rs, TableRowSet uf)
        {
            ...
            var uc = CList<long>.Empty;//uf.rowType - ab.key();
            for (var b = uf.First(); b != null; b = b.Next())
                uc += b.value();
            return r + (UsingCols,uc)+(UrlCol,ul) + (RowType,rs.rowType) 
                + (Representation,rs.representation) + (_Depth,rs.depth+1)
                + (ObInfo._Names,rs.names+uf.names);
        }
                                ...
                                if (rc?.tabledefpos == lrs.targetTable)
                                    for (var c = lrs.rowType.First(); c != null; c = c.Next())
                                        if (c.value() is long cp)
                                            switch (c.key())
                                            {
                                                case 0: vs += (cp, new TRef(p,cx._Ob(lrs.targetTable) as Domain??Ref)); break;
                                                case 1: vs += (cp, new TChar("Insert")); break;
                                                case 2: vs += (cp, new TInt(rc.defpos)); break;
                                                case 3: vs += (cp, new TInt(rc.trans)); break;
                                                case 4:
                        ...
                        case Physical.Type.Update1:
                            {
                                ...
                                for (var c = lrs.rowType.First(); c != null; c = c.Next())
                                    if (c.value() is long cp)
                                        vs += c.key() switch
                                        {
                                            0 => (cp, new TRef(p,cx._Ob(lrs.targetTable) as Domain??Ref)),
                                            1 => (cp, new TChar("Update")),
                                            2 => (cp, new TInt(rc.defpos)),
                                            3 => (cp, new TInt(rc.trans)),
                ...
                (Physical?, long) nx, PTransaction? trans = null)
            {
                ...
                for (; nx.Item1 != null;)
                    ...
                    {                    // may be an Update 
                        ...
                        {
                            vs += (p1, new TRef(rc.ppos,cx._Ob(rc.tabledefpos) as Domain??Ref));
                            b = b.Next();
                        }
