...
namespace Pyrrho.Level4
{
    ...
    internal class SystemRowSet : TableRowSet
    {
        ...
        static void LogResults()
        {
            ...
            t+=new SystemTableColumn(t, "Pos", Int,1);
        ...
        internal class LogCheckBookmark : LogSystemBookmark
        {
            ...
            /// <summary>
            /// the current value: (Pos,Int,Name,CheckFields,Transaction)
            /// </summary>
            static TRow _Value(SystemRowSet res, Physical ph)
            {
                PCheck c = (PCheck)ph;
       ...
       //     t+=new SystemTableColumn(t, "Adapter", Int,0);
        ...
        internal class LogIndexBookmark : LogSystemBookmark
        {
            ...
            /// <summary>
            /// the current value: (Pos,Name,Table,Transaction)
            /// </summary>
            static TRow _Value(SystemRowSet res, Physical ph)
            {
                    PIndex p = (PIndex)ph;
    ...
    //        t += new SystemTableColumn(t, "Graph", Char, 0);
        ...
        static void SysRoleResults()
        {
            ...
            t+=new SystemTableColumn(t, "Pos", Int,0);
        ...
        static void SysAuditKeyResults()
        {
            ...
            t+=new SystemTableColumn(t, "Col", Int,0);
        ...
        static void SysClassificationResults()
        {
            ...
            t+=new SystemTableColumn(t, "Type", Char,0);
            t+=new SystemTableColumn(t, "Classification", Char,0);
            t+=new SystemTableColumn(t, "LastTransaction", Int,0);
            t.AddIndex("Pos");
            t.Add();
        }
        /// <summary>
        ...
        static void SysClassifiedColumnDataResults()
        {
            ...
            t+=new SystemTableColumn(t, "Pos", Int,1);
            t+=new SystemTableColumn(t, "Col", Int,1);
            t+=new SystemTableColumn(t, "Classification", Char,0);
            t+=new SystemTableColumn(t, "LastTransaction", Int,0);
            t.AddIndex("Pos", "Col");
            t.Add();
 ...
 /*       static void SysGraphResults()
        {
            ...
            t += new SystemTableColumn(t, "Uid", Int, 1);
        ...
        internal class RoleColumnBookmark : SystemBookmark
        {
            ...
            static TRow _Value(Context cx, SystemRowSet rs, object ta, int i,long p,Domain d)
            {
                ...
                return new TRow(rs,
                    ...
                    new TChar(si.name??""),
                    new TInt(i),
                    (tc.domain is StandardDataType)?new TChar(tc.domain.ToString()):Pos(tc.domain.defpos),
                    new TChar(d.defaultString??""),
                    TBool.For(!tc.optional),
                    new TChar(DBObject.Uid(tc.colDefault)));
            }
            /// <summary>
       ...
       //     t+=new SystemTableColumn(t, "Adapter", Char,0);
        ...
        internal class RoleGraphCatalogBookmark(Context cx, SystemRowSet r, int pos, ABookmark<string, long> bmk, DBObject ob) 
            : SystemBookmark(cx, r, pos, ob.defpos,ob.defpos,_Value(r,bmk,ob))
        {
            ...
            internal static RoleGraphCatalogBookmark? New(Context cx, SystemRowSet res)
            {
                for (var b = cx.db.catalog.First(); b != null; b = b.Next())
                    if (cx._Ob(b.value()) is DBObject c)
                    {
        ...
        internal class RoleGraphEdgeTypeBookmark(Context cx, SystemRowSet r, int pos,
            ABookmark<long, object> obmk, ABookmark<long, bool> bmk, DBObject g, Table e) : SystemBookmark(cx, r, pos, e.defpos, e.defpos, _Value(r,g,e))
        {
            ...
            /// </summary>
            static TRow _Value(SystemRowSet rs,DBObject g, Table e)
            {
                return new TRow(rs,
        ...
        internal class RoleGraphInfoBookmark : SystemBookmark
        {
            ...
            internal static RoleGraphInfoBookmark? New(Context cx, SystemRowSet res)
            {
                ...
                var ls = new TSet(Char); // labels
                var ll = new TSet(Content); // label sets
                for (var b = ro.dbobjects.First(); b != null; b = b.Next())
                    if (b.value() is long p && p >= 0 && cx._Ob(p) is Table t && t.infos[t.definer] is ObInfo ti)
                    {
                        if (ti.metadata[Qlx.LABELS] is TSet ts)
                            ls += ts;
                        ll += ls;
                        ps += t.representation;
                        if (t is Table)
                        {
                            ...
                            {
                                var tr = c.value();
                                if (cx._Ob(tr.tabledefpos) is Table tb)
                                {
                                    st += (t.defpos, true);
                                    te++;
                        ...
                        else
                        {
                            ...
                            for (var c = t.tableRows.First(); c != null; c = c.Next())
                            {
                                ...
                                if (cx._Ob(tr.tabledefpos) is Table tb)
                                {
                                    ...
                                    else for (var d = tb.super.First();d!=null;d=d.Next())
                                    if (d.key().defpos==t.defpos)
                                    {
                                        st += (t.defpos, true);
                                        tn++;
                ...
                v += ("labels", (int)ls.tree.Count);
                v += ("label-sets", (int)ll.tree.Count);
        ...
        internal class RoleGraphLabelBookmark(Context cx, SystemRowSet r, int pos,
            ABookmark<long, object> obmk, ABookmark<long, bool> mbmk, IBookmark<TypedValue> bmk,
            DBObject g, Table nt, TypedValue lb) : SystemBookmark(cx, r, pos, nt.defpos, nt.defpos, _Value(r, g, nt, lb))
        {
            readonly ABookmark<long, object> _obmk = obmk;
            readonly DBObject _g = g;
            readonly ABookmark<long,bool> _mbmk = mbmk;
            readonly Table _nt = nt;
            readonly IBookmark<TypedValue> _bmk = bmk;
                    ...
                    if (b.value() is GraphType g)
                        for (var c = g.graphTypes.First(); c != null; c = c.Next())
                        {
                            if (cx._Ob(c.key()) is Table e && e.infos[e.definer]?.metadata[Qlx.LABELS] is TSet ls)
                                ...
                                {
                                    var rb = new RoleGraphLabelBookmark(cx, res, 0, b, c, d, g, e, d.Value());
                                    if (rb.Match(res) && Eval(res.where, cx))
                                        return rb;
                                }
                        }
            ...
            protected override Cursor? _Next(Context cx)
            {
                ...
                for (; obmk != null;)
                {
                    for (var c = bmk?.Next(); c != null; c = c.Next())
                        if (mbmk!=null)
                        {
                            var rb = new RoleGraphLabelBookmark(cx, res, _pos + 1, obmk, mbmk, c, g, nt, c.Value());
                            if (rb.Match(res) && Eval(res.where, cx))
                                return rb;
                        }
                    for (mbmk = mbmk?.Next(); mbmk != null; mbmk = mbmk.Next())
                        if (cx._Ob(mbmk.key()) is Table n && n.infos[n.definer] is ObInfo ni)
                        {
                            nt = n;
                            bmk = ni.metadata[Qlx.LABELS].First();
                            goto next;
                        }
                    for (obmk = obmk.Next(); obmk != null; obmk = obmk.Next())
                    {
            ...
            /// the current value: (Pos,Graph,name,Owner)
            /// </summary>
            static TRow _Value(SystemRowSet rs, DBObject g, Table e, TypedValue s)
            {
                return new TRow(rs,
                    new TChar(Uid(e.defpos)),
                    new TChar(Uid(g.defpos)),
        ...
        internal class RoleGraphNodeTypeBookmark(Context cx, SystemRowSet r, int pos,
            ABookmark<long, object> obmk, ABookmark<long, bool> bmk, GraphType g, Table e) : SystemBookmark(cx, r, pos, e.defpos, e.defpos, _Value(r, g, e))
        {
            readonly ABookmark<long, object> _obmk = obmk;
            ...
            /// the current value: (Pos,Graph,name,Owner)
            /// </summary>
            static TRow _Value(SystemRowSet rs, GraphType g, Table e)
            {
                return new TRow(rs,
                    new TChar(Uid(e.defpos)),
                    new TChar(Uid(g.defpos)),
        ...
        internal class RoleGraphPropertyBookmark(Context cx, SystemRowSet r, int pos,
            ...
            DBObject g, Table nt, TableColumn tc) : SystemBookmark(cx, r, pos, nt.defpos, nt.defpos, _Value(cx, r, g, nt, tc))
        {
            readonly ABookmark<long, object> _obmk = obmk;
            readonly DBObject _g = g;
            readonly ABookmark<long, bool> _mbmk = mbmk;
            ...
            /// the current value: (Pos,Graph,name,Owner)
            /// </summary>
            static TRow _Value(Context cx,SystemRowSet rs, DBObject g, Table e, TableColumn tc)
            {
                return new TRow(rs,
                    new TChar(Uid(e.defpos)),
                    new TChar(Uid(g.defpos)),
       ...
       //     t += new SystemTableColumn(t, "IdName", Char, 0);
        ...
        {
            readonly ABookmark<long, object> _fb;
            RoleEdgeTypeBookmark(Context cx, SystemRowSet r, int pos, ABookmark<long, object> fb, Table nt)
                : base(cx, r, pos, nt.defpos, nt.defpos, _Value(cx, r, nt))
            {
                _fb = fb;
            }
