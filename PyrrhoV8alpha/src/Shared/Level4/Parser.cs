...
namespace Pyrrho.Level4
{
    ...
    {
        internal const long
            QueryList = -318; // bool   do not restore names at commas
        /// <summary>
        ...
        /// <param name="xp">the expected valueType type (default is Domain.Content)</param>
     ...
     //           return ParseJSONCall(sql);
            ...
            {
                e = ParseStatements(BTree<long,object>.Empty);
        ...
        Database ParseJSONCall(string jd)
        {
            ...
            {
                var (pn, pr) = bm.value();
                if (pr is TDocument pd && cx.role.procedures[pn] is CTree<CList<Domain>, long> bp)
                {
                    for (var b = bp.First(); b != null; b = b.Next())
                        if (cx._Ob(b.value()) is Procedure proc)
                        {
        ...
        public Database ParseQl(string ql)
        {
            ...
            do
            {
                var e = ParseStatement(BTree<long, object>.Empty);
          /*      if (cx.parse==ExecuteStatus.Obey)
                {
        ...
        public Executable ParseStatement(BTree<long, object> m)
        {
            ...
            if (StartStatement())
                switch (tok)
                {
                    case Qlx.ALTER: ParseAlter(); return Executable.Empty;
                    case Qlx.AT:
                        {
                            Next();
                    ...
                    case Qlx.BINDING:
                        {
                            Next();
                            ParseBindingVariableDefinitions(m);
                            return ParseStatement(BTree<long, object>.Empty);
                        }
                    ...
                    case Qlx.DELETE: return ParseSqlDelete(m);
                    case Qlx.DETACH: goto case Qlx.DELETE;
                    case Qlx.DROP: ParseDropStatement(); return Executable.Empty; // some GQL TBD
                    case Qlx.EOF: return Executable.Empty;
                    ...
                    case Qlx.FINISH: return ParseReturn(m);
                    case Qlx.FOR: return ParseForStatement(m);
                    ...
                    case Qlx.INSERT: return ParseSqlInsert(m);
                    ...
                    case Qlx.ORDER:
                        {
                            if (m.Contains(QlValue.Left)) // Leave ORDER for ParseCompositeQueryStatement
                                return EmptyStatement.Empty;
                            return ParseOrderAndPage(m);
                        }
                    ...
                    case Qlx.RETURN:
                        ...
                        if (m.Contains(QlValue.Left)) // leave RETURN for ParseCompositeQueryStatement
                            return EmptyStatement.Empty;
                    ...
                    case Qlx.TABLE:
                        {
                            ...
                            Mustbe(Qlx.Id);
                            var tb = cx._Ob(cx.role.dbobjects[tn.ident]) as Table
                    ...
                    case Qlx.EXPECT: ParseExpectation(m); return Executable.Empty;
                    case Qlx.UNION: return ParseCompositeQueryStatement(m);
                    case Qlx.UPDATE: (cx, var ue) = ParseSqlUpdate(m); return ue;
                    case Qlx.USE:
                        {
                            Next();
                            var ug = "/";
                    ...
                    case Qlx.WHILE: return ParseSqlWhile(m);
        ...
        internal Expectation ParseExpectation(BTree<long, object> m)
        {
            ...
            Mustbe(Qlx.BECAUSE);
            Next();
            var b = ParseStatement(m);
            var c = ParseSqlValue(m + (DBObject._Domain, Domain.Real));
            if (tok != Qlx.EOF)
                throw new DBException("42000");
        ...
        private Executable ParseSchemaStatement(BTree<long, object> m)
        {
            ...
            return new SchemaStatement(cx.GetUid(),cx,m) { expects = e };
        }
        ...
        { 
            return Match(Qlx.ALTER, Qlx.AT, Qlx.BEGIN, Qlx.BINDING, Qlx.BREAK, Qlx.CALL, 
                ...
                Qlx.SET, Qlx.SIGNAL, Qlx.SKIP, Qlx.TABLE, Qlx.EXPECT,
                Qlx.UNION, Qlx.UPDATE, Qlx.USE, Qlx.VALUE,
        ...
        Executable ParseGrant()
        {
            ...
            if (Match(Qlx.SECURITY))
            {
                ...
                Mustbe(Qlx.Id);
                var usr = cx._Ob(cx.db.roles[nm]) as User
        ...
        internal Role? GetRole(string n)
        {
            ...
            if (p == 0L)
                return null;
            return cx._Ob(p) as Role;
        }
        internal User? GetUser(string n)
        {
            var p = cx.db.users[n];
            if (p == 0L)
                return null;
            return cx._Ob(p) as User;
        }
        /// <summary>
		/// Grantee = 	[USER] id
		/// 	|	ROLE id . 
		...
		DBObject ParseGrantee(BList<PrivNames> priv)
        {
            ...
            {
                Next();
                return (Role?)cx._Ob(Database.Guest) ?? throw new PEException("PE2400");
            }
            if (Match(Qlx.USER))
                Next();
		...
		Executable ParseCallStatement(BTree<long, object> m)
        {
            var lp = lxr.start;
            Next();
        ...
        Executable ParseCreateClause(bool parm = false)
        {
            ...
            if (cx.role is Role dr
                && dr.priv.HasFlag(Grant.Privilege.AdminRole) != true
                && dr.defpos != -502)
            ...
            else if (Match(Qlx.ROLE))
            {
                ...
                ParseViewDefinition(BTree<long,object>.Empty);
            else if (tok == Qlx.OR)
            {
                Next();
                Mustbe(Qlx.REPLACE);
            ...
            for (var b = cx.metaPending.First(); b != null; b = b.Next())
                if (cx._Ob(b.key()) is DBObject tg)
                    for (var c = b.value().First(); c != null; c = c.Next())
        ...
        CTree<Qlx, TypedValue> SpecWord(CTree<Qlx,TypedValue> wds, params Qlx[] tks)
        {
        ...
        /// We also allow alternative node type references and multiplicity metadata.
        ...
        long ParseElementTypeSpec(CTree<string, long> ts)
        {
            var ab = tok;
            Table? r = null;
            var wds = CTree<Qlx, TypedValue>.Empty; // accumulate metadata
            var lp = LexLp();
            var lv = lxr.val;
            ...
            if (tok == Qlx.Id)
            {
                ...
                cx.dnames += id;
                wds += (tok, new TIdent(id));
                Next();
            }
            var tps = BList<(Ident, Domain)>.Empty; // the new node type properties
            ...
            if (tok == Qlx.Id)
            {
                ...
                cx.dnames += al;
                wds += (Qlx.AS, new TIdent(al));
                Next();
            }
            if (id.ident == "")
                id = al;
            Table td = (wds.Contains(Qlx.EDGE) || wds.Contains(Qlx.RELATIONSHIP)) ? Domain.EdgeType : Domain.NodeType;
            td = CreateElementType(id, td, wds);
            var (dm, fa, ps) = ElementDetails(id, td, wds);
...
: Qlx.NODETYPE;
            ...
            if (kind == Qlx.EDGETYPE)
            {
                ...
                if (eg)
                {
                    ...
                    (ms, md) = ParseMetadata(Qlx.EDGETYPE, BTree<long, object>.Empty);
                }
            }
            if (wds.Contains(Qlx.NODE))
            {
                ms += " NODE";
                md += (Qlx.NODETYPE, TBool.True);
            }
            if (StartMetadata(Qlx.TYPE))
            {
                var (s, m) = ParseMetadata(Qlx.TYPE, BTree<long, object>.Empty);
                if (r != null && m != null)
                    cx.Add(new PMetadata(id.ident, -1, r, s, m, nst, cx.db.nextPos, cx));
                ms += s; 
                if (m!=null)
                    md += m;
            }
            (cx, var o) = dm.Add(cx, md);
            dm = (Domain)o;
            return dm.defpos;
        }
        Table CreateElementType(Ident id, Domain dm, CTree<Qlx, TypedValue> wds)
        {
            var existing = cx._Ob(cx.role.dbobjects[id.ident]);
            if (existing is Table ext)
                return ext;
            var un = dm.super ?? CTree<Domain, bool>.Empty; // relevant node types
            NodeType? ut = null;
            if (wds.Contains(Qlx.NODE)) // It is a new node tye
            {
            ...
            else throw new DBException("42000");
            return (Table)cx.Add(ut);
        }
        /// <summary>
        ...
        /// <param name="xp"></param>
        /// <returns></returns>
        (Domain, BList<(Ident, TMetadata)>, BList<(Ident, Domain)>) ElementDetails(Ident id, Table xp, CTree<Qlx,TypedValue> wds)
        {
            ...
            while (Match(Qlx.VBAR))
            {
                ...
                Mustbe(Qlx.Id);
                la += (ia, TMetadata.Empty + ParseMetadata(Qlx.TYPE, BTree<long, object>.Empty).Item2);
            }
            QlValue le = new SqlLiteral(id.uid, new TTypeSpec(xp.name,xp));
            Domain ld = Domain.TypeSpec;
            var ms = "";
            var md = TMetadata.Empty;
            ...
            {
                var (s, tm) = ParseMetadata(Qlx.TABLE, m+(TableColumn._Table, xp.defpos));
                ms += s; md += tm;
            }
            xp = cx._Ob(xp.defpos) as Table ?? throw new DBException("42105");
            (cx,var o) = xp.Add(cx, md);
            xp = (Table)o;
            cx.db += xp;
            return (xp, la, ps);
        }
        ...
        internal CList<long> ParseBindingVariableDefinitions(BTree<long,object> m)
        {
            ...
            while (Match(Qlx.BINDING, Qlx.GRAPH, Qlx.TABLE, Qlx.VALUE))
            {
                ...
                switch (tp)
                {
                    case Qlx.GRAPH:
                        ...
                        Mustbe(Qlx.EQL);
                        ParseSqlValue(m +(DBObject._Domain,Domain.GraphSpec));
                        break;
                    case Qlx.TABLE:
                        if (Match(Qlx.DOUBLECOLON, Qlx.TYPED, Qlx.BINDING, Qlx.TABLE))
                        {
                            ...
                            Mustbe(Qlx.EQL);
                            var gs = ParseSqlValue(m+(DBObject._Domain, Domain.GraphSpec));
                    ...
                    case Qlx.VALUE:
                        {
                            ...
                            Mustbe(Qlx.EQL);
                            var sv = ParseSqlValue(m+(DBObject._Domain,dt));
        ...
        internal Executable ParseInsertGraph()
        {
            if (cx.result is RowSet rs)
                for (var b = rs.First(); b != null; b = b.Next())
                    if (b.value() is long p && cx.NameFor(p) is string s
                        && !cx.names.Contains(s))
                        cx.names += (s, (LexLp(), p));
            var sch = false;
            if (Match(Qlx.SCHEMA))
        ...
        BTree<long, (long, long)> ParseTruncation(BTree<long,object> m)
        {
            ...
            {
                // state M3
                var k = Domain.TypeSpec.defpos;
                Domain? et = null;
                if (tok == Qlx.Id)
                {
                    var ei = lxr.val.ToString();
                    Next();
                    et = cx._Ob(cx.role.dbobjects[ei]) as Domain
                ...
                if (pa)
                    Next();
                var ot = cx.Add(ParseSqlValue(m+(DBObject._Domain,Domain.Char))).defpos;
                if (pa)
                    Mustbe(Qlx.RPAREN);
                // state M7
                Mustbe(Qlx.EQL);
                var lm = cx.Add(ParseSqlValue(m+(DBObject._Domain,Domain.Int))).defpos;
                if (et?.kind == Qlx.UNION)
                    for (var b = et.alts.First(); b != null; b = b.Next())
        ...
        (GqlNode, ObTree) ParseInsertGraphItem(ObTree svg, bool sch = false, GqlNode? ln = null)
        {
            ...
            if (tok == Qlx.Id)
            {
                ...
                else
                {
                    id = ix;
                    bound = true;
                    nd = cx.obs[id] as QlValue ?? throw new PEException("PE80201");
                }
                Next();
            }
            if (!bound)
            ...
            {
                Next();
                while (tok != Qlx.RBRACE && dm is not null)
                {
                    var (n, v) = GetDocItem(lp, dm, sch);
                    if (dm.name is not null && cx._Ob(b.uid) is DBObject bo)
                    {
                        var px = b.uid;
                        var ic = new Ident(n.ident, px, null);
                        var it = new Ident(dm.name, px, null);
     ...
     //       lxr.tgt = Names.Empty;
            ...
            if (Match(Qlx.Id))
            {
                ...
                if (!Match(Qlx.COLON, Qlx.LBRACE, Qlx.RPAREN))
                {
                    ...
                    tok = lxr.PushBack(ot);
                    lxr.val = ov;
                    var be = ParseSqlValue(m + (DBObject._Domain, Domain.TypeSpec));
                    if (be.domain.kind == Qlx.REF && cx._Ob(be.domain.target) is Table tg)
                    {
         ...
         //       cx.defs -= cx.sD-1;
                ...
                dm ??=cx.obs[cx.names[b.ident].Item2]?.domain as Table;
                dm ??= cx.obs[(cx.obs[cx.names[b.ident].Item2]?.domain as TableRowSet)?.target ?? -1L] as Table;
                var ed = cx.db.objects[dm?.defpos??-1L] as Table?? r?.domain as Table;
                // state M31
                if (ln is not null)
                {
                ...
                po = ed?.PostConnect(cx, ba, Domain.TableType, cn)??TNull.Value;
                var le = (ln != null && cx.names[ln.ident].Item2 is long pl && pl >= 0L) ? pl : ln?.uid?? -1L;
                if (r is null)
                {
                    var ml = m;
                    if (pr != TNull.Value)
                        ml += (GqlNode.PreCon, pr);
                    if (po != TNull.Value)
                        ml += (GqlNode.PostCon, po);
           ...
           //         var ld = cx.obs[cx.names[ln?.ident ?? ""].Item2]?.domain;
            ...
            if (r is GqlEdge && an is not null)
            {
                // an may give us an updated version of postCon
                var oo = r.postCon as TConnector ?? new TConnector(ba, "", Domain.TypeSpec);
                if (an.postCon is TConnector ac && ac.rd.defpos>0)
                r = r.Add(cx, an, st, LexLp(), new TConnector(ba, (cn != "") ? cn : oo.cn,
                    (an.domain.EqualOrStrongSubtypeOf(oo.rd)) ? an.domain : Domain.TypeSpec, oo.cp));
            }
            cx.Add(r);
            tgs += r.state;
            svg += r.defpos;
   ...
   //         cx.DecSD();
        ...
        Domain ParseLabelClause(Domain dm, bool create = true, string? a = null, Domain? lt = null, Domain? at = null)
        {
            ...
            if (r.defpos < 0 && create && Match(Qlx.DOUBLEARROW, Qlx.IMPLIES, Qlx.COLON))
            {
                ...
                var nt = (Table)(cx.Add(pt) ?? throw new DBException("42105"));
                r = (Table)cx._Add(nt.Relocate(pt.defpos));
                var ab = tok;
            ...
            Mustbe(Qlx.Id);
            var left = cx.FindTable(c1.ident) ?? (Domain)cx.Add(new GqlLabel(c1, cx, lt, at,
                new BTree<long, object>(Domain.Kind, dm.kind)));
            if ((ab == Qlx.DOUBLEARROW || ab == Qlx.IMPLIES) && a!=null)
            {
                var right = cx.FindTable(a) ?? throw new DBException("42107", a);
                left += (Domain.Under, new CTree<Domain, bool>(right, true));
                var li = left.infos[cx.role.defpos] ?? new ObInfo(c1.ident, Grant.AllPrivileges));
                left += (DBObject.Infos, li + (ObInfo._Names, right.names));
            }
            ...
            cx.Add(left);
            cx.Add(c1.ident, cp, left);
            if (neg)
            {
                left = new GqlLabel(cp, cx, -1L, left.defpos, new BTree<long, object>(Domain.Kind, Qlx.EXCLAMATION));
                cx.Add(left);
            }
            while (Match(Qlx.VBAR, Qlx.COLON, Qlx.AMPERSAND, Qlx.DOUBLEARROW))
            {
                ...
                if (Match(Qlx.COLON, Qlx.DOUBLEARROW))
                    Next();
                var right = ParseLabelClause(Domain.NodeType, create, c1.ident, lt, at);
                cx.Add(right);
                left = new GqlLabel(cp, cx, left.defpos, right.defpos, new BTree<long, object>(Domain.Kind, ab)); // leave name empty for now
                cx.Add(left);
            }
            var ns = left.names;
            if (ns.Count == 0)
        ...
        CTree<Domain,bool> ParseTypeReferenceList(Qlx dl)
        {
            var id = new Ident(this);
            Mustbe(Qlx.Id);
 ...
 /*           var ab = tok;
            ...
            var c1 = new Ident(this);
            Mustbe(Qlx.Id);
            Domain ld = (cx._Ob(cx.role.dbobjects[c1.ident]) as UDType)
                ?? (cx._Ob(cx.role.dbobjects[cx.obs[c1.uid]?.Eval(cx)?.ToString() ?? ""]) as UDType)
                ?? ((dm.defpos == Domain.TypeSpec.defpos) ? (Domain)dm.Relocate(cx.GetUid()) : dm);
            QlValue left = new SqlLiteral(cp, new TTypeSpec(c1.ident, ld));
            /* if (ab == Qlx.DOUBLEARROW || ab == Qlx.IMPLIES)
                 left = new SqlTypeExpr(cp,Qlx.NO,left, null, 
                     new BTree<long, object>(Domain.Kind, ld.kind)
                     +(Domain.Under,new CTree<Domain,bool>(ld,true)));
             else 
            if (ld.kind == Qlx.UNION && (lt is not null || at is not null))
            {
                var un = CTree<Domain, bool>.Empty;
                UDType? ee = null;
            ...
            if (left is SqlTypeExpr && cx.binding[left.defpos] is TTypeSpec sp)
                cx.bindings += (c1.uid, sp._dataType);
            cx.Add(left);
            cx.Add(c1.ident, cp, left);
            if (neg)
            {
                left = new SqlTypeExpr(cx.GetUid(), Qlx.NO, left, null, new BTree<long, object>(Domain.Kind, Qlx.EXCLAMATION));
                cx.Add(left);
            }
            while (Match(Qlx.VBAR, Qlx.COLON, Qlx.AMPERSAND, Qlx.DOUBLEARROW))
            {
                ...
                if (Match(Qlx.COLON, Qlx.DOUBLEARROW))
                    Next();
                var right = ParseNodeLabelExpression(Domain.TypeSpec, c1.ident, lt, at);
                cx.Add(right);
                left = new SqlTypeExpr(cx.GetUid(), Qlx.NO, left, right, new BTree<long, object>(Domain.Kind, ab)); // leave name empty for now
                cx.Add(left);
            }
            var ns = left.names;
            if (ns.Count == 0)
                ns = left.names ?? Names.Empty;
            if (ns.Count > 0)
                left += (ObInfo._Names, ns);
            return left;
        } */
        ...
        /// <returns></returns>
        /// <exception cref="DBException"></exception>
        internal Executable ParseMatchStatement(BTree<long, object> m)
        {
            var olddefs = cx.defs; 
    ...
    /*        if (Match(Qlx.YIELD))
            {
                ...
                cx.result = (RowSet)cx.Add(ers + (DBObject._Domain, yd)
                    + (RowSet.RowOrder, yd));
            } */
            return ms;
        }
        internal static (BindingRowSet, Names) BindingTable(Context cx, long ap, CTree<long, TGParam> gs, CList<long> svgs)
        {
            ...
            var rt = CTree<int,long>.Empty; // was incoming.rowType;
            ...
            if (rt.Count == 0)
            {
                ...
                rt += ((int)rt.Count, rc.defpos);
                re += (rc.defpos, Domain.Bool);
            }
            var nd = ((rt==CTree<int,long>.Empty)?Domain.Row:new Domain(-1L, cx, Qlx.TABLE, re, rt, (int)rt.Count))
        ...
        internal View? ParseViewDefinition(BTree<long,object> m)
        {
            var op = cx.parse;
            var lp = LexDp();
            var ap = LexLp();
            var sl = lxr.start;
            var id = (string?)m[ObInfo.Name];
            if (id == null)
            {
                Next();
                id = lxr.val.ToString();
            ...
            // whose keymap and datatype are recorded in the framing part.
            // For a normal view the keymap are QlInstances that refer to a derived tble
            ...
            else if (Match(Qlx.OF))
            {
                ...
                if (Match(Qlx.LPAREN)) // inline type def (RestView only)
                    dm = (Domain)cx.Add(ParseRowTypeSpec(new BTree<long,object>(Domain.Kind,Qlx.VIEW)));
                else
                {
                    var tn = lxr.val.ToString();
                    Mustbe(Qlx.Id);
            ...
            else
            {
                ...
                {
                    Next();
                    ur = ParseTableReferenceItem(lp, m+(DBObject._Domain, Domain.TableType));
            ...
            {
                var (s,tm) = ParseMetadata(Qlx.VIEW,m);
                if (tm != null)
                    vw = (View)(cx.Add(new PMetadata(id, -1, vw, s,tm, nst, cx.db.nextPos, cx))
                        ??throw new DBException("42105"));
            }
            cx.result = null;
            cx.binding = CTree<long, TypedValue>.Empty;
        ...
        private void ParseCreateOrdering()
        {
            ...
            Mustbe(Qlx.Id);
            Domain ut = ((cx.role.dbobjects[n.ident] is long p) ? cx._Ob(p) as Domain : null)
        ...
        DBObject ParseColRef(DBObject ta)
        {
            if (tok == Qlx.PERIOD)
            {
                ...
                if (cx._Ob(tb.applicationPS) is not PeriodDef pd || pd.NameFor(cx) != pn.ToString())
            ...
            var id = new Ident(this);
            var p = ta.ColFor(cx, id.ident, kt);
            var tc = cx.obs[p] ?? cx._Ob(p) as DBObject ??
            ...
            while (tok == Qlx.DOTTOKEN)
            {
                ...
                if (cx._Ob(ta.ColFor(cx, pa.ident)) is TableColumn cc
        ...
        ///             | (' Member {',' Member }')' .
        /// </summary>
        void ParseTypeClause(BTree<long, object> m)
        {
            var lp = lxr.pos;
            var typename = new Ident(lxr.val.ToString(), cx.GetPos());
            var kind = (Qlx)(m[Domain.Kind] ?? Qlx.TABLE);
            ...
            m = m + (Domain.Kind, Qlx.TYPE) + (DBObject.Definer, cx.role.defpos) + (DBObject._Ident, typename)
                + (DBObject._Domain, dt) + (Domain.Under, supers) + (TableColumn._Table, tb.defpos);
            if (StartMetadata(Qlx.TYPE) || (tok == Qlx.AS && lxr.ch != '/' && lxr.ch != '.')) // catref)
            {
                if (tok == Qlx.AS)
                {
                    Next();
                    if (tok == Qlx.LPAREN)
                    {
                        ...
                        for (; ; )
                        {
                            if (StartMetadata(Qlx.TABLE))
                            {
                                var (ss, mt) = ParseMetadata(Qlx.TABLE, m);
                                si += ss; sm += mt;
                            }
                            else
                ...
                var mv = TMetadata.Empty;
                string st = "";
                var nst = cx.db.nextStmt;
                if (StartMetadata(Qlx.TYPE))
                {
                    while (StartMetadata(Qlx.TYPE))
                    {
                        var (ss, mu) = ParseMetadata(Qlx.TYPE, m);
                        st += ss; mv += mu;
                    }
                    if (mv.md.Count == 0L)
                        mv = TMetadata.Empty;
                    if (mv != TMetadata.Empty)
                        tb = (Table)(cx.Add(new PMetadata(typename.ident, 0L, tb, st, mv, nst, cx.db.nextPos, cx))
                            ?? throw new PEException("PE070751"));
                    sm += mv;
                }
            }
            ...
            else if (cx.role.dbobjects[lxr.val.ToString()] is long up && cx._Ob(up) is UDType tt)
            {
                Next();
                dt = tt;
                if (ParseWhereClause(m + (DBObject._Domain, dt)) is CTree<long, bool> w)
                    m += (RowSet._Where, w);
            }
            else
            {
            ...
            if (cx._Ob(supers.First()?.key().defpos ?? -1L) is UDType ut)
            {
                ...
                tb = (UDType)(cx._Ob(pp) ?? Domain.TypeSpec);
            }
            (cx, var o) = tb.Add(cx, sm);
            tb = (Table)o;
            ...
            {
                cx.obs = ObTree.Empty;
                tb = ParseMethodHeader(tb) ?? throw new PEException("PE42140");
                while (tok == Qlx.COMMA)
                {
                    Next();
                    cx.obs = ObTree.Empty;
                    tb = ParseMethodHeader(tb) ?? throw new PEException("PE42141");
                }
            }
            cx.Add(tb);
            if (cx.parse!=ExecuteStatus.Obey)
                cx.db += (Executable)cx.Add(new Executable(cx, new string(lxr.input, lp, lxr.pos - lp)));
            cx.parse = op;
        }
        void ParseTypeMetadata(Ident typename, Table dt, BTree<long, object> m)
        {
            // ParseTypeMetadata is also used for the CREATE NODE syntax, which is
            // part of the DDL, and so any new properties added here are best prepared with
            // SqlValues/SqlLiterals instead of Domains/TableColumns.
            // We prepare a useful tree of all the keymap we know about in case their names
            ...
            var (s,tm) = ParseMetadata(Qlx.TYPE,m+(TableColumn._Table,dt.defpos));
            (cx,var o) = cx.Add(cx._Ob(dt.defpos) ?? dt, tm);
            dt = (Table)o;
            if (tm.Contains(Qlx.NODETYPE))
            {
                dt = dt.Build(cx, null, 0L, typename.ident, ls);
                var pm = new PMetadata(typename.ident, -1L, dt,
                    new string(lxr.input, st, lxr.start - st), tm, nst, cx.db.nextPos, cx);
                cx.Add(pm);
            }
            var odt = dt;
            if (tm[Qlx.REFERENCES] is TSet ts) //Each entry in ts is a TConnector
            {
                var os = ts;
                var et = new UDType(LexLp(), typename.uid, typename.ident, dt,dt.mem,cx);
                // watch for an edge subtype
                TSet? sm = null;
                for (var b=dt.super.First();b!=null;b=b.Next())
                    if (b.key() is UDType be && be.colRefs.Count > 1L 
                        && be.infos[be.definer]?.metadata[Qlx.REFERENCES] is TSet bs)
                ...
                (cx,var oo) = cx.Add(et, TMetadata.Empty + (Qlx.REFERENCES, ts));
                et = (UDType)oo;
           //     et = et.FixEdgeType(cx, typename, ts.tree);
                dt = et.Build(cx, null, 0L, typename.ident, ls);
            }
            if (((Transaction)cx.db).physicals[dt.defpos] is PType pt)
                pt.dataType = dt;
            cx.Add(dt);
        }
        /// <summary>
        ...
        MethodName ParseMethod(Domain? xp = null)
        {
            ...
            if (tok == Qlx.FOR)
            {
                ...
                xp = (Domain?)cx._Ob(cx.role.dbobjects[tname.ident]) ??
                    StandardDataType.Get(ttok);
            }
            else if (mn.methodType == PMethod.MethodType.Constructor)
                xp = (Domain?)cx._Ob(cx.role.dbobjects[mn.name.ident]);
		...
		void ParseMethodDefinition()
        {
            ...
            var meth = cx._Ob(oi?.methodInfos[nm.ident]?[Database.Signature(mn.ins)] ?? -1L) as Method ??
    throw new DBException("42132", nm.ToString(), oi?.name ?? "??").Mix();
            ...
            var tn = ut.names ?? Names.Empty;
            cx.Add(tn);
            cx.defs += (meth.defpos, tn + meth.names);
            cx.anames = cx.names;
            var oa = cx.anames;
            var od = cx.defs;
            var m = BTree<long, object>.Empty + (DBObject._Domain, mn.retType) +
                (NestedStatement.WfOK, true) + (Procedure.ProcBody, true);
            meth += (Procedure.Body,(ParseStatement(m)?? throw new DBException("42000", "MethodBody")).defpos);
        ...
        /// </summary>
        /// <returns>A tree of Executable references</returns>
        void ParseDomainDefinition(BTree<long, object> m)
        {
            var colname = new Ident(this);
            Mustbe(Qlx.Id);
            if (tok == Qlx.AS && lxr.ch != '/' && lxr.ch != '.') // catref)
            ...
            if (Match(Qlx.DEFAULT))
            {
                Next();
                int st = lxr.start;
                var dv = ParseSqlValue(m+(DBObject._Domain,type));
        ...
        PCheck? ParseCheckConstraint(Table tb, TableColumn? tc=null)
        {
            var oc = cx.parse;
            cx.parse = ExecuteStatus.Compile;
            ...
            Mustbe(Qlx.LPAREN);
            var nst = cx.db.nextStmt;
            var se = ParseSqlValue(new BTree<long,object>(DBObject._Domain, Domain.Bool)
                +(TableColumn._Table,tb.defpos));
            Mustbe(Qlx.RPAREN);
            var cs = new string(lxr.input, st, lxr.start - st);
            var pc = (tc!=null)?
                new PCheck2(tb,tc,n.ident,se,cs, nst, cx.db.nextPos, cx):
                new PCheck(tb, n.ident, se, cs, nst, cx.db.nextPos, cx);
            cx.parse = oc;
            return pc;
        }
        ...
        Executable ParseCreateTable(int lp)
        {
            var r = Executable.Empty;
            var nst = cx.db.nextStmt;
            ...
            Mustbe(Qlx.LPAREN);
            var m = BTree<long, object>.Empty;
            var tb = new Table(cx.NewObject(),m);
            if (cx.parse == ExecuteStatus.Obey)
            {
                var pt = new PTable(name.ident, Domain.TableType, tb.defpos, cx);
                tb = (Table)(cx.Add(pt) ?? throw new DBException("42105").Add(Qlx.TABLE));
                var ti = tb.infos[cx.role.defpos] ?? new ObInfo(name.ident);
                m += (TableColumn._Table, tb.defpos);
            }
            for (; ; )
            {
                if (StartMetadata(Qlx.TABLE))
                {
                    var (st, mt) = ParseMetadata(Qlx.TABLE,m);
                    si += st; sm += mt;
                }
                else
                    (si,sm,tb) = ParseColumnDefin(tb,si,sm);
            ...
            if (StartMetadata(Qlx.TABLE))
            {
                var (s, md) = ParseMetadata(Qlx.TABLE,m);
                si += s; sm += md;
            }
            if (cx.parse==ExecuteStatus.Obey)
            {
                tb = (Table)(cx.db.objects[tb.defpos] ?? throw new PEException("PE47812"));
                tb = (Table)tb.Add(cx, sm).Item2;
                // ensure that reference columns added by foreign key metadata are not part of the display
                if (tb.infos[cx.role.defpos]?.metadata[Qlx.TRIM] is TSet hs) // of TInt
                {
                    ...
                    {
                        var p = b.value();
                        if (!hs.Contains(new TInt(p)))
                            rt += (j++, p);
                    }
                    tb += (Domain.Display, j);
                    for (var b = hs.First(); b != null; b = b.Next())
                        rt += (j++, b.Value().ToLong() ?? -1L);
                    tb += (Domain.RowType, rt);
                    cx.Add(tb);
                    cx.db += tb;
                    tb = (Table)(cx.db.objects[tb.defpos] ?? throw new DBException("42105"));
                }
            }
            else
                cx.db += r= (Executable)cx.Add(new Executable(cx,new string(lxr.input, lp, lxr.pos - lp)));
            return r;
        }
        ...
        private Table ParseVersioningClause(Table tb, bool drop)
        {
            ...
            if (drop)
            {
                ...
                for (var e = tb.indexes.First(); e != null; e = e.Next())
                    for (var c = e.value().First(); c != null; c = c.Next())
                        if (cx._Ob(c.key()) is Level3.Index px &&
                                    px.tabledefpos == tb.defpos)
                            ph = new Drop(px.defpos, tr.nextPos, tr);
                if (sa == Qlx.SYSTEM && cx._Ob(tb.systemPS) is PeriodDef pd)
            ...
            pi = (Level3.Index)(cx.Add(new PIndex("", tb, pi.keys.rowType, 0, tr.nextPos, tr))
                ?? throw new DBException("42105").Add(Qlx.PRIMARY));
            Mustbe(Qlx.VERSIONING);
            var ixs = tb.indexes;
            var iks = ixs[pi.keys.rowType] ?? CTree<long, bool>.Empty;
            iks += (pi.defpos, true);
            ixs += (pi.keys.rowType, iks);
        ...
        (string,TMetadata,Table) ParseColumnDefin(Table tb,string s,TMetadata d)
        {
            ...
            d -= Qlx.DEFAULT;
            var m = BTree<long, object>.Empty + (TableColumn._Table, tb.defpos) +
                (DBObject._Domain, dom);
            var tc = (TableColumn)cx.Add(new TableColumn(cx.NewObject(),m));
            tb += (Domain.Representation, tb.representation + (tc.defpos, dom));
            ...
            if (cx.parse == ExecuteStatus.Obey || cx.parse == ExecuteStatus.Compile)
            {
                ...
                tc = (TableColumn)(cx.db.objects[pc.defpos] ?? throw new PEException("PE50100"));
            }
            tc += (ObInfo._Metadata, cmd);
            if (dom == null)
                throw new DBException("42120", colname.ident);
            var (tms, tmd) = ParseMetadata(Qlx.TABLE,m+(TableColumn._Table,tb.defpos));
            tc = cx.db.objects[tc.defpos] as TableColumn ?? throw new PEException("PE67382");
            if (cx.parse == ExecuteStatus.Obey|| cx.parse==ExecuteStatus.Compile)
            {
                if (tmd != TMetadata.Empty)
                {
                    var tm = (TMetadata)(tb.mem[ObInfo._Metadata] ?? TMetadata.Empty);
                    tb += (ObInfo._Metadata, tm + tmd);
                }
            }
            cx.db += tc;
            return (s+cms,d+cmd,tb);
        }
        /// <summary>
        ...
        bool StartMetadata(Qlx kind)
        {
            switch (kind)
            {
                case Qlx.TABLE:
                    ...
                    Qlx.ETAG, Qlx.SECURITY, Qlx.PRIMARY,Qlx.UNIQUE,Qlx.CHECK, Qlx.REFERENCES, 
                ...
                case Qlx.COLUMN:
                    ...
                    Qlx.CHARLITERAL, Qlx.SECURITY, Qlx.MULTIPLICITY, Qlx.OPTIONAL, Qlx.REFERENCES,
                    Qlx.PRIMARY, Qlx.UNIQUE,
        ...
        internal TMetadata ParseMetadata(string s, int off, Qlx kind, BTree<long, object> m)
        {
            lxr = new Lexer(cx, s, off);
            return ParseMetadata(kind,m).Item2;
        }
        /// <summary>
        ...
        internal (string,TMetadata) ParseMetadata(Qlx kind, BTree<long, object> m)
        {
            var drop = false;
            var md = m[ObInfo._Metadata]as TMetadata??TMetadata.Empty;
            ...
            var st = lxr.start;
            long iv = -1;
            var lp = Match(Qlx.LPAREN,Qlx.LBRACE);
            if (lp)
                Next();
            while (StartMetadata(kind))
            {
                ...
                switch (tok)
                {
                    ...
                    case Qlx.ARROW:
                        {
                            Next();
                            md += (Mustbe(Qlx.TO, Qlx.FROM, Qlx.WITH),TBool.True);
                            break;
                        }
                    case Qlx.PREFIX:
                    case Qlx.SUFFIX:
                    ...
                    case Qlx.EDGETYPE:
                        {
                            Next();
                            var tb = cx._Ob((long)(m[TableColumn._Table] ?? m[DBObject.Defpos] ?? -1L)) as Table;
                            if (tb is not Table dt)
                                throw new PEException("PE50731");
                            var cl = (dt.infos[cx.role.defpos]?.metadata[Qlx.REFERENCES] as TSet) ?? new TSet(Domain.Connector);
                            var fc = 0;
                            var ac = 0;
                            var wc = 0;
                            if (tok == Qlx.LPAREN)
                            {
                                ...
                                {
                                    (tb,cl) = ParseConnector(tb,cl,Qlx.FROM,++fc);
                                    Mustbe(Qlx.COMMA,Qlx.TO,Qlx.ARROWR);
                                    (tb, cl) = ParseConnector(tb,cl, Qlx.TO,++ac);
                                }
                                else while (!Match(Qlx.RPAREN))
                                {
                                    if (Match(Qlx.COMMA))
                                        Next();
                                    if (!Match(Qlx.FROM, Qlx.TO, Qlx.WITH))
                                    ...
                                    {
                                        Next();
                                        (tb, cl) = ParseConnector(tb,cl, Qlx.WITH,++wc);
                                        while (Match(Qlx.COMMA))
                                        {
                                            Next();
                                            (tb, cl) = ParseConnector(tb,cl, Qlx.WITH,++wc);
                    ...
                    case Qlx.FOR:
                        {
                            graph = new TList(Domain.Int);
                            Next();
                            (_, var fg, _) = ParseMatchExp(CList<long>.Empty, null, CTree<long, TGParam>.Empty, -1L, Qlx.NO, m);
                            for (var b = fg.First(); b != null; b = b.Next())
                                graph += new TInt(b.value());
                            goto case Qlx.REQUIRE;
                        }
                    case Qlx.REQUIRE:
                        {
                            Mustbe(Qlx.REQUIRE);
                            ...
                            if (Match(Qlx.LPAREN))
                            {
                                graph += new TInt(ParseVarOrColumn(m).defpos);
                                while (Match(Qlx.COMMA))
                                {
                                    Next();
                                    graph += new TInt(ParseVarOrColumn(m).defpos);
                    ...
                    case Qlx.CONSTRAINT:
                        {
                            var tc = cx.obs[(long)(m[DBObject.Defpos] ?? -1L)] as TableColumn;
                            if (tc != null && Match(Qlx.FOREIGN))
                                throw new DBException("42161", Qlx.REFERENCES, Qlx.FOREIGN);
                            var cn = (m[ObInfo.Name] is string s) ? new TChar(s) : null;
                            var gq = m[DBObject.Gql] as GQL?;
                            drop = (gq is GQL gQL) && gQL == GQL.DropStatement;
                            if (cx._Ob((long)(m[TableColumn._Table] ?? -1L)) is Table tb)
                            {
                                var x = ConstraintDefinition(tb, tc, drop);
                                if (x is Table)
                                    tb = (Table)x;
                            }
                            // don't add these to TMetadata: they are structural integrity constraints
                            else
                                throw new PEException("PE40101");
                            break;
                        }
                    case Qlx.DEFAULT:
                        {
                            Next();
                            int dd = lxr.start;
                            var g = Generation.Default;
                            var gs = lxr.start;
                            var oc = cx.parse;
                            cx.parse = ExecuteStatus.Compile;
                            var nst = cx.db.nextStmt;
                            if (cx._Ob((long)(m[DBObject.Defpos] ?? -1L)) is TableColumn tc
                                && cx._Ob(tc.tabledefpos) is Table tb)
                            for (var b = tb.rowType.First(); b != null; b = b.Next())
                                if (b.value() is long p && cx.db.objects[p] is TableColumn c)
                                {
                                    var nm = c.NameFor(cx) ?? "";
                                        ...
                                        QlValue rv = (c.domain.kind==Qlx.REF 
                                            && c.domain.elType is Domain dm) ?
                            ...
                            cx.Add(sv);
                            md += (Qlx.VALUE, new TInt(sv.defpos));
                            md += (Qlx.DEFAULT, new TChar(new string(lxr.input, dd, lxr.pos - dd)));
                            md += (Qlx.GENERATED, new TInt((int)g));
                            cx.parse = oc;
                            break;
                        }
                    ...
                    case Qlx.GENERATED:
                        {
                            ...
                            } else
                            {
                                ...
                                cx.parse = ExecuteStatus.Compile;
                                var nst = cx.db.nextStmt;
                                var tb = cx.db.objects[(long)(m[TableColumn._Table]??-1L)] as Table;
                                for (var b = tb?.rowType.First(); b != null; b = b.Next())
                                ...
                                var gnv = ParseSqlValue(m);
                                var s = new string(lxr.input, gs, lxr.start - gs);
                                cx.Add(gnv);
                                md += (Qlx.VALUE, new TInt(gnv.defpos));
                                md += (Qlx.DEFAULT, new TChar(s));
                                cx.parse = oc;
                            }
                            md += (Qlx.GENERATED, new TInt((int)g));
                            break;
                    ...
                    case Qlx.SECURITY:
                        {
                            ...
                            if (Match(Qlx.LEVEL))
                                Next();
                            md += (Qlx.SECURITY,TLevel.New(MustBeLevel()));
                            if (Match(Qlx.SCOPE))
                            {
                                Next();
                                Grant.Privilege r = Grant.Privilege.NoPrivilege;
                    ...
                    case Qlx.LABELS:
                        {
                            Next();
                            var ls = new TSet(Domain.Char);
                            var lv = lxr.val as TChar;
                            Mustbe(Qlx.CHARLITERAL);
                            if (lv != null)
                                ls += lv;
                            while (Match(Qlx.AMPERSAND))
                            {
                                Next();
                                lv = lxr.val as TChar;
                                Mustbe(Qlx.CHARLITERAL);
            ...
            if (lp)
                Next();
            if (ds != TNull.Value && !md.Contains(Qlx.DESC))
                md += (Qlx.DESC, ds);
            if (iv != -1L)
                md += (Qlx.INVERTS, new TInt(iv));
            if (iri != TNull.Value)
                md += (Qlx.IRI, iri);
            var ss = new string(lxr.input, st, lxr.start - st);
            return (ss, md);
        }
        internal DBObject ConstraintDefinition(Table tb, TableColumn? tc, bool drop)
        {
            DBObject x;
            Qlx s = Mustbe(Qlx.UNIQUE, Qlx.PRIMARY, Qlx.FOREIGN, Qlx.REFERENCES, Qlx.CHECK, Qlx.CONSTRAINT);
            if (s == Qlx.CHECK && ParseCheckConstraint(tb, tc) is PCheck pc)
                x = cx.Add(pc) ?? throw new DBException("42015");  // returns the checked object
            else
            {
                var key = CTree<int, long>.Empty;
                if (tc != null)
                {
                    key += (0, tc.defpos);
                    tb += (Domain.Representation, tb.representation + (tc.defpos, tc.domain));
                }
                if (Match(Qlx.KEY))
                    Next();
                if (Match(Qlx.LPAREN))
                    key = ParseColsList(tb, Qlx.ROW)?.rowType ?? throw new DBException("42161", "cols");
                else if (tc != null) // && s!=Qlx.REFERENCES
                    key = new CTree<int, long>(0, tc.defpos);
                if (s == Qlx.FOREIGN)
                ...
                switch (s)
                {
                    ...
                    case Qlx.REFERENCES:
                    case Qlx.FOREIGN: x = Reference(tb, key, drop, s); break;
                    default:
        ...
        DBObject UniqueIndex(Table tb, CTree<int,long> key, bool drop)
        {
            ...
            if (cx.parse == ExecuteStatus.Obey || cx.parse == ExecuteStatus.Compile)
            {
                if (drop)
                {
                    ...
                    var ob = cx.Add(new Drop(dx.defpos, cx.db.nextPos, cx.db)) ?? throw new DBException("42105");
                    return dx;
                }
                var px = new PIndex("", tb, key, PIndex.ConstraintType.Unique, tr.nextPos, tr);
                tb = (Table)(cx.Add(px) ?? throw new DBException("42105"));
            }
            return tb;
        }
        DBObject PrimaryIndex(Table tb, CTree<int,long> key, bool drop)
        {
            ...
            if (cx.parse == ExecuteStatus.Obey || cx.parse == ExecuteStatus.Compile)
            {
                ...
                var px = new PIndex("", tb, key, PIndex.ConstraintType.PrimaryKey, tr.nextPos, tr);
                tb = (Table)(cx.Add(px) ?? throw new DBException("42105"));
            }
            return tb;
        }
        Table Reference(Table tb, CTree<int, long> refs, bool drop, Qlx s)
        {
            ...
            var cols = CTree<int, long>.Empty;
            var st = lxr.start;
            var nst = cx.db.nextStmt;
            Mustbe(Qlx.Id);
            if (tok == Qlx.LPAREN)
                cols = ParseColsList(rt)?.rowType ?? throw new DBException("42000").Add(Qlx.REFERENCING);
            else
                cols = refs;
            if (s == Qlx.FOREIGN)
            {
                var km = new TList(Domain.Int, cols);
                md += (Qlx.FOREIGN, km);
            }
            PIndex.ConstraintType ct = (s == Qlx.FOREIGN) ? PIndex.ConstraintType.ForeignKey : PIndex.Reference;
            ct |= PIndex.ConstraintType.RestrictUpdate | PIndex.ConstraintType.RestrictDelete;
            md += (Qlx.REFERENCING, new TTypeSpec(refname.ident, rt));
            TableColumn? co = null;
            // For Alters, we want to see if we already have a referencing column with the right target and keys
            for (var b = tb.rowType.First(); b != null; b = b.Next())
                if (cx._Ob(b.value()) is TableColumn c && c.domain.kind == Qlx.REF)
                    if (s != Qlx.FOREIGN)
                    {
                        if (rt.defpos == c.domain.defpos)
                        {
                            co = c; goto found;
                        }
                    }
                    else if (c.keyMap != CTree<int, long>.Empty)
                    {
                        var kb = c.keyMap.First();
                        for (var kc = cols.First(); kb != null && kc != null; kb = kb.Next(), kc = kc.Next())
                            if (kc.value() != kb.value())
                                continue;
                        co = c; goto found;
                    }
        found:;
            ...
            if (Match(Qlx.USING))
            {
                ...
                if (tok == Qlx.Id)
                {
                    ...
                    for (var b = refs.First(); b != null; b = b.Next())
                        if (cx._Dom(b.value()) is Domain d)
                            rs += (b.value(), d);
                    var dm = new Domain(cx, rs, refs, (int)rs.Count, BTree<long, ObInfo>.Empty);
                    var pr = cx.GetProcedure(LexDp(), ic.ident, Database.Signature(dm))
                ...
                {
                    Mustbe(Qlx.LPAREN);
                    ParseSqlValueList(BTree<long, object>.Empty);
                    Mustbe(Qlx.RPAREN);
            ...
            {
                st = lxr.start;
                ct = ParseReferentialAction(ct);
                md += (Qlx.ACTION, new TInt((int)ct));
            }
            Level3.Index? rx = cols == CTree<int, long>.Empty ? rt.FindPrimaryIndex(cx) 
                : rt.FindIndex(cx.db, cols,tb.representation)?[0];
            if (cx.parse == ExecuteStatus.Obey)
            {
                if ((rx == null || (refs.Count != 0 && (int)rx.keys.Length != refs.Count))
                    && cols[0] is long p && cx.obs[p] is TableColumn cr)
                {
                    if (cr.domain.kind == Qlx.Null)// make cr a new simple reference to rt
                    {
                        ...
                        {
                            pc.domdefpos = rd.defpos;
                            pc.dataType = rd;
                            ta += (Transaction.Physicals, ta.physicals + (p, pc));
                        }
                    }
                    else if (rx == null)
                ...
                if (co != null)
                {
                    if (cx.parse == ExecuteStatus.Obey || cx.parse == ExecuteStatus.Compile)
                        if (drop)
                            cx.Add(new Drop(co.defpos, cx.db.nextPos, cx.db));
                        else
                            cx.Add(new PMetadata("Refs", 0L, co, md.ToString(), md, nst, cx.db.nextPos, cx));
                }
                else
                {
                    var ri = -1L;
                    var ti = tb.infos[cx.role.defpos] ?? new ObInfo(tb.name, Grant.AllPrivileges);
                    var mr = ti.metadata ?? TMetadata.Empty;
                        ...
                        && (tb.representation[cp] is not Domain cd || cd.kind == Qlx.Null))
                    {
                        ...
                        {
                            ac = (TableColumn)cx.Add(ac + (QuerySearch.Action, ct));
                            cx.db += ac;
                        }
                    ...
                    {
                        // There will be no Index constructed, but we need to pick up refs on Database.Load
                        ...
                        if (cx.parse == ExecuteStatus.Obey || cx.parse == ExecuteStatus.Compile)
                        {
                            ...
                            pc.flags = PIndex.ReferentialAction(pc.flags, ct);
                            tb = (Table)(cx.Add(pc) ?? throw new DBException("42105"));
                            tc = new TConnector(Qlx.TO, "", rt, pc.defpos, true);
                            // in this case, we note the extra column in table metadata so that it will get placed
        ...
        PIndex.ConstraintType ParseReferentialAction(PIndex.ConstraintType ct, bool drop=false)
        {
            while (Match(Qlx.ON))
            {
                Next();
                ...
                PIndex.ConstraintType f;
                if (when == Qlx.UPDATE)
                {
                    f = what switch
                    {
                        Qlx.CASCADE => PIndex.ConstraintType.CascadeUpdate,
                        Qlx.DEFAULT => PIndex.ConstraintType.SetDefaultUpdate,
                        Qlx.NULL => PIndex.ConstraintType.SetNullUpdate,
                        Qlx.RESTRICT => PIndex.ConstraintType.RestrictUpdate,
                        _ => PIndex.ConstraintType.NoType
                    };
                    ct = drop? (ct&~f)|PIndex.Updates : (ct&~PIndex.Updates)|f;
                }
                else
                {
                    f = what switch
                    {
                        Qlx.CASCADE => PIndex.ConstraintType.CascadeDelete,
                        Qlx.DEFAULT => PIndex.ConstraintType.SetDefaultDelete,
                        Qlx.NULL => PIndex.ConstraintType.SetNullDelete,
                        Qlx.RESTRICT => PIndex.ConstraintType.RestrictDelete,
                        _ => PIndex.ConstraintType.NoType
                    };
                    ct = drop? (ct&~f)|PIndex.Deletes : (ct&~PIndex.Deletes)|f;
                }
            }
            return ct;
        }
        ...
        (Table, TSet) ParseConnector(Table et, TSet cl, Qlx tk, int k)
        {
            ...
            if (sc == "")
                sc = tk.ToString()+k;
            Match(Qlx.TO,Qlx.WITH);
            while (tok == Qlx.VBAR)
            {
                Next();
                ln = lxr.val;
                tp = (Table)(cx._Ob(cx.role.nodeTypes[ln.ToString()]) ?? throw new DBException("42107", ln));
                xl += ((Domain)cx.Add(tp),true);
                Mustbe(Qlx.Id);
            }
            var s = false;
            ...
            if (xl.Count > 1)
            {
                ...
                if (cx.db.role.dbobjects[tn] is long yp && yp!=0L)
                {
                    xp = yp;
                    ct = (Domain)(cx._Ob(xp) ?? Domain.Content);
                }
                else
                {
                    var tu = new Table(-1L, xl)
                        + (DBObject.Infos, new BTree<long, ObInfo>(cx.role.defpos, new ObInfo(tn)));
                    var up = cx.db.types[tu];
                    if (up != 0L)
                        ct = (Domain)(cx._Ob(up) ?? throw new PEException("PE60664"));
                    else
                    {
                        var pt = new PUnion(xl, xp, cx);
                        cx.Add(pt);
                        tu = pt.domain ?? tu;
                        ct = (Domain?)cx.Add(tu) ?? throw new PEException("PE60665");
                        cx.db += ct;
                    }
                }
            }
            ...
            if (dm.kind == Qlx.Null)
                dm = ct;
            if (ct is null || dm is null)
                throw new DBException("42000");
            var cd = s ? new Domain(-1L, Qlx.SET, dm) : dm;
            string ts = "";
            var tm = TMetadata.Empty;
            if (cx.parsingGQL!=Context.ParsingGQL.Match && StartMetadata(Qlx.CONNECTING))
                (ts, tm) = ParseMetadata(Qlx.CONNECTING,BTree<long,object>.Empty);
            var cp = et.names[sc].Item2;
            TConnector? cc = null;
            if (cp > 0)
            {
                cc = new TConnector(tk, sc, cd, cp, false, ts, tm);
                et = et.Add(cx, cc);
            }
            else
            {
                var np = cx.db.nextPos;
                var rd = (cx.db.refTypes[ct] is long rp && rp != 0L) ? cx._Ob(rp) as Domain
                    : (Domain?)cx.Add(new PDomain(Physical.Type.PDomain, "", Qlx.REF, 0, 0, CharSet.UCS,
                ...
                et = (Table)(cx._Ob(et.defpos) ?? throw new DBException("42105").Add(Qlx.REF));
            }
            cx.db += et;
            var ns = new TSet(Domain.Connector);
            for (var b=cl.First();b!=null;b=b.Next()) // remove any connector for this column
            {
                var e = b.Value() as TConnector ?? throw new PEException("PE73671");
                if (e.cp!=cc.cp)
                    ns += e;
            }
            return (et, ns + cc);
        ...
        public void ParseProcedureClause(bool func, Qlx create)
        {
            ...
            if (pr == null)
            {
                if (create == Qlx.CREATE)
                {
                    ...
                    pr = new Procedure(pp.defpos, cx, ps, Domain.Null,
                        new BTree<long, object>(DBObject.Definer, cx.role.defpos)
                        + (DBObject._Domain,rdt)
            ...
            else
            {
                ...
                {
                    var (ss, tm) = ParseMetadata(Qlx.FUNCTION,m);
                    var pm = new PMetadata3(n.ident, 0, pr, ss,  tm, nst, cx.db.nextPos, cx);
                    cx.Add(pm);
                }
                ...
                if (tok != Qlx.EOF && tok != Qlx.SEMICOLON)
                {
                    cx.AddParams(pr);
                    cx.Add(pr);
                    var bd = ParseStatements(m+(Procedure.ProcBody,true)
                        +(DBObject.Scope,ap)+(DBObject._Domain,pr.domain)
                        + (NestedStatement.WfOK,true)) ?? throw new DBException("42000", "Statement");
                    cx.Add(pr);
        ...
        internal Domain ParseReturnsClause(Ident pn)
        {
            ...
            if (tok == Qlx.Id)
            {
                var s = lxr.val.ToString();
                Next();
                var ob = (Domain)(cx._Ob(cx.role.dbobjects[s])
     ...
     //           cx.names += (pn.ident, pn.uid);
		...
		Executable ParseDeclaration()
        {
            ...
            if (Match(Qlx.CURSOR))
            {
                Next();
                Mustbe(Qlx.FOR);
                var cs = ParseSelectStatement(BTree<long,object>.Empty);
            ...
            else
            {
                ...
                {
                    Next();
                    var iv = ParseSqlValue(new BTree<long,object>(DBObject._Domain,ld));
                    cx.Add(iv);
        ...
        HandlerStatement ParseHandlerDeclaration()
        {
            ...
            ParseStatement(new BTree<long,object>(NestedStatement.WfOK,true));
		...
		NestedStatement ParseNestedStatement(BTree<long,object> m)
        {
            var cs = new NestedStatement(LexDp(), cx, CList<long>.Empty, m);
        ...
        GetDiagnostics ParseGetDiagnosticsStatement()
        {
            ...
            for (; ; )
            {
                var t = ParseSqlValueEntry(BTree<long,object>.Empty);
                cx.Add(t);
                Mustbe(Qlx.EQL);
		...
		Executable ParseLabelledStatement(BTree<long, object> m)
        {
            ...
            if (tok == Qlx.COLON)
            {
                ...
                var e = tok switch
                {
                    Qlx.BEGIN => ParseNestedStatement(m+(Executable.Label,s)),
                    Qlx.FOR => ParseForStatement(m+(Executable.Label, s)),
                    Qlx.LOOP => ParseLoopStatement(m+(Executable.Label, s)),
                    Qlx.REPEAT => ParseRepeat(m+(Executable.Label, s)),
                    Qlx.WHILE => ParseSqlWhile(m+(Executable.Label, s)),
        ...
        Executable ParseAssignment()
        {
            ...
            if (Match(Qlx.COLON, Qlx.IS) && cx.binding[vb.defpos] is TNode tn)
            {
                ...
                Mustbe(Qlx.Id);
                if (cx._Ob(cx.role.dbobjects[ln.ident]) is not Table nt)
		...
		Executable ParseMultipleAssignment()
        {
            Mustbe(Qlx.EQL);
            var ids = ParseIDList();
            var v = ParseSqlValue(BTree<long,object>.Empty);
            cx.Add(v);
        ...
        /// |   FINISH
        /// </summary>
		Executable ParseReturn(BTree<long, object> m)
        {
            var ap = (long)(m[DBObject.Scope] ?? 0L);
            var xp = m[DBObject._Domain] as Domain?? Domain.Null;
            if (Match(Qlx.FINISH))
            {
                Next();
            ...
            if (xp.kind == Qlx.CONTENT)
            {
                ...
                re = ParseSqlValue(m);
            cx.Add(re);
            var rs = new ReturnStatement(cx.GetUid(),cx,xp,re);
        ...
        /// </summary>
        /// <returns>the Executable resulting from the parse</returns>
		Executable ParseCaseStatement(BTree<long, object> m)
        {
            Next();
            if (tok == Qlx.WHEN)
            {
                var ws = ParseWhenList(m);
                var ss = Executable.Empty;
                if (tok == Qlx.ELSE)
                {
                    Next();
                    ss = ParseStatements(m);
                }
            ...
            {
                var op = ParseSqlValue(m);
                var ws = ParseWhenList(m+(DBObject._Domain, op.domain));
                var ss = Executable.Empty;
                if (tok == Qlx.ELSE)
                {
                    Next();
		...
		BList<WhenPart> ParseWhenList(BTree<long, object> m)
        {
            ...
            {
                Next();
                var c = ParseSqlValue(m+(DBObject._Domain,Domain.Bool));
                Mustbe(Qlx.THEN);
                r += new WhenPart(dp, c, ParseStatements(m));
            }
            return r;
        }
        LetStatement ParseLet()
        {
            ...
            while (tok == Qlx.LET || tok == Qlx.COMMA)
            {
                Next();
                if (tok == Qlx.VALUE)
                    r += ParseBindingVariableDefinitions(BTree<long,object>.Empty); // we expect just one
                else
                {
                    var id = new Ident(this);
                    Mustbe(Qlx.Id);
                    ...
                    Mustbe(Qlx.EQL);
                    var sv = ParseSqlValue(BTree<long,object>.Empty);
                    cx.Add(sv);
                    var sa = new UpdateAssignment(vb.defpos, sv.defpos);
                    vb += (DBObject._Domain, sv.domain);
                    cx.Add(vb);
                    r += ((int)r.Count, vb.defpos);
                    rs += (vb.defpos, sv.domain);
                    sg += (sa, true);
                }
            }
            var dm = (r==CTree<int,long>.Empty)?Domain.TableType:new Domain(-1L,cx,Qlx.TABLE,rs,r);
            var ls = new LetStatement(lp, new BTree<long, object>(RowSet.Assig,sg)+(DBObject._Domain,dm));
            cx.Add(ls);
            return ls;
        }
        OrderAndPageStatement ParseOrderAndPage(BTree<long,object> m)
        {
            var lp = LexDp();
            ...
            if (tok == Qlx.ORDER)
            {
                var os = ParseOrderClause(rs,rs.ordSpec,m);
                m += (RowSet.RowOrder, os);
            }
            if (Match(Qlx.OFFSET,Qlx.SKIP))
            {
                Next();
                var so = ParseSqlValue(m+(DBObject._Domain, Domain.Int));
                cx.Add(so);
        ...
        FilterStatement ParseFilter()
        {
            ...
            if (ParseWhereClause(m) is CTree<long,bool> wh)
                m += (RowSet._Where, wh);
        ...
        /// <returns>the Executable valueType of the parse</returns>
        Executable ParseForStatement(BTree<long, object> m)
        {
            var n = m[Executable.Label] as string;
            var lp = LexDp();
            Next();
            ...
            var ss = ParseSelectStatement(m); // use ambient declarations
            d = Math.Max(d, ss.depth + 1);
            var cs = (RowSet?)cx.obs[ss.result] ?? throw new DBException("42000", "CursorSpec");
            var sawDo = Match(Qlx.DO);
            if (sawDo) Next();
            var xs = ParseStatements(m);
            m += (ForSelectStatement.ForVn, c.ident);
            m += (ForSelectStatement.Sel, cs.defpos);
            m += (ConditionalStatement.Then, xs);
            var fs = new ForSelectStatement(lp, m) + (DBObject._Depth, d);
        ...
        Executable ParseCompositeQueryStatement(BTree<long, object> m)
        {
            var lp = LexDp();
        ...
        /// <param name="xp">The type of the test expression</param>
        /// <returns>the Executable valueType of the parse</returns>
        Executable ParseIfThenElseStmt(BTree<long, object> m)
        {
            var lp = LexDp();
            var old = cx;
            Next();
            var se = ParseSqlValue(m+(DBObject._Domain, Domain.Bool));
            cx.Add(se);
            Mustbe(Qlx.THEN);
            ...
            while (Match(Qlx.ELSEIF))
            {
                var d = LexDp();
                Next();
                var s = ParseSqlValue(m+(DBObject._Domain, Domain.Bool));
                cx.Add(s);
                Mustbe(Qlx.THEN);
                Next();
        ...
        Executable ParseConditionalStmt(BTree<long, object> m)
        {
            var lp = LexDp();
            var old = cx;
            Next();
            ...
            var w030 = th.gql;
            m += (DBObject.Gql,th.gql); // for WG3:GYD-030 now apply this value to all branches
            var ei = CList<long>.Empty;
            while (Match(Qlx.ELSEIF))
            {
                var d = LexDp();
        ...
        internal Executable ParseStatements(BTree<long, object> m)
        {
            var ap = (long)(m[DBObject.Scope] ?? -1L);
            var lp = LexDp();
            ...
            while (StartStatement() && !Match(Qlx.UNTIL,Qlx.EOF))
            {
                ...
                if (b is not EmptyStatement)
                {
                    b = (Executable)cx.Add(b);
                    s += b.defpos;
                    BindingAggs(s);
                    ls = b;
                }
            ...
            if (s.Count > 1)
            {
                var ns = new NestedStatement(cx.GetUid(), cx, s, m);
                ...
                if (cx.obs[b.value()] is Executable e)
                {
                    ...
                    if (bs is not null && e is QueryStatement qs && cx.obs[qs.result] is RowSet qr)
                    {
                        ...
                        if (gs != CTree<long, Domain>.Empty)
                        {
                            ...
                            bs = bs + (RowSet.GroupCols, gd) + (RowSet.Groupings, new CList<long>(gd.defpos));
                        }
		...
		CList<long> ParseTargetList(BTree<long, object> m)
        {
            bool b = (tok == Qlx.LPAREN);
            if (b)
                Next();
        ...
        QlValue ParseVarOrColumn(BTree<long, object> m)
        {
            var ap = LexLp();
            ...
            if (tok == Qlx.LPAREN)
            {
                ...
                if (ic.Length == 1)
                {
                    ...
                    if (pr == null && (cx._Ob(cx.role.dbobjects[pn.ident])
                        ?? tb ?? StandardDataType.Get(ttok)) is Domain ut && ut != Domain.Content)
                    {
                        cx.Add(ut);
                        var oi = ut.infos[cx.role.defpos];
                        var ml = oi?.methodInfos[pn.ident];
                        if (cx._Ob(ml?[n] ?? -1L) is Method me)
                ...
                else if (ic.Prefix(ic.Length - 2) is Ident pf)
                {
                    ...
                    if (qo is TableRowSet ts && cx._Ob(ts.target) is UDType ut
                        && ut.infos[cx.role.defpos] is ObInfo ui
                        && cx._Ob(ui.methodInfos[pn.ident]?[n] ?? -1L) is Method me
                        && me.methodType == PMethod.MethodType.Instance)
                    {
                        var cu = (QlValue)cx.Add(new SqlCursor(cx.GetUid(), ts, ""));
                        return (QlValue)cx.Add(new SqlMethodCall(lp, cx, me, ps, cu));
                    }
                    var vr = (QlValue)qo;
        ...
        DBObject Identify(Ident ic, BList<Ident> il, Domain xp, long f=-1L)
        {
            ...
            if (pa is not null && sub is null)
                return pa;
            var m = sub?.Length ?? 0;
            if (pa is QlInstance sc && cx._Ob(sc.sPos) is TableColumn tc
            ...
            if (pa is QlValue sv && sv.domain.infos[cx.role.defpos] is ObInfo si && ic.sub is not null
                && cx._Ob(si.names[ic.sub.ident].Item2) is TableColumn tc1)
            {
                var co = new QlInstance(ic.sub, cx, sv.defpos, tc1);
                var nc = new SqlValueExpr(ic, cx, Qlx.DOTTOKEN, sv, co, Qlx.NO);
                return nc;
            }
            if (pa is QlValue sv1 && cx._Ob(sv1.domain.defpos) is Table tt && sub is not null)
            {
            ...
            for (var i = nm; i < len; i++)
                ...
                {// the ident of the component to create
                    if (i == len - 1)
                    {
                        ...
                        else if (cx.parsingGQL!=Context.ParsingGQL.No)
                        {
        ...
        Ident ParseIdent(bool stdfns = false)
        {
            ...
            if (stdfns && StartStdFunctionRefs())
                ...
                Mustbe(Qlx.Id, Qlx.PARTITION, Qlx.REF, Qlx.VERSIONING, Qlx.CHECK,
                    Qlx.SYSTEM_TIME);
            return c;
        }
        /// <summary>
        ...
        /// <param name="xp">The type of the test expression</param>
        /// <returns>the Executable valueType of the parse</returns>
		Executable ParseLoopStatement(BTree<long, object> m)
        {
            var ls = new LoopStatement(LexDp(), m);
            Next();
            ls += (NestedStatement.Stms, ParseStatements(m));
            Mustbe(Qlx.END);
            Mustbe(Qlx.LOOP);
            if (tok == Qlx.Id && m[Executable.Label] is string n && n == lxr.val.ToString())
                Next();
        ...
        /// <param name="xp">The type of the test expression</param>
        /// <returns>the Executable valueType of the parse</returns>
        Executable ParseSqlWhile(BTree<long, object> m)
        {
            var xp = cx.result ?? Domain.Null;
            var ws = new WhileStatement(LexDp(),m);
            ...
            Mustbe(Qlx.DO);
            ws += (cx, WhileStatement.What, ParseStatements(m));
            Mustbe(Qlx.END);
            Mustbe(Qlx.WHILE);
            if (tok == Qlx.Id && m[Executable.Label] is string n && n == lxr.val.ToString())
                Next();
        ...
        /// </summary>
        /// <param name="n">the label</param>
        /// <param name="xp">The object type of the test expression</param>
        /// <returns>the Executable valueType of the parse</returns>
        Executable ParseRepeat(BTree<long, object> m)
        {
            var rs = new RepeatStatement(LexDp(),m);
            Next();
        ...
        /// </summary>
        /// <returns>The Executable valueType of the parse</returns>
        Executable ParseFetchStatement(BTree<long, object> m)
        {
            if (Match(Qlx.FETCH))
                Next();
            var dp = LexDp();
            ...
            if (how == Qlx.ABSOLUTE || how == Qlx.RELATIVE)
                where = ParseSqlValue(m+(DBObject._Domain,Domain.Int));
            if (tok == Qlx.FROM)
                Next();
            var o = new Ident(this);
            Mustbe(Qlx.Id);
        ...
        internal long ParseTriggerDefinition(PTrigger trig)
        {
            ...
            var tb = cx._Ob(trig.target) as Table
                ?? throw new PEException("PE1562");
            tb = (Table)cx.Add(tb);
            cx.Add(tb.framing);
            ...
            if (trig.oldRow != null)
            {
                ...
                cx.Add(so);
                cx.binding += (trig.oldRow.uid, new TTypeSpec(tb.name,fm));
            ...
            if (trig.newRow != null)
            {
                ...
                cx.Add(sn);
                cx.binding += (trig.newRow.uid, new TTypeSpec(tb.name,fm));
            ...
            {
                Next();
                when = ParseSqlValue(new BTree<long,object>(DBObject._Domain,Domain.Bool));
            }
            if (Match(Qlx.BEGIN, Qlx.LBRACE))
            {
                ...
                if (Match(Qlx.TRANSACTION))
                    throw new DBException("22G01", "Nested transactions are not supported").ISO();
                var rr = CList<long>.Empty;
                while (tok != et && StartStatement() 
                    && ParseStatement(BTree<long,object>.Empty) is Executable a)
                {
                    rr += cx.Add(a).defpos;
                    if (Match(Qlx.SEMICOLON, Qlx.NEXT))
                        Next();
                ...
                act = ParseStatements(new BTree<long,object>(Procedure.ProcBody,true)) ??
                    throw new DBException("42161", "statement");
            cx.Add(act);
            var r = (WhenPart)cx.Add(new WhenPart(LexDp(), when, act));
            trig.def = r.defpos;
            trig.framing = new Framing(cx, trig.nst);
            cx.Add(tg + (HandlerStatement.Action, r.defpos));
		...
		void ParseAlter()
        {
            Next();
            MethodModes();
            Match(Qlx.CONSTRUCTOR, Qlx.DOMAIN, Qlx.FUNCTION, Qlx.GRAPH, Qlx.INSTANCE, Qlx.METHOD,
            ...
            switch (tok)
            {
                ...
                case Qlx.GRAPH: ParseAlterGraph(); break;
        ...
        private void ParseAlterGraph()
        {
            ...
            var id = lxr.val.ToString();
            Mustbe(Qlx.Id);
            var ob = cx.db.objects[cx.role.dbobjects[id]] as DBObject;
            var m = BTree<long, object>.Empty;
            if (ob!=null)
                m += (Executable.UseGraph,ob);
            DBObject? nb = null;
            var op = Mustbe(Qlx.ADD,Qlx.SET,Qlx.REMOVE);
            var s = StartStatement();
            var e = Executable.Empty;
            string? ni = null;
            if (Match(Qlx.Id))
            {
            ...
            if (s)
                e = ParseStatement(m);
            if (t && ob is GraphType gt)
            {
                var ts = gt.elTypes;
                var tl = CTree<string, long>.Empty;
                if (cx.db.objects[nb?.defpos ?? -1L] is Domain nt)
                    switch (op)
                    {
                        case Qlx.ADD: ts += (nt.defpos, true); break;
                        case Qlx.REMOVE: ts -= nt.defpos; break;
                    }
                else
                {
                    if (Match(Qlx.LBRACE))
                    {
                        tl = ParseNestedGraphTypeSpecification(tl);
                        if (op == Qlx.SET)
                            ts = CTree<long, bool>.Empty;
                        for (var b = tl.First(); b != null; b = b.Next())
                            switch (op)
                            {
                                case Qlx.SET:
                                case Qlx.ADD: ts += (b.value(), true); break;
                                case Qlx.REMOVE: ts -= b.value(); break;
                            }
                    }
                    else
                    {
                        var dp = ParseElementTypeSpec(tl);
                        if (cx.db.objects[dp] is Domain nd)
                            switch (op)
                            {
                                case Qlx.SET:
                                case Qlx.ADD: ts += (nd.defpos, true); break;
                                case Qlx.REMOVE: ts -= nd.defpos; break;
                            }
                    }
                }
            ...
            else if (ParseStatement(m) is MatchStatement ms)
            {
                ...
                var gp = new TGParam(pi.uid, "_p", Domain.PathType, TGParam.Type.Path, ms.defpos);
                lxr.tgs += (pi.uid, gp);
                var qi = cx.Add(new QlValue(pi, new BList<Ident>(pi), cx, Domain.PathType));
                cx.Add(pi.ident, pi.lp, qi);
                cx.bindings += (pi.uid, qi.domain);
        ...
        void ParseAlterOp(Domain ob)
        {
            ...
            if (tok == Qlx.SET && ob is View)
            {
                ...
                Mustbe(Qlx.TO);
                st = lxr.start;
                var qe = ParseStatements(BTree<long,object>.Empty);
                if (qe is not NestedStatement ne || ne.stms.Count!=1)
                    throw new DBException("42000");
                s = new Ident(new string(lxr.input, st, lxr.start - st), lp);
                cx.Add(new Modify("Source",lp, new QueryStatement(lp, ob), s, cx.db.nextPos, cx));
                cx.obs -= ne.defpos;
            }
            else if (tok == Qlx.TO)
            {
            ...
            else if (Match(Qlx.ALTER))
            {
                ...
                var m = BTree<long, object>.Empty + (TableColumn._Table, tb.defpos) +
                     (DBObject.Defpos, cx.db.nextPos);
                var (s,tm) = ParseMetadata(Qlx.COLUMN,m);
                cx.Add(new PMetadata(ic.ident, 0, ob, s, tm, nst, cx.db.nextPos, cx));
            }
            if (StartMetadata(kind) || Match(Qlx.ADD))
            {
                ...
                if (Match(Qlx.ALTER))
                    Next();
                var (s,tm) = ParseMetadata(kind,BTree<long,object>.Empty);
                np = tr.nextPos;
                cx.Add(new PMetadata(ob.NameFor(cx), -1, ob, s, tm, nst, np, cx));
            }
                cx.parse = op;
        }
        /// <summary>
        ...
        CList<long> ParseAlterDomainOp(Domain d)
        {
            ...
            if (tok == Qlx.SET)
            {
                ...
                Mustbe(Qlx.DEFAULT);
                int st = lxr.start;
                var dv = ParseSqlValue(new BTree<long,object>(DBObject._Domain,Domain.For(d.kind)));
            ...
            else if (Match(Qlx.ADD))
            {
                ...
                var nst = cx.db.nextStmt;
                int st = lxr.pos;
                var sc = ParseSqlValue(new BTree<long,object>(DBObject._Domain, Domain.Bool));
            ...
            else if (tok == Qlx.DROP)
            {
                ...
                else if (StartMetadata(Qlx.DOMAIN) || Match(Qlx.ADD, Qlx.DROP))
                {
                    if (tr.role.Denied(cx, Grant.Privilege.Metadata))
                        throw new DBException("42105").Add(Qlx.METADATA).Mix();
                    var (s,m) = ParseMetadata(Qlx.DOMAIN,BTree<long,object>.Empty);
                    if (cx.Add(new PMetadata(d.name, -1, d, s, m, cx.db.nextStmt, dp, cx)) is DBObject me)
                        es += me.defpos;
                }
                else
                {
            ...
            else if (StartMetadata(Qlx.DOMAIN) || Match(Qlx.ADD, Qlx.DROP))
            {
                ...
                if (cx.role.Denied(cx, Grant.Privilege.Metadata))
                    throw new DBException("42105").Add(Qlx.METADATA).Mix();
                var (s, m) = ParseMetadata(Qlx.DOMAIN,BTree<long,object>.Empty);
                if (cx.Add(new PMetadata(d.name, -1, d, s, m, nst, tr.nextPos, cx)) is DBObject eo)
                    es += eo.defpos;
                cx.parse = op;
            }
            else
        ...
        void ParseAlterTable(Table tb)
        {
            ...
            if (Match(Qlx.SCOPE))
                ParseEnforcement(tb);
            var md = tb.infos[tb.definer]?.metadata??TMetadata.Empty;
            var ms = "";
            if (StartMetadata(Qlx.TABLE))
            {
                var m = BTree<long, object>.Empty + (TableColumn._Table, tb.defpos) + (DBObject.Gql, GQL.DropStatement);
                var (x, y) = ParseMetadata(Qlx.TABLE, m);
                ms += x; md += y;
            }
            if (Match(Qlx.ADD))
            {
                ...
                if (StartMetadata(Qlx.TABLE))
                {
                    var (x, y) = ParseMetadata(Qlx.TABLE, new BTree<long, object>(TableColumn._Table, tb.defpos));
                    ms += x; md += y;
                    cx.Add(new PMetadata(tb.NameFor(cx), 0, tb, ms, md, nst, cx.db.nextPos, cx));
                    cx.parse = op;
                    return;
                }
                else
            ...
            else if (Match(Qlx.ALTER))
            {
                ...
                var o = new Ident(this);
                Mustbe(Qlx.Id);
                var col = (TableColumn?)cx._Ob(tb.ColFor(cx, o.ident))
                        ?? throw new DBException("42112", o.ident);
                while (StartMetadata(Qlx.COLUMN) || Match(Qlx.TO, Qlx.REF, Qlx.SET, Qlx.DROP, Qlx.ADD, Qlx.TYPE))
            ...
            {
                Next();
                ConstraintDefinition(tb, null, false);
            }
            ...
            else if (md != TMetadata.Empty)
            {
                        ...
                        case Qlx.UNIQUE:
                            {
                                ...
                                for (var c = (b.value() as TList)?.First(); c != null; c = c.Next())
                                    cl += cx._Ob(c.Value().ToLong() ?? -1L) as DBObject
                                        ?? throw new PEException("PE31402");
                                var dm = new Domain(Qlx.ROW, cx, cl);
                                if (tb.FindIndex(cx.db, dm.rowType)?[0] is Level3.Index x)
                ...
                cx.Add(new PMetadata(tb.NameFor(cx), 0, tb, ms, md, cx.db.nextStmt, cx.db.nextPos, cx));
        }
        /// <summary>
        /// <summary>
        /// AlterColumn = 	TO id
        ...
        Table ParseAlterColumn(Table tb, TableColumn tc, string nm)
        {
            ...
            cx.parse = ExecuteStatus.Compile;
            var m = BTree<long, object>.Empty + (TableColumn._Table, tb.defpos)
                + (DBObject._Domain,tc.domain) + (ObInfo.Name, nm);
            TypedValue o;
            if (Match(Qlx.TO))
            {
                Next();
            ...
            else if (Match(Qlx.REF))
            {
                ...
                var pa = new Alter3(tc.defpos, nm, n, tb, tc.domain,
                   "", tc.optional, tc.generation, nst, tr.nextPos, cx);
                tb = (Table)(cx.Add(pa) ?? throw new DBException("42105").Add(Qlx.REF));
                return tb;
            }
           ...
           //             Next();
                    ...
                    tm += (Qlx.GENERATED, new TInt((int)g));
                    cx.parse = oc;
                    var s = new string(lxr.input, st-1, lxr.pos - st+1);
                    tc = (TableColumn)(cx.Add(new PMetadata(nm, 0, tc, "default " + s, tm,nst,cx.db.nextPos,cx))
                        ??throw new DBException("42105"));
                }
                else
                {
                    ...
                    if (tb.Denied(cx, Grant.Privilege.Metadata))
                        throw new DBException("42105").Add(Qlx.METADATA).Mix();
                    var pm = new PMetadata(nm, 0, tc, ms, md, nst, tr.nextPos, cx);
                    tc = (TableColumn)(cx.Add(pm) ?? throw new DBException("42105").Add(Qlx.METADATA));
                }
 //               tc += (DBObject._Framing, tc.framing + new Framing(cx, nst));
                cx.Add(tc);
                cx.db += tc;
                cx.parse = op;
            }
            else if (tok == Qlx.DROP)
            {
                ...
                if (StartMetadata(Qlx.COLUMN))
                {
                    ...
                    var pm = new PMetadata(nm, 0, tc, s, tm, nst, tr.nextPos, cx);
                    tc = (TableColumn)(cx.Add(pm) ?? throw new DBException("42105").Add(Qlx.COLUMN));
                }
            }
            return (Table)(cx._Ob(tb.defpos) ??tb);
        }
        /// <summary>
		/// AlterType = TO id
        ///     |   ADD ( Member | Method )
        ...
        void ParseAlterType()
        {
            ...
            cx._Ob(ro.dbobjects[id.ident]) is not UDType tp || tp.infos[ro.defpos] is not ObInfo oi)
            ...
            if (tok == Qlx.TO)
            {
                ...
                Mustbe(Qlx.Id);
                cx.Add(new Change(ro.dbobjects[id.ident], ni.ident, tr.nextPos, cx));
            }
            else if (tok == Qlx.SET)
            {
                Next();
                if (Match(Qlx.UNDER))
                {
                    ...
                    if (cx.role.dbobjects.Contains(ui.ident)
                        && cx._Ob(cx.role.dbobjects[ui.ident]) is UDType tu)
                ...
                else
                {
                    ...
                    if (cx._Ob(ts) is not TableColumn tc)
            ...
            else if (tok == Qlx.DROP)
            {
                ...
                else
                {
                    ...
                    if (MethodModes())
                    {
                        ...
                        if (mn.name is not Ident nm
                            || cx._Ob(oi?.methodInfos?[nm.ident]?[Database.Signature(mn.ins)] ?? -1L) is not Method mt)
                    ...
                    {
                        if (cx._Ob(tp.ColFor(cx, id.ident)) is not TableColumn tc)
                            throw new DBException("42133", id).Mix()
                                .Add(Qlx.TYPE, new TChar(id.ident));
                        ParseDropAction();
            ...
            else if (Match(Qlx.ADD))
            {
                ...
                if (StartMetadata(Qlx.TYPE))
                    ParseTypeMetadata(id, tp, BTree<long, object>.Empty);
                else
                {
                    var m = BTree<long, object>.Empty + (Domain.Kind, tp.kind)
                        + (TableColumn._Table, tp.defpos);
                    var (nm, dm, s, md) = ParseMember(m);
                    var c = new PColumn2(tp, nm.ident, dm, "",
                        md[Qlx.OPTIONAL]?.ToBool() ?? true, Generation.No, tr.nextStmt, tr.nextPos, cx);
                    cx.Add(c);
                    var tc = (TableColumn)(cx.obs[c.defpos] ?? throw new DBException("42105").Add(Qlx.COLUMN));
                    if (md != TMetadata.Empty)
                        cx.Add(new PMetadata(nm.ident, 0, tc, s, md, nst, tr.nextPos, cx));
                }
            }
            else if (Match(Qlx.ALTER))
            {
            ...
            else if (StartMetadata(Qlx.TYPE))
                ParseTypeMetadata(id, tp, BTree<long, object>.Empty);
            cx.parse = op;
        }
        /// <summary>
            ...
            for (; ; )
            {
                ...
                if (cx._Ob(tc.tabledefpos) is Domain td && ci.name is not null)
                    tc = (TableColumn)(cx.Add(new Alter3(tc.defpos, ci.name, td.PosFor(cx, ci.name),
                         (Table?)cx._Ob(tc.tabledefpos) ?? throw new DBException("42105").Add(Qlx.DOMAIN),
                         tc.domain, "", tc.optional,tc.generation, nst,
		...
		void ParseDropStatement()
        {
            ...
            if (Match(Qlx.ORDERING))
            {
                ...
                var tp = cx._Ob(cx.role.dbobjects[o.ident]) as Domain ??
    ...
    Qlx.EXTRACT, Qlx.FIRST_VALUE, Qlx.FUSION, Qlx.GROUPING, Qlx.HTTP, Qlx.JSON,
    ...
    Qlx.VERSIONING);
		...
		Domain ParseDataType(Ident? pn = null)
        {
            ...
            if (Match(Qlx.TABLE, Qlx.ROW, Qlx.TYPE, Qlx.LPAREN))// anonymous row type
            {
                ...
                if (Match(Qlx.TABLE, Qlx.ROW, Qlx.TYPE))
                    ...
                    tp = Qlx.TYPE;
                var m = new BTree<long,object>(Domain.Kind, tp);
                if (pn!=null)
                    m += (DBObject._Ident, pn);
                if (tok == Qlx.LPAREN)
                    r = ParseRowTypeSpec(m); 
            }
            else
            {
                var br = false;
                if (br = Match(Qlx.REFERENCES))
                    Next();
                var cn = new Ident(this);
                if (tok == Qlx.Id && cx.GetObject(cn.ident) is Table ut)
                {
                    Next();
                    r = ut;
                } 
            ...
            if (r == null || r.kind == Qlx.Null || r.kind == Qlx.CONTENT)
            {
                var o = new Ident(this);
                Next();
                r = (Domain)(cx._Ob(cx.role.dbobjects[o.ident]) ?? Domain.Content);
            }
        ...
        Domain ParseStandardDataType()
        {
            ...
            if (cx._Ob(cx.db.Find(r)?.defpos ?? -1L) is Domain nr
                && r.CompareTo(nr) == 0)
                return (Domain)cx.Add(nr);
            if (cx.newTypes.Contains(r) && cx.obs[cx.newTypes[r]] is Domain ns)
        ...
        /// <returns>the modified object type</returns>
        ...
        internal Domain ParseRowTypeSpec(BTree<long,object> m)
        {
            var k = (Qlx)(m[Domain.Kind] ?? Qlx.NO);
            var nst = cx.db.nextStmt;
            Ident? pn = (Ident?)m[DBObject._Ident];
            var under = (CTree<Domain,bool>)(m[Domain.Under]??CTree<Domain,bool>.Empty);
            var dt = (Domain)(m[DBObject._Domain]??Domain.Null);
            if (tok == Qlx.Id)
            {
                var id = new Ident(this);
                Next();
            ...
            {
                // no longer require uniqueness in hierarchy (despite rules in GQL v1)
                var hm = (b.key() as Table)?.names ?? Names.Empty;
                sm += hm;
                ...
                if (b.key() is Table be && be.colRefs.Count > 1L)
                    k = Qlx.EDGETYPE;
                if (cx._Ob(b.key().defpos) is Table && k != Qlx.EDGETYPE)
                    k = Qlx.NODETYPE;
            }
            var sl = lxr.start;
            Mustbe(Qlx.LPAREN);
            for (var n = 0; ; n++)
            {
                ...
                if (pn != null)
                    mm += (DBObject._Ident, pn);
                var mi = ParseMember(mm);
                ns += mi;
                if (tok != Qlx.COMMA)
                    break;
                Next();
            ...
            if (k == Qlx.VIEW)
                ...
                dt = Domain.TypeSpec.New(pn, under, st, cx) ?? throw new PEException("PE40407");
            m += (DBObject._Domain, dt);
            if (k==Qlx.TYPE || k==Qlx.TABLE)
                m += (TableColumn._Table, dt.defpos);
            if (k != Qlx.ROW)
                pn ??= new Ident("", lp);
            ...
            for (var b = ns.First(); b != null; b = b.Next(), j++)
            {
                ...
                if ((k == Qlx.TYPE || k == Qlx.NODETYPE || k == Qlx.EDGETYPE) && pn != null)
                {
                    //ms += (nm.uid, dm);
                    var np = cx.db.nextPos;
                    var pc = new PColumn3((Table)dt, nm.ident, dm, "", md, nst, np, cx);
                    cx.Add(pc);
                    ms += (pc.defpos, dm);
                    sm += (nm.ident, (nm.lp, pc.defpos));
                    rt += ((int)rt.Count,pc.defpos);
                    cx.names += (pn.ident, (nm.lp, pc.defpos));
                    sb.Append(cm); cm = ',';
                    sb.Append(nm.ident);
                }
            ...
            if (k == Qlx.LPAREN && cx.role.dbobjects[tn] is long p && p > 0L)
                return (Domain)(cx._Ob(p) ?? throw new DBException("42105").Add(Qlx.DOMAIN));
            ...
            for (var a = under?.First(); a != null; a = a.Next())
                if (cx._Ob(a.key().defpos) is Table tu && tu.infos[tu.definer] is ObInfo ti)
                {
                    for (var b = tu.indexes.First(); b != null; b = b.Next())
                        for (var c = b.value().First(); c != null; c = c.Next())
                            if (cx._Ob(c.key()) is Level3.Index x && dt is Table t)
                            {
                                cx.Add(new PIndex(t.name + "." + x.name, t, b.key(), 0, cx.db.nextPos, cx.db));
                                r = (Domain)(cx._Ob(r.defpos) ?? dt);
                            }
		...
		(Ident, Domain, string, TMetadata) ParseMember(BTree<long,object> mm)
        {
            var pn = (Ident?)mm[DBObject._Ident]; // the parent of this member
            var dt = (Domain)(mm[DBObject._Domain] ?? Domain.TableType);
            var k = (Qlx)(mm[Domain.Kind] ?? Qlx.NO);
            Ident? n = null;  // the name of the member
            StartDataType();
            if (tok == Qlx.Id)
            {
                n = new Ident(this);
            ...
            else if (Match(Qlx.FROM, Qlx.TO, Qlx.WITH))
            {
                n = new Ident(this);
                n = new Ident(tok.ToString(),n.uid);
                tok = Qlx.Id;
                Next();
            }
            ...
            var m = new BTree<long, object>(DBObject._Domain, dm);
            if (cx._Ob((long)(mm[TableColumn._Table] ?? -1L)) is Table tb)
                m += (TableColumn._Table, tb.defpos);
            if (Match(Qlx.DEFAULT))
            {
                Next();
                if (Match(Qlx.VALUE))
                    Next();
                int st = lxr.start;
                var v = dm.Parse(new Scanner(-1L, lxr.val.ToString().ToCharArray(), 0, cx))
                    ?? throw new DBException("42161", Qlx.VALUE);
                var ds = new string(lxr.input, st, lxr.start - st);
                Next();
                dm = dm + (Domain.Default, v) + (Domain.DefaultString, ds);
            }
            if (Match(Qlx.COLLATE))
                dm += (Domain.Culture, ParseCollate());
            string ms = "";
            var md = TMetadata.Empty;
            if (StartMetadata(Qlx.COLUMN) ||Match(Qlx.PRIMARY, Qlx.FOREIGN, Qlx.UNIQUE, Qlx.KEY))
                (ms,md) = ParseMetadata(Qlx.COLUMN,m + 
        ...
        /// <summary>
        /// Rename =SET ObjectName[Signature] TO id .
        /// </summary>
        /// <returns>the executable</returns>
		void ParseSqlSet()
        {
		...
		internal QueryStatement ParseSelectStatement(BTree<long, object> m)
        {
            var on = cx.names;
            cx.names = cx.anames;
            ...
            var qs = new QueryStatement(cx.GetUid(),
                m+(QueryStatement.Result,left.defpos)+(DBObject._Domain,left));
            cx.Add(qs);
            cx.result = left;
            cx.names += on;
            return qs;
		...
		RowSet ParseRowSetTerm(BTree<long, object> m)
        {
            var xp = m[DBObject._Domain] as Domain ?? Domain.Null;
            RowSet left, right;
		...
		RowSet ParseQueryPrimary(BTree<long, object> m)
        {
            var xp = m[DBObject._Domain] as Domain ?? Domain.Null;
            var lp = LexDp();
            var ap = (long)(m[DBObject.Scope]?? 0L);
            ...
            switch (tok)
            {
                ...
                case Qlx.VALUES:
                    ...
                    while (sep == Qlx.COMMA)
                    {
                        ...
                        var llp = LexDp();
                        Mustbe(Qlx.LPAREN);
                        var x = ParseSqlValueList(m+(QueryList,true));
                        Mustbe(Qlx.RPAREN);
		...
		Domain ParseOrderClause(RowSet rs, Domain ord, BTree<long, object> m)
        {
            if (tok != Qlx.ORDER)
                return ord;
            var on = cx.names;
            ...
            if (lb)
                Next();
            var mi = m +  (DBObject._Domain, Domain.Content);
        ...
        /// <returns></returns>
        Domain ParseOrderClause(BTree<long,object> m)
        {
            var ord = (Domain)(m[DBObject._Domain] ?? Domain.Row);
            if (tok != Qlx.ORDER)
                return ord;
            Next();
            Mustbe(Qlx.BY);
            var mm = new BTree<long, object>(NestedStatement.WfOK, false);
            var bs = BList<DBObject>.Empty;
            for (var b = ord.rowType.First(); b != null; b = b.Next())
                bs += cx._Ob(b.value()) ?? SqlNull.Value;
            bs += cx._Ob(ParseOrderItem(mm)) ?? SqlNull.Value;
            while (tok == Qlx.COMMA)
            {
                Next();
                bs += cx._Ob(ParseOrderItem(mm)) ?? SqlNull.Value;
		...
		internal long ParseOrderItem(BTree<long,object> m)
        {
		...
		RowSet ParseQueryExpression(BTree<long,object> m)
        {
            var xp = m[DBObject._Domain] as Domain ?? Domain.Content;
            var id = new Ident(this);
            if (!m.Contains(DBObject.Scope))
        ...
        {
            Domain cd = Domain.Content;
            if (b?.value() is long p)
                cd = dm.representation[p] ?? cd;
            var cn = c?.value() ?? "";
            return ParseSelectItem(q, m + (DBObject._Domain, cd) + (ObInfo.Name, cn));
        }
        /// <summary>
		/// SelectItem = * | (Scalar [AS id ]) | (RowValue [.*] [AS IdList]) .
        /// </summary>
        /// <param name="q">the query being parsed</param>
        /// <param name="t">the expected object type for the query</param>
        ...
        QlValue ParseSelectItem(long q, BTree<long, object> m)
        {
            Ident alias;
            QlValue v;
            if (tok == Qlx.TIMES)
            ...
            {
                v = ParseSqlValue(m + (NestedStatement.WfOK,true));
                if (v is SqlReview)
		...
		SelectRowSet ParseTableExpression(long lp, BTree<long, object> m)
        {
            ...
            return ParseSelectRowSet(
                new BTree<long,object>(DBObject._Domain,new SelectRowSet(ap,lp, cx, dm, fm, mf)));
        }
        SelectRowSet ParseSelectRowSet(BTree<long,object> m)
        {
            var r = m[DBObject._Domain] as SelectRowSet?? throw new PEException("PE20951");
            var gc = Domain.Null;
            ...
            {
                var wc = ParseWhereClause(m) ?? throw new DBException("42161", "condition");
            ...
            if (tok == Qlx.HAVING)
            {
                ...
                if (r.aggs.Count != 0)
                    m += (RowSet.Having, ParseHavingClause(m));
                else
                {
 ...
 /*           for (var b = cx.undefined.First(); b != null; b = b.Next())
		...
		RowSet ParseFromClause(long dp, BTree<long, object> m)
        {
            if (tok == Qlx.FROM)
            {
                Next();
            ...
            {
                var ap = (long)(m[DBObject.Scope] ?? 0L);
                var dm = m[DBObject._Domain] as Domain ?? Domain.TableType;
        ...
        RowSet ParseTableReference(long st, BTree<long, object> m)
        {
...
/*                if (a is JoinRowSet ja)
		...
		RowSet ParseTableReferenceItem(long st, BTree<long, object> m)
        {
            var dm = m[DBObject._Domain] as Domain ?? Domain.TableType;
            var ap = (long)(m[DBObject.Scope] ?? 0L);
            RowSet rf;
            ...
            if (Match(Qlx.ROWS)) // Pyrrho specific
            {
                Next();
                Mustbe(Qlx.LPAREN);
                var v = ParseSqlValue(m+(DBObject._Domain,Domain.Ref));
                QlValue w = SqlNull.Value;
                if (tok == Qlx.COMMA)
                {
                    Next();
                    w = ParseSqlValue(m+(DBObject._Domain, Domain.Ref));
                }
                Mustbe(Qlx.RPAREN);
                Match(Qlx.COMMA, Qlx.CROSS, Qlx.NATURAL, Qlx.JOIN, Qlx.INNER, Qlx.LEFT, Qlx.RIGHT, Qlx.FULL);
                if (tok == Qlx.Id || (tok == Qlx.AS && lxr.ch != '/' && lxr.ch != '.')) // catref)
                {
                    if (tok == Qlx.AS)
                        Next();
                    new Ident(this);
            ...
            else if (Match(Qlx.TABLE))
            {
                ...
                if (tok != Qlx.RPAREN)
                    for (; ; )
                    {
                        r += cx.Add(ParseSqlValue(BTree<long, object>.Empty)).defpos;
                        if (tok == Qlx.RPAREN)
                            break;
                        Mustbe(Qlx.COMMA);
                    }
        ...
        RowSet _From(Ident ic, DBObject ob, Domain dm, Grant.Privilege pr, long ap, string? a = null)
        {
            ...
            if (ob != null)
            {
                if (ob is View ov)
                {
                    RowSet? ur = (cx._Ob((ov as RestView)?.usingTable ?? -1L) is not Table ut) ? 
            ...
            for (var b = cx.undefined.First(); b != null; b = b.Next())
            {
                if (b.key() is long k && cx.obs[k] is DBObject uo)
                {
                    if (uo is SqlMethodCall um)
                    {
                        ...
                        && ui.methodInfos[um.name] is CTree<CList<Domain>, long> st)
                        {
                                ...
                                if (c.key().Length == dl.Length)
                                {
                                    ...
                                    for (var d = c.key().First(); d != null && db != null;
                                        ...
                                        if (!db.value().CanTakeValueOf(d.value()))
                                            goto next;
                                    if (c.value() is long m && cx._Ob(m) is Method me)
                                    {
                                        cx.undefined -= k;
        ...
        PeriodSpec ParsePeriodSpec()
        {
            ...
            switch (tok)
            {
                case Qlx.AS:
                    Next();
                    Mustbe(Qlx.OF);
                    t1 = ParseSqlValue(m+(DBObject._Domain, Domain.UnionDate));
                    break;
                case Qlx.BETWEEN:
                    Next();
                    kn = Qlx.ASYMMETRIC;
                    ...
                    t1 = ParseSqlValueTerm(m+(DBObject._Domain, Domain.UnionDate));
                    Mustbe(Qlx.AND);
                    t2 = ParseSqlValue(m+(DBObject._Domain, Domain.UnionDate));
                    break;
                case Qlx.FROM:
                    Next();
                    t1 = ParseSqlValue(m+(DBObject._Domain, Domain.UnionDate));
		...
		ObInfo? ParseCorrelation(Domain xp)
        {
            if (tok == Qlx.Id || (tok == Qlx.AS && lxr.ch != '/' && lxr.ch != '.')) // catref)
            {
                ...
                if (tok == Qlx.LPAREN)
                {
                    ...
                    xp = (cs==CTree<int,long>.Empty)?Domain.TableType:new Domain(cx.GetUid(), cx, Qlx.TABLE, rs, cs);
                    cx.Add(xp);
        ...
        RowSet ParseJoinPart(long dp, RowSet fi, BTree<long, object> m)
        {
            var left = fi;
            Qlx jkind;
            RowSet right;
      ...
      //              jm += (JoinRowSet.JoinUsing, cs);
                ...
                {
                    Mustbe(Qlx.ON);
                    var oc = ParseSqlValue(m+(DBObject._Domain,Domain.Bool)).Disjoin(cx);
       ...
       //             var jf = (CTree<long, CTree<long, bool>>)(m[JoinRowSet.JoinFactors] ?? CTree<long, CTree<long, bool>>.Empty);
        ...
        GroupSpecification ParseGroupingElement(GroupSpecification g, RowSet rs, ref bool simple)
        {
            if (Match(Qlx.Id))
            {
                var c = ParseSqlValue(BTree<long,object>.Empty);
                for (var b = rs.rowType.First(); b != null; b = b.Next())
		...
		CTree<long, bool> ParseHavingClause(BTree<long,object> m)
        {
            var dm = m[DBObject._Domain] as Domain ?? throw new PEException("PE20871");
            var r = CTree<long, bool>.Empty;
            if (tok != Qlx.HAVING)
                return r;
            Next();
            r = ParseSqlValueDisjunct(m+(DBObject._Domain, Domain.Bool)+(DBObject.HavingDom,dm));
            if (tok != Qlx.OR)
                return r;
            var left = Disjoin(r);
            while (tok == Qlx.OR)
            {
                ...
                left = (QlValue)cx.Add(new SqlValueExpr(lp, cx, Qlx.OR, left,
                    Disjoin(ParseSqlValueDisjunct(m+(DBObject._Domain, Domain.Bool)+(DBObject.HavingDom, dm))), Qlx.NO));
            }
            r += (left.defpos, true);
        ...
        /// </summary>
        /// <returns>The QlValue (Boolean expression)</returns>
		CTree<long, bool>? ParseWhereClause(BTree<long,object> m)
        {
            if (!Match(Qlx.WHERE))
                return null;
            Next();
            return _ParseWhereClause(m);
        }
        CTree<long, bool>? _ParseWhereClause(BTree<long, object> m)
        { 
            cx.done = ObTree.Empty;
            var ro = cx.result;
            var ap = (long)(m[DBObject.Scope] ?? -1L);
            var left = ParseSqlValue(m+(DBObject._Domain, Domain.Bool)+(DBObject._From, ap));
            if (ap >= 0)
                cx.result = ro;
        ...
        internal void ParseSqlInsert(string sql)
        {
            ...
            if (LexDp() > Transaction.TransPos)  // if sce is uncommitted, we need to make space above nextIid
                cx.db += (Database.NextId, cx.db.nextId + sql.Length);
            ParseSqlInsert(BTree<long,object>.Empty);
        }
        /// <summary>
        ...
        Executable ParseSqlInsert(BTree<long,object> m)
        {
            ...
            cx.defs += (lp, cx.names);
            cx.names = cx.anames;
            var fm = ParseTableReference(ic.uid, m + (DBObject._Domain, Domain.TableType)
                +(DBObject.Scope, LexLp()));
            cx.Add(fm);
            if (fm is not TableRowSet && !cx.names.Contains(ic.ident))
                cx.Add(ic.ident, ap, fm);
            Domain? cs = null; // the keymap list, which may be different from the TableRowSet fm
            QlValue? sv = null;            
            if (tok==Qlx.LBRACE) // a document: if so, identify keymap from the doc
            {
                var sd = ParseSqlValueItem(m + (DBObject._Domain, fm)) as SqlRow ??
            ...
            if (Match(Qlx.DEFAULT))
            {
                ...
                sv ??= ParseSqlValue(m+(DBObject._Domain,fm));
            ...
            cx.defs -= lp;
            cx.names += on;
            if (cx.parse==ExecuteStatus.Http && cx.db is Transaction tr)
                cx = tr.Execute(s, cx);
            cx.exec = s;
        ...
        /// </summary>
        /// <returns>the executable</returns>
		Executable ParseSqlDelete(BTree<long,object> m)
        {
            var lp = LexDp();
            var ap = LexLp();
            var cc = Match(Qlx.DETACH)?PIndex.ConstraintType.CascadeDelete:PIndex.ConstraintType.NoType;
            if (Match(Qlx.DETACH, Qlx.NODETACH))
                Next();
            Mustbe(Qlx.DELETE);
            if (!Match(Qlx.WITH, Qlx.FROM))
            {
                var de = DeleteNode(cc!=0);
                if (!Match(Qlx.COMMA))
                    return de;
                ...
                {
                    Next();
                    ds += DeleteNode(cc!=0).defpos;
                }
                return (Executable)cx.Add(new NestedStatement(cx.GetUid(),
                    new BTree<long, object>(NestedStatement.Stms, ds)));
            }
            Mustbe(Qlx.FROM);
            Ident ic = new(this);
            // cx.IncSD(ic);
            ...
            if (ParseWhereClause(m+(DBObject._Domain,rs)) is CTree<long, bool> wh)
            {
                rs = (RowSet?)cx.obs[rs.defpos] ?? throw new PEException("PE2007");
                rs = rs.Apply(RowSet.E + (RowSet._Where, rs.where + wh), cx);
            }
            if (Match(Qlx.RESTRICT,Qlx.CASCADE))
            {
                cc |= (tok == Qlx.RESTRICT) ? PIndex.ConstraintType.RestrictDelete : PIndex.ConstraintType.CascadeDelete;
                Next();
            }
            if (cc != PIndex.ConstraintType.NoType)
                qs += (QuerySearch.Action, cc);
            cx._Add(rs);
            cx.result = rs;
            if (tok != Qlx.EOF)
            ...
            cx.defs -= lp;
            cx.names += on;
            if (cx.parse==ExecuteStatus.Http)
        ...
        internal Context ParseSqlUpdate(Context cx, string sql)
        {
            ...
            if (LexDp() > Transaction.TransPos)  // if sce is uncommitted, we need to make space above nextIid
                cx.db += (Database.NextId, cx.db.nextId + sql.Length);
            return ParseSqlUpdate(BTree<long,object>.Empty).Item1;
        }
        /// <summary>
		/// Update = UPDATE [XMLOption] Table_id Assignment [WhereClause] .
        /// </summary>
        /// <returns>The UpdateSearch</returns>
		(Context, Executable) ParseSqlUpdate(BTree<long,object>m)
        {
            var st = LexDp();
            Next();
            Ident ic = new(this);
            ...
            if (Match(Qlx.ON))
            {
                var ct = ParseReferentialAction(PIndex.ConstraintType.NoType);
                us += (QuerySearch.Action, ct);
            }
            cx.result = rs;
            if (cx.parse == ExecuteStatus.Http && cx.db is Transaction tr)
                cx = tr.Execute(us, cx);
        ...
        internal CTree<UpdateAssignment, bool> ParseAssignments(string sql)
        {
            lxr = new Lexer(cx, sql);
            tok = lxr.tok;
            return ParseAssignments(BTree<long,object>.Empty);
        }
        /// <summary>
        /// Assignment = 	SET Target '='  TypedValue { ',' Target '='  TypedValue }
        /// </summary>
        /// <returns>the tree of assignments</returns>
		CTree<UpdateAssignment, bool> ParseAssignments(BTree<long,object> m)
        {
            var r = CTree<UpdateAssignment, bool>.Empty + (ParseUpdateAssignment(m), true);
            while (tok == Qlx.COMMA)
            {
                Next();
                r += (ParseUpdateAssignment(m), true);
            }
            return r;
        }
        /// <summary>
		...
		UpdateAssignment ParseUpdateAssignment(BTree<long,object> m)
        {
            QlValue vbl;
            QlValue val;
            Match(Qlx.SECURITY);
            ...
            else vbl = ParseVarOrColumn(m+(DBObject.Scope,LexDp()));
            Mustbe(Qlx.EQL);
            val = ParseSqlValue(m+(DBObject._Domain,vbl.domain));
        ...
        /// <param name="t">the expected object type if any</param>
        /// <returns>the QlValue</returns>
        internal QlValue ParseSqlValue(string s, BTree<long, object> m)
        {
            lxr = new Lexer(cx, s);
            tok = lxr.tok;
            return ParseSqlValue(m);
        }
        internal QlValue ParseSqlValue(Ident ic, BTree<long, object> m)
        {
            lxr = new Lexer(cx, ic.ident, ic.uid);
            tok = lxr.tok;
            return ParseSqlValue(m);
        }
        internal QlValue ParseSqlValue(BTree<long, object> m)
        {
            var xp = m[DBObject._Domain] as Domain ?? Domain.Content;
            if (Match(Qlx.MATCH))
            ...
            if (Match(Qlx.PERIOD))
            {
                Next();
                Mustbe(Qlx.LPAREN);
                var op1 = ParseSqlValue(m+(DBObject._Domain,Domain.UnionDate));
                Mustbe(Qlx.COMMA);
                var op2 = ParseSqlValue(m+(DBObject._Domain, Domain.UnionDate));
                Mustbe(Qlx.RPAREN);
            ...
            {
                var mm = m + (DBObject._Domain, Domain.Content);
                left = Disjoin(ParseSqlValueDisjunct(mm));
                while (left.domain.kind == Qlx.BOOLEAN && tok == Qlx.OR)
                {
                    mm += (DBObject._Domain, left.domain);
                    var lp = LexDp();
                    Next();
            ...
            else if (xp.kind == Qlx.REF && cx.conn.refIdsToPos)
            {
                ...
                if (m[RowSet.Target] is long fp && cx.db.objects[fp] is Table ft
                    ...
                    && cx.obs[p] is QlInstance qi
                    && ft.infos[cx.role.defpos] is ObInfo fi
                    && fi.metadata[Qlx.REFERENCES] is TSet ts)
                    for (var b = ts.First(); b != null; b = b.Next())
                        if (b.Value() is TConnector bc && bc.cp == qi.sPos
                        && bc.rd is Table bt
                        && bt.EqualOrStrongSubtypeOf(et))
                        {
                            et = bt;
                            break;
                        }
        ...
        CTree<long, bool> ParseSqlValueDisjunct(BTree<long, object> m)
        {
        ...
        QlValue ParseSqlValueConjunct(BTree<long, object> m)
        {
            var wfok = (bool)(m[NestedStatement.WfOK] ?? false);
            var fp = (long)(m[DBObject._From] ?? -1L);
            var left = ParseSqlValueExpression(m);
            ...
            {
                Next();
                if (cx._Ob(cx.role.dbobjects[lxr.val.ToString()]) is Table ft)
                {
                    Next();
                    left = new TypePredicate(cx.GetUid(), left, true, new BList<Domain>(ft));
                }
            } else if (m[DBObject._Domain] is Domain xp)
            {
                var nd = left.domain.LimitBy(cx, left.defpos, xp);
                if (nd != left.domain && nd != null)
                    left += (DBObject._Domain, nd);
            }
            if (m[DBObject.HavingDom] is Domain dm)
                left = left.Having(cx, dm, LexLp());
            return (QlValue)cx.Add(left);
        }
        QlValue ParseSqlValueExpression(BTree<long, object> m)
        {
                ...
                && Match(Qlx.PLUS, Qlx.MINUS))
            {
                ...
                left = (QlValue)cx.Add(new SqlValueExpr(lp, cx, op, left, x, Qlx.NO, m));
            }
            return (QlValue)cx.Add(left);
        }
        /// <summary>
        ...
        QlValue ParseSqlValueTerm(BTree<long, object> m)
        {
            var wfok = (bool)(m[NestedStatement.WfOK] ?? false);
            var fp = (long)(m[DBObject._From] ?? -1L);
            ...
            if (sign)
                left = new SqlValueExpr(lp, cx, Qlx.MINUS, null, left, Qlx.NO,m)
            ...
            if (Match(Qlx.CONTAINS, Qlx.OVERLAPS, Qlx.EQUALS, Qlx.PRECEDES, Qlx.SUCCEEDS))
            {
                ...
                return (QlValue)cx.Add(new SqlValueExpr(lp, cx,
                    op, left, ParseSqlValueFactor(m+(DBObject._Domain,left.domain)
                    +(NestedStatement.WfOK,wfok)), imm,m));
            }
            while (Match(Qlx.TIMES, Qlx.DIVIDE, Qlx.LIST, Qlx.MULTISET, Qlx.SET))
            {
                ...
                left = (QlValue)cx.Add(new SqlValueExpr(lp, cx, op, left,
                    ParseSqlValueFactor(m+(DBObject._Domain, left.domain)
                    +(NestedStatement.WfOK, wfok)), md, m));
            }
            return (QlValue)cx.Add(left);
        }
        /// <summary>
        ...
        /// <param name="wfok">whether to allow a window function</param>
        /// <returns>the QlValue</returns>
		QlValue ParseSqlValueFactor(BTree<long, object> m)
        {
            ...
            while (Match(Qlx.CONCATENATE, Qlx.COLON))
            {
                ...
                var fp = (long)(m[DBObject._From] ?? -1L);
                var right = ParseSqlValueEntry(m+(DBObject._Domain,left.domain)
                    +(NestedStatement.WfOK,wfok) + (DBObject._From,fp));
                left = new SqlValueExpr(LexDp(), cx, op, left, right, Qlx.NO, m);
                cx.Add(left);
            }
            return left;
        }
        ...
        /// <param name="wfok">whether to allow a window function</param>
        /// <returns>the sqlValue</returns>
		QlValue ParseSqlValueEntry(BTree<long, object> m)
        {
            var wfok = m[NestedStatement.WfOK] ?? false;
            var fp = (long)(m[DBObject._From] ?? -1L);
            ...
            while (tok == Qlx.DOTTOKEN || tok == Qlx.LBRACK)
                if (tok == Qlx.DOTTOKEN)
                {
                    ...
                    if (tok == Qlx.LPAREN)
                    {
                        ...
                        if (cx._Ob(ut.defpos) is not Domain u || u.infos[cx.role.defpos] is not ObInfo oi)
                            throw new DBException("42105").Add(Qlx.TYPE);
                        var ar = cx.db.Signature(cx,ps);
                        var pr = cx._Ob(oi.methodInfos[n.ident]?[ar] ?? -1L) as Method
                    ...
                    else
                    {
                        ...
                        left = (QlValue)cx.Add(new SqlValueExpr(cx.GetUid(), cx, Qlx.DOTTOKEN, left, el, Qlx.NO, m));
                    }
                }
                else // tok==Qlx.LBRACK
                {
                    ...
                    left = new SqlValueExpr(lp, cx, Qlx.LBRACK, left,
                        ParseSqlValue(m + (DBObject._Domain,Domain.Int) + (DBObject._From,fp)), Qlx.NO);
                    Mustbe(Qlx.RBRACK);
                }
            var xp = (Domain)(m[DBObject._Domain] ?? Domain.Content);
            if (tok == Qlx.IS)
            {
            ...
            if (tok == Qlx.BETWEEN)
            {
                ...
                Next();
                var od = left.domain;
                var lw = ParseSqlValueTerm(m+(DBObject._Domain,od)+(DBObject._From,fp));
                Mustbe(Qlx.AND);
                var hi = ParseSqlValueTerm(m+(DBObject._Domain, od)+(DBObject._From, fp));
            ...
            if (tok == Qlx.LIKE)
            {
                ...
                LikePredicate k = new(lp, cx, left, !invert, 
                    ParseSqlValue(m+(DBObject._Domain,Domain.Char)+(DBObject._From, fp)), null);
                if (Match(Qlx.ESCAPE))
                {
                    Next();
                    k += (cx, LikePredicate.Escape, 
                        ParseSqlValueItem(m+(DBObject._Domain, Domain.Char)+(DBObject._From,fp))?.defpos ?? -1L);
                }
            ...
            if (tok == Qlx.IN)
            {
                ...
                if (tok == Qlx.LPAREN || tok == Qlx.LBRACK)
                {
                    ...
                    {
                        RowSet rs = ParseQueryExpression(m+(DBObject._Domain,Domain.TableType));
                        cx.Add(rs);
                        n += (cx, QuantifiedPredicate._Select, rs.defpos);
                    }
                    else
                        n += (cx, QuantifiedPredicate.Vals, 
                            ParseSqlValueList(m+(DBObject._Domain,left.domain)));
                    Mustbe((tp == Qlx.LPAREN) ? Qlx.RPAREN : Qlx.RBRACK);
                }
                else
                {
                    var vl = ParseSqlValue(m+
                        (DBObject._Domain, cx.Add(
                            new Domain(cx.GetUid(), Qlx.COLLECT, left.domain))));
                    n += (cx, QlValue.Val, vl.defpos);
                }
            ...
            if (Match(Qlx.MEMBER))
            {
                ...
                return (QlValue)cx.Add(new MemberPredicate(LexDp(), cx, left,
                    !invert, ParseSqlValue(m+(DBObject._Domain,dm))));
            }
            if (invert)
            {
        ...
        ///     | NaturalLogarithm | Next | Nullif | Percentile | Ref | PowerFunction | Rank | Regression | RowNumber | SquareRoot | StandardDeviation | Sum | Variance | HttpGet .
        ...
        internal QlValue ParseSqlValueItem(BTree<long, object> m)
        {
            ...
            Match(Qlx.SCHEMA, Qlx.LABELS, Qlx.ELEMENTID, Qlx.TYPE, // we may get these non-reserved words
                Qlx.TABLE, Qlx.EXTRACT, Qlx.FIRST_VALUE, Qlx.GROUPING, Qlx.HTTP, Qlx.LAST_VALUE, Qlx.LAST_DATA,Qlx.RANK,
    Qlx.CHAR_LENGTH, Qlx.WITHIN, Qlx.REF, Qlx.ROW_NUMBER, Qlx.SUBSTRING, Qlx.COLLECT, Qlx.INTERSECTION,
    Qlx.ELEMENT, Qlx.USER, Qlx.VERSIONING, Qlx.MULTISET, Qlx.LIST, Qlx.VALUES);
            ...
            if (Match(Qlx.Id, Qlx.FIRST, Qlx.NEXT, Qlx.LAST, Qlx.CHECK, Qlx.TYPE_URI)) // Id or pseudo ident
            {
                ...
                {
                    Next();
                    if (vr.name == null || cx._Ob(cx.role.dbobjects[vr.name]) is not Domain ut
                    ...
                    Mustbe(Qlx.RPAREN);
                    var n = cx.db.Signature(cx,ps);
                    var me = cx._Ob(oi.methodInfos[name.ident]?[n] ?? -1L) as Method
                ...
                case Qlx.LIST:
                    {
                        ...
                        if (Match(Qlx.LPAREN))
                        {
                            ...
                            {
                                var st = lxr.start;
                                var cs = ParseSelectStatement(m+(DBObject._Domain, Domain.Null));
                                Mustbe(Qlx.RPAREN);
                        ...
                        var v = ParseSqlValueList(m+(DBObject._Domain, et));
                        if (v.Length == 0)
                ...
                case Qlx.ARRAY:
                    {
                        ...
                        var k = (QlValue)cx.Add(ParseSqlValue(m+(DBObject._Domain,Domain.Int))
                            ??throw new DBException("42161","INT value",tok));
                        Mustbe(Qlx.EQL);
                        var v = (QlValue)cx.Add(ParseSqlValue(m+(DBObject._Domain, et))
                            ?? throw new DBException("42161", et, tok));
                        vs += (k.defpos, v.defpos);
                        while (Match(Qlx.COMMA))
                        {
                            Next();
                            k = (QlValue)cx.Add(ParseSqlValue(m+(DBObject._Domain, Domain.Int))
                                ?? throw new DBException("42161", "INT value", tok));
                            Mustbe(Qlx.EQL);
                            v = (QlValue)cx.Add(ParseSqlValue(m+(DBObject._Domain, et))
                                ?? throw new DBException("42161", et, tok));
                            vs += (k.defpos, v.defpos);
                        }
                        if (vs.Length == 0)
                ...
                case Qlx.SCHEMA:
                    {
                        ...
                        if (Match(Qlx.COLUMN))
                        {
                            ...
                            if (cx._Dom(tb.defpos) is not Domain ft ||
                                cx._Ob(ft.ColFor(cx, cn.ToString())) is not DBObject oc)
          ...
          /*      case Qlx.JSON:
                ...
                case Qlx.LPAREN:// subquery
                    {
                        ...
                        if (Match(Qlx.SELECT, Qlx.MATCH))
                        {
                            ...
                            cx.result = es.domain;
                            Mustbe(Qlx.RPAREN);
                            var qv = new QlValueQuery(cx.GetUid(), cx, es.domain, xp, es.defpos)
                                + (RowSet._Scalar, true);
                            cx.result = xp;
                            cx.defs = od;
                            cx.names = on;
                        ...
                        for (var i = 0; ; i++)
                        {
                            var it = ParseSqlValue(m+(DBObject._Domain,et ??
                ...
                case Qlx.NEW:
                    {
                        ...
                        Mustbe(Qlx.Id);
                        lp = LexDp();
                        if (cx._Ob(cx.role.dbobjects[o.ident]) is not Domain ut
                            || ut.infos[cx.role.defpos] is not ObInfo oi)
                            throw new DBException("42142").Mix();
                        Mustbe(Qlx.LPAREN);
                        var ps = ParseSqlValueList(m+(DBObject._Domain,ut));
                        var n = cx.db.Signature(cx, ps);
                        Mustbe(Qlx.RPAREN);
                        if (cx._Ob(oi.methodInfos[o.ident]?[n] ?? -1L) is not Method me)
                        {
                ...
                case Qlx.TABLE: // allowed by 6.39
                    {
                        Next();
                        var lf = ParseSqlValue(m+(DBObject._Domain,Domain.TableType));
                ...
                case Qlx.CASE:
                    {
                        Next();
                        QlValue? v = null;
                        var cp = new BTree<long,object>(DBObject._Domain,Domain.Bool);
                        Domain rd = Domain.Content;
                        if (tok != Qlx.WHEN)
                        {
                            v = ParseSqlValue(m);
                            cx.Add(v);
                            cp += (DBObject._Domain,v.domain);
                        }
            ...
            m += (DBObject._Domain, Domain.Content);
            lp = LexDp();
            StartStdFunctionRefs();
            switch (tok)
            {
                case Qlx.ABS:
                    {
                        ...
                        Next();
                        Mustbe(Qlx.LPAREN);
                        val = ParseSqlValue(m+(DBObject._Domain,Domain.UnionNumeric));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
                ...
                case Qlx.CARDINALITY:
                    {
                        ...
                        Next();
                        Mustbe(Qlx.LPAREN);
                        val = ParseSqlValue(m+(DBObject._Domain,Domain.Collection));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
                case Qlx.CAST:
                    {
                        ...
                        val = ParseSqlValue(m);
                        Mustbe(Qlx.AS);
                        op1 = (QlValue)cx.Add(new SqlTypeExpr(new Ident("",cx.GetUid()), ParseDataType()));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
                ...
                case Qlx.CHAR_LENGTH:
                    {
                        ...
                        Next();
                        Mustbe(Qlx.LPAREN);
                        val = ParseSqlValue(m+(DBObject._Domain,Domain.Char));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
                ...
                case Qlx.COUNT: // actually a special case: but deal with all ident-arg aggregates here
                    {
                        ...
                        if (tok == Qlx.FILTER)
                        {
                            ...
                            filter = ParseWhereClause(m);
                            Mustbe(Qlx.RPAREN);
                        }
                        if (Match(Qlx.OVER) && (bool)(m[NestedStatement.WfOK]??false)) // extension to GQL v1
                        {
                            Next();
                            if (tok == Qlx.Id)
                            {
                            ...
                            {
                                ws = ParseWindowSpecificationDetails(m);
                ...
                case Qlx.EXTRACT:
                    {
                        ...
                        Mustbe(Qlx.FROM);
                        val = ParseSqlValue(m+(DBObject._Domain, Domain.UnionDate));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
                ...
                case Qlx.WITH:
                    {
                        kind = tok;
                        var tb = cx._Ob((long)(m[TableColumn._Table] ?? -1L)) as Table ?? Domain.TableType;
                        if (tb.names.Contains(tok.ToString()))
                        {
                            lxr.val = new TChar(tok.ToString());
                            lxr.tok = Qlx.Id;
                            tok = Qlx.Id;
                            return ParseVarOrColumn(new BTree<long, object>(DBObject._Domain, tb));
                        }
                        break;
                    }
                ...
                case Qlx.MIN:
                    {
                        ...
                        Next();
                        Mustbe(Qlx.LPAREN);
                        val = ParseSqlValue(m+(DBObject._Domain, Domain.Comparable));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
                ...
                case Qlx.NULLIF:
                    {
                        ...
                        op1 = ParseSqlValue(m);
                        Mustbe(Qlx.COMMA);
                        op2 = ParseSqlValue(m+(DBObject._Domain,op1.domain));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
#if SIMILAR
                ...
                case Qlx.REF:
                    {
                        ...
                        {
                            Next();
                            op1 = ParseSqlValue(m+(DBObject._Domain,Domain.Int));
                            Mustbe(Qlx.IN);
                            op2 = ParseSqlValue(m);
                            Mustbe(Qlx.RPAREN);
                        }
                ...
                case Qlx.RECORD:
                    {
                        ...
                        Next();
                        Mustbe(Qlx.LPAREN);
                        op1 = ParseSqlValue(m+(DBObject._Domain,Domain.Char));
                        Mustbe(Qlx.COMMA);
                        val = ParseSqlValue(m+(DBObject._Domain, Domain.Int));
                        Mustbe(Qlx.RPAREN);
                        break;
                    }
#if OLAP
                ...
                case Qlx.ROW_NUMBER:
                    {
                        ...
                        if (tok == Qlx.RPAREN)
                        {
                            ...
                            {
                                ws = ParseWindowSpecificationDetails(BTree<long,object>.Empty);
                ...
                case Qlx.SUBSTRING:
                    {
                        ...
                        if (kind == Qlx.SUBSTRING)
                        {
...
#endif
                            {
                                ...
                                {
                                    Next();
                                    op2 = ParseSqlValue(m+(DBObject._Domain, Domain.Int));
                                }
                            }
                        }
                        Mustbe(Qlx.RPAREN);
                ...
                case Qlx.VECTOR:
                    ...
                    if (Match(Qlx.COMMA))
                    {
                        ...
                        {
                            Next();
                            op2 = (QlValue)cx.Add(new SqlTypeExpr(new Ident("", cx.GetUid()), ParseDataType()));
                        }
                    }
                    Mustbe(Qlx.RPAREN);
                    break;
                case Qlx.VECTOR_DIMENSION_COUNT:
                    ...
                    Next();
                    Mustbe(Qlx.LPAREN);
                    val = ParseSqlValue(m+(DBObject._Domain, Domain.Vector));
                    Mustbe(Qlx.RPAREN);
                    break;
                case Qlx.VECTOR_DISTANCE:
                    ...
                    val = ParseSqlValue(m+(DBObject._Domain, Domain.Vector));
                    Mustbe(Qlx.COMMA);
                    op1 = ParseSqlValue(m+(DBObject._Domain, Domain.Vector));
                    Mustbe(Qlx.COMMA);
        ...
        WindowSpecification ParseWithinGroupSpecification()
        {
            ...
            if (r.order != Domain.Row)
                r += (cx, WindowSpecification.Order, 
                    ParseOrderClause(new BTree<long,object>(DBObject._Domain,r.order)));
            Mustbe(Qlx.RPAREN);
            return r;
        }
        /// <summary>
		...
		WindowSpecification ParseWindowSpecificationDetails(BTree<long,object> m)
        {
            Mustbe(Qlx.LPAREN);
            ...
            if (Match(Qlx.PARTITION))
            {
                ...
                var rt = dm.rowType;
                var d = 1;
                for (var b = ParseSqlValueList(m).First(); b != null; b = b.Next())
            ...
            if (tok == Qlx.ORDER)
            {
                var oi = ParseOrderClause(m + (DBObject._Domain,dm));
                oi = (Domain)cx.Add(oi);
            ...
            if (Match(Qlx.ROWS, Qlx.RANGE))
            {
                ...
                {
                    Next();
                    w += (WindowSpecification._Low, ParseWindowBound());
                    Mustbe(Qlx.AND);
                    w += (WindowSpecification._High, ParseWindowBound());
                }
                else
                    w += (WindowSpecification._Low, ParseWindowBound());
        ...
        /// </summary>
        /// <returns>the Executable</returns>
        Executable ParseProcedureCall(BTree<long, object> m)
        {
            var id = new Ident(this);
            Mustbe(Qlx.Id);
            Mustbe(Qlx.LPAREN);
        ...
        CList<long> ParseSqlValueList(BTree<long, object> m)
        {
            var xp = m[DBObject._Domain] as Domain ?? Domain.Content;
            var ql = m.Contains(QueryList); // cx.names can grow as we traverse the list
            var r = CList<long>.Empty;
            Domain ei;
            var on = cx.names;
            switch (xp.kind)
            {
                ...
                case Qlx.CONTENT:
                    for (; ; )
                    {
                        ...
                        if (tok == Qlx.COMMA)
                        {
                            if (!ql) cx.names = on; // name environment for each item needs to be the same
                            Next();
                        }
                        else break;
                    }
            ...
            for (; ; )
            {
                var v = (ei.Length > 0 && xp is not UDType) ?
                    ParseSqlRow(m+(DBObject._Domain,ei)) :
                    ParseSqlValue(m+(DBObject._Domain,ei));
                cx.Add(v);
                if (tok == Qlx.AS)
                {
                    Next();
        ...
        public SqlRow ParseSqlRow(BTree<long, object> m)
        {
            var xp = (m[DBObject._Domain] as Domain) ?? Domain.Row;
            var llp = LexDp();
            Mustbe(Qlx.LPAREN);
            var lk = CList<long>.Empty;
            var i = 0;
            var ts = xp as TableRowSet;
            for (var b = xp.rowType.First(); b != null && i < xp.rowType.Count
                && (xp.display==0||i<xp.display); b = b.Next(), i++)
                if (b.value() is long p && xp.representation[p] is Domain dt)
                {
                    if (cx.db.objects[ts?.sRowType[b.key()] ?? -1L] is TableColumn c && c.hide)
                        continue;
                    if (i > 0)
                        Mustbe(Qlx.COMMA);
                    if (ts is TableRowSet)
                        m += (RowSet.Target, ts.target);
                    var v = ParseSqlValue(m+(DBObject._Domain,dt)+(DBObject.Defpos,p));
                    cx.Add(v);
                    lk += v.defpos;
                }
            Mustbe(Qlx.RPAREN);
        ...
        /// <param name="result">the expected object type</param>
        /// <returns>the SqlRow</returns>
        public QlValue ParseSqlValueList(string sql, BTree<long, object> m)
        {
            lxr = new Lexer(cx, sql);
            tok = lxr.tok;
            if (tok == Qlx.LPAREN)
                return ParseSqlRow(m);
            return ParseSqlValueEntry(m);
        }
        /// <summary>
        ...
        (Ident, QlValue) GetDocItem(long pa,Domain lb,bool sch=false)
        {
            ...
            if (tok == Qlx.Id && !cx.Known(eq))
            {
                ...
                cx.Add(vv);
                cx.names += (q.ident, (q.lp,q.uid));
                if (cx.parsingGQL!=Context.ParsingGQL.No && !cx.anames.Contains(q.ident))
                    lxr.tgs += (q.uid, new TGParam(q.uid, q.ident, xd, lxr.tgg | TGParam.Type.Value, pa));
            }
            var dm = (lb is TableRowSet ts && cx._Ob(ts.target) is Table lt) ? xd
                : (lb.representation[ns[k.ident].Item2] ?? Domain.Content); 
            r ??= ParseSqlValue(new BTree<long,object>(DBObject._Domain,dm));
        ...
        public QlValue ParseSqlDocArray()
        {
            ...
            if (tok != Qlx.RBRACK)
            {
                if (tok != Qlx.LPAREN)
                {
                    var ls = ParseSqlValueList(BTree<long,object>.Empty);
                    Mustbe(Qlx.RBRACK);
                ...
                v += ParseSqlRow(BTree<long,object>.Empty);
            }
            while (tok == Qlx.COMMA)
            {
                Next();
