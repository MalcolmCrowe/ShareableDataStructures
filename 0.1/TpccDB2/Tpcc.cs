...
namespace Tpcc
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
        ...
        public Form1()
		{
			conn = new DB2Connection("Database=Tpcc;Server=localhost;Userid=db2admin;password=adminadmin");
            conn.Open();
            //
        ...
        public static void RecordRequest(DB2Command cmd,int cid,int tid)
        {
            if (reqs == null)
                return;
            lock (reqs)
        ...
        public static void CloseRequests()
        {
            if (reqs != null)
            {
                var conn = new DB2Connection("Database=Tpcc;Server=localhost;Userid=db2admin;password=adminadmin");
                conn.Open();
 ...
 /*           try
            {
                ...
                var conn = new DB2Connection("Data Source=.;Initial Catalog=Tpcc;Integrated Security=True");
                conn.Open();
                var cmd = conn.CreateCommand();
	...
	//			wid = int.Parse(textBox1.Text);
		...
		private void button2_Click(object sender, System.EventArgs e) // Run button for New Order
		{
            ...
            if (checkBox1.Checked)
            {
                ...
                n.db = new DB2Connection("Database=Tpcc;Server=localhost;Userid=db2admin;password=adminadmin");
                n.db.Open();
            }
			n.btn = button2;
			n.txtBox = textBox3;
